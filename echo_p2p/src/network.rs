...
use echo_chain::AcceptedBlockBundle;
...
enum PreparedSnapshotExport {
    ...
    GenerationError(echo_chain::storage::SnapshotGenerationError),
    InvalidManifest,
}
...
const SNAPSHOT_BOUNDARY_MAX_LIVE_GAP: u64 =
    echo_chain::consensus::params::RETAINED_BLOCK_SERVING_DEPTH - 6;
...
impl RelayReservations {
    fn new(target: usize, local_peer: PeerId) -> Self {
        ...
        hasher.update(b"lostecho/relay-selection/v1");
    ...
    fn candidate_rank(&self, peer: PeerId) -> [u8; 32] {
        let mut hasher = blake3::Hasher::new();
        hasher.update(b"lostecho/relay-candidate/v1");
...
/// table is small. Lostecho has its own identity-jittered, topology-bounded
...
const _: () = assert!(
    MAX_PENDING_STATE_SEGMENT_REQUESTS >= echo_chain::consensus::wire_limits::MAX_INFLIGHT_SEGMENTS
);
        ...
        let requests = {
            ...
            for (index, page) in assembly.header.descriptor_pages.iter().copied().enumerate() {
                ...
                providers.sort_by_key(|peer| {
                    let mut hasher = blake3::Hasher::new();
                    hasher.update(b"LOSTECHO/P2P/MANIFEST-PAGE-SOURCE/V1");
...
// Hard caps on incoming response sizes are shared via echo_chain::consensus::wire_limits.
...
fn retained_object_inventory_floor(tip_height: u64) -> u64 {
    tip_height.saturating_sub(echo_chain::consensus::params::RETAINED_BLOCK_SERVING_DEPTH)
}
...
fn deterministic_snapshot_boundary_window(
    ...
    let oldest = tip_height
        .saturating_sub(echo_chain::consensus::params::UNDO_RETENTION_DEPTH)
        .max(1);
...
fn local_history_step_boundary(store: &MdbxStore) -> Option<(u64, [u8; 32])> {
        ...
        .filter(|height| height % crate::protocol::SNAPSHOT_BOUNDARY_INTERVAL == 0)
    {
        ...
        let block_hash = echo_chain::hash_block_header(&header);
        ...
        let has_cached = store
            .has_any_history_step_proof_object(
                height,
                echo_chain::block_header::semantic_header_id(&header),
            )
            .ok()?;
    ...
    match object {
        ObjectId::BlockBody(expected) => {
            ...
            let bytes = match canonical {
                Some(bytes) => {
                    let matches = echo_chain::Block::from_bytes(&bytes)
                        .ok()
                        .is_some_and(|block| {
                            block.header.height == expected.claim.height
                                && echo_chain::block_header::block_id(&block.header)
            ...
            let block = echo_chain::Block::from_bytes(&bytes)
                .map_err(|error| format!("decode cached block object: {error:?}"))?;
            if block.header.height != expected.claim.height
                || echo_chain::block_header::block_id(&block.header) != expected.claim.block_hash
                || !expected.matches_bytes(&bytes)
            {
                return Ok(None);
            }
        ...
        ObjectId::Terminal(expected) => {
            ...
            let metadata =
                echo_chain::history_step::HistoryStepTerminalMetadata::decode_prefix(&bytes)
    ...
    exports
        ...
        .filter(|generation| {
            ...
            let boundary_matches = store
                ...
                .flatten()
                .is_some_and(|header| {
                    echo_chain::hash_block_header(&header) == manifest.target_hash
            ...
            let bridge_matches = store
                ...
                .flatten()
                .is_some_and(|header| {
                    echo_chain::hash_block_header(&header) == manifest.bridge_tip_hash
                });
...
fn local_history_step_terminal(
    ...
    let header = store.get_header(height).ok().flatten()?;
    if echo_chain::block_header::block_id(&header) != block_hash {
        return None;
    }
...
pub enum NetworkEvent {
    ...
    SnapshotHeadersBatch {
        ...
        start_height: u64,
        requested_count: u16,
        headers: Vec<echo_chain::block_header::BlockHeader>,
        snapshot_boundary: Option<crate::object_protocol::ChainPoint>,
    },
                    ...
                    match result {
                        ...
                        PreparedSnapshotExport::GenerationError(error) => {
                            let retry_after_tail_install = matches!(
                                error,
                                echo_chain::storage::SnapshotGenerationError::MissingBridgeTerminal(_)
                                    | echo_chain::storage::SnapshotGenerationError::MissingBoundaryTerminal(_)
                            );
...
fn forget_departed_tx(
    ...
    if mempool.try_contains(&echo_poseidon2b::primitives::TxBodyHash(txid)) != Some(true) {
                                        ...
                                        .then(|| {
                                            ...
                                            canonical.or_else(|| {
                                                store
                                                    .get_any_history_step_proof_object(
                                                        header.height,
                                                        echo_chain::block_header::semantic_header_id(
                                                            &header,
                                                        ),
                                                    )
                                                    .ok()
...
mod tests {
    ...
    fn snapshot_terminal_serving_requires_retained_suffix() {
        let retention = echo_chain::consensus::params::RECENT_BLOCK_RETENTION_DEPTH;
    ...
    fn header_inventory_serves_the_complete_retained_payload_window() {
        let serving = echo_chain::consensus::params::RETAINED_BLOCK_SERVING_DEPTH;
        let recent = echo_chain::consensus::params::RECENT_BLOCK_RETENTION_DEPTH;
        let tip = serving + 100;
    ...
    fn deterministic_snapshot_boundary_exists_for_every_bucket_residue() {
        let finality = echo_chain::consensus::params::CONSENSUS_FINALITY_DEPTH;
        for residue in 0..crate::protocol::SNAPSHOT_BOUNDARY_INTERVAL {
            ...
            assert!(tip_height - boundary <= echo_chain::consensus::params::UNDO_RETENTION_DEPTH);
            assert!(
    ...
    fn cached_boundary_proof_outlives_the_recent_suffix_window() {
        let tip_height = 100;
        let boundary_height =
            tip_height - echo_chain::consensus::params::RECENT_BLOCK_RETENTION_DEPTH - 5;
        assert!(!snapshot_suffix_is_retained(tip_height, boundary_height));
        assert!(
            tip_height - boundary_height <= echo_chain::consensus::params::UNDO_RETENTION_DEPTH,
    ...
    fn aggregate_and_individual_dns_sources_count_one_target_peer() {
        let local = PeerId::random();
        let peer = PeerId::random();
        let aggregate: Multiaddr = "/dnsaddr/echo.network".parse().unwrap();
        let individual: Multiaddr = "/dns4/seed1.lostecho.org/tcp/9500".parse().unwrap();
    ...
    fn header_batch_shape_rejects_noncontiguity_without_rehashing_links() {
        let mut first = echo_chain::consensus::genesis::genesis_header();
        first.height = 77;
        let mut second = first;
        second.height = 78;
        second.prev_block_hash = echo_chain::hash_block_header(&first);
        assert_eq!(
    ...
    fn general_header_response_is_bound_to_the_requested_range() {
        ...
        third.height = 79;
        third.prev_block_hash = echo_chain::hash_block_header(&second);
