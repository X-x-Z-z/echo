...
//! `echo_ivc_core::verifier::verify_field`.

use echo_ivc_core::challenger::Challenger;
use echo_ivc_core::field::{F128, F256};
use echo_ivc_core::field_r1cs::{CompactFieldR1cs, FieldProverRelation, FieldR1cs};
use echo_ivc_core::lincheck::{self, LocallyAuthoredFreshLincheckCapture, QuirkyPoint};
use echo_ivc_core::pcs::{self, C1QuirkyDirectClaim, Commitment, PcsParams, QuirkyDirectClaim};
use echo_ivc_core::proof::{
    ...
    ) -> (Aux, (Vec<QuirkyDirectClaim>, Vec<C1QuirkyDirectClaim>)),
{
    ...
    check_cancelled()?;
    let timing = std::env::var_os("ECHOH_FIELD_PROVE_TIMING").is_some();
...
/// `echo_ivc_core::public_io`). The witness must hold the envelope lanes in
    ...
    PostCommit: FnOnce(&[F128], &Commitment, &mut Ch) -> (Aux, Vec<QuirkyDirectClaim>),
{
    ...
    // Phase timings + resident-set size, env-gated (mirrors ECHOH_ZC_TIMING's
