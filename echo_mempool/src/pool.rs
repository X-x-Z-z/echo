...
//! `AsyncMempool` — async wrapper around the synchronous `echo_chain::Mempool`.
//!
//! ## Architecture
//!
//! ```text
...
use echo_chain::consensus::params::BLOCK_MAX_USER_PAGES;
use echo_chain::consensus::wire_limits::{MAX_AUTHORIZATION_BYTES, MAX_TX_INTENT_BYTES_GLOBAL};
use echo_chain::consensus::{fee_breakdown, tx_epoch_anchor_height_for_child};
use echo_chain::fri_state::SlotValue;
use echo_chain::Mempool;
use echo_poseidon2b::primitives::{Address, TxBodyHash};
use echo_tx::{
...
fn refresh_fee_floor(state: &mut MempoolState, config: &MempoolConfig, retained_bytes: usize) {
    state.floor.update_pressure(
        echo_chain::consensus::params::v1_1_active(state.view.tip_height.saturating_add(1)),
        state.pool.len(),
        config.capacity,
        retained_bytes,
...
fn entry_metadata(
    hash: TxBodyHash,
    entry: &echo_chain::mempool::MempoolEntry,
) -> MempoolEntryMetadata {
    MempoolEntryMetadata {
        tx_hash: hash,
        fee_microecho: entry.spend.fee,
...
/// Async, thread-safe mempool for the Lostecho full node.
///
...
impl AsyncMempool {
    ...
    pub async fn submit(
        ...
        match st.pool.admit(intent.pages, tip_height) {
            ...
            Err(echo_chain::mempool::MempoolError::Full) => {
                return Err(SubmitError::Full {
                    capacity: self.config.capacity,
                });
            }
            Err(echo_chain::mempool::MempoolError::AlreadyAdmitted) => {
                return Err(SubmitError::AlreadyAdmitted(hash));
            }
            Err(e) => {
    ...
    /// Current dynamic fee floor (μECHO).
    ...
    pub async fn verified_authorization_proof_bytes(
        ...
        use echo_gkr::WalletAuthorizationBundle;
...
fn run_admission_checks(
    ...
    if actual < required {
        return Err(SubmitError::Consensus(
            echo_chain::consensus::ConsensusError::BelowMinFee { required, actual },
        ));
    }
    ...
    if spend.epoch_anchor == [0u8; 32] {
        return Err(SubmitError::Consensus(
            echo_chain::consensus::ConsensusError::BadEpochAnchor,
        ));
    }
    ...
    for page in pages {
        for (_, inp) in page.body.live_inputs() {
            if pool_inputs.contains(&inp.slot_index) {
                return Err(SubmitError::Consensus(
                    echo_chain::consensus::ConsensusError::SlotConflict,
                ));
            }
        }
        for (_, out) in page.body.live_outputs() {
    ...
    for page in pages {
        for (_, inp) in page.body.live_inputs() {
            ...
            if (idx as u64) >= view.num_slots {
                return Err(SubmitError::Consensus(
                    echo_chain::consensus::ConsensusError::ShapeMismatch(format!(
            ...
            if actual != expected {
                ...
                return Err(SubmitError::Consensus(
                    echo_chain::consensus::ConsensusError::BadStateRoot,
                ));
            }
        }
    }
...
fn verify_intent_authorization(pages: &[TxPage], authorization_bytes: &[u8]) -> Result<(), String> {
    use echo_gkr::{verify_paged_spend_authorization, WalletAuthorizationBundle};
...
mod tests {
    ...
    use echo_chain::consensus::genesis::genesis_header;
    use echo_chain::fri_state::SlotValue;
    use echo_chain::state::ChainState;
    ...
    fn retained_intent(auth_byte: u8, auth_len: usize) -> Vec<u8> {
        let mut bytes = vec![0; echo_tx::paged_spend_authorization_wire_offset(1).unwrap()];
    ...
    async fn empty_pool_resets_stale_dynamic_fee_floor_on_block_advance() {
        use echo_chain::consensus::params::MIN_FEE_BASE;
    ...
    async fn fee_test_pool(
        ...
        AsyncMempool,
        ChainView,
        Vec<echo_poseidon2b::primitives::TxBodyHash>,
    ) {
    ...
    async fn fee_test_pool_at(
        ...
        let anchor_height = echo_chain::consensus::tx_epoch_anchor_height_for_child(tip_height + 1);
        let mut epoch_header = genesis_header();
        epoch_header.height = anchor_height;
        epoch_header.timestamp += anchor_height * echo_chain::consensus::params::BLOCK_TIME;
        let anchor = echo_chain::consensus::pow::block_id(&epoch_header);
    ...
    async fn configured_activation_uses_next_child_and_reorg_restores_legacy_policy() {
        use echo_chain::consensus::params::{MIN_FEE_BASE, V1_1_ACTIVATION_HEIGHT};
        // Run the same scenario at the configured mainnet height and with
        // echo_chain/isolated-v1-1-testnet (the shared H5 test gate).
    ...
    async fn fee_floor_is_local_and_a_fresh_pool_starts_at_base() {
        ...
        assert_eq!(
            ...
            echo_chain::consensus::params::MIN_FEE_BASE
        );
    }
    ...
    async fn growth_fee_test_pool(
        ...
        AsyncMempool,
        ChainView,
        echo_poseidon2b::primitives::TxBodyHash,
    ) {
        let genesis = genesis_header();
        let anchor = echo_chain::consensus::pow::block_id(&genesis);
    ...
    async fn mint_preference_preserves_a_real_pending_spend_for_the_next_block() {
        use echo_chain::consensus::template::{
            build_block_template, build_node_owned_block_template_avoiding_outputs,
        };
        use echo_gkr::{prove_paged_spend_authorization, OwnerAuthWitness};
        use echo_poseidon2b::primitives::{derive_address, SpendSecret};
        use echo_tx::PagedSpendIntent;
        ...
        for avoid in [false, true] {
            ...
            for tx in &block.transactions {
                echo_chain::state::apply_tx_at(&mut next_state, &tx.body, block.header.height)
                    .unwrap();
            }
            ...
            if avoid {
                ...
                let users = entries[0]
                    .pages
                    .iter()
                    .map(|page| echo_tx::Transaction::new(page.body.clone()))
                    .collect();
                ...
                for tx in &child.transactions {
                    echo_chain::state::apply_tx_at(&mut next_state, &tx.body, child.header.height)
                        .unwrap();
                }
    ...
    async fn metadata_snapshot_never_carries_retained_intent_or_authorization_bytes() {
        ...
        assert_eq!(
            ...
            echo_chain::consensus::wire_limits::MAX_MEMPOOL_BYTES
        );
        assert_eq!(
    ...
    fn admission_accepts_tip_reward_for_the_next_block() {
        use echo_chain::consensus::params::coinbase_creation_id;
        ...
        let mempool_state = |view: ChainView| MempoolState {
            pool: echo_chain::Mempool::new(16),
            view,
        ...
        let required = echo_chain::consensus::fee_breakdown(
            1,
            1,
