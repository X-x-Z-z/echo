...
fn finalize_commit(
    ...
    let timing = std::env::var_os("ECHOH_COMMIT_TIMING").is_some();
...
pub fn prefault_codeword_during<R: Send>(
    ...
    if rayon::current_num_threads() <= 1 || std::env::var_os("ECHOH_NO_PREFAULT").is_some() {
        // Truly single-threaded (or explicitly disabled): no extra OS thread;
        // commit allocates inline. ECHOH_NO_PREFAULT lets benchmarks A/B the
