...
use echo_poseidon2b::native::domain::{TAG_EXSTSLT, capacity_iv_flat};
use echo_poseidon2b::native::permutation::STATE_SIZE;

/// Domain tag for encoded-source leaf hashes — the flat image of
/// `echo_fri_binius::interleaved_commit::SOURCE_LEAF_DOMAIN`. Kept in sync
...
// `echo_chain::exact_state_hash`, whose per-permutation form is
// `echo_gkr::state_leaf_killshot::evaluate_slot_leaf`):
//
// ```text
...
mod tests {
    ...
    /// `echo_chain::exact_state_hash::slot_leaf_hash` /
    ...
    fn sponge_leaf_digest_matches_native_slot_leaf_hash() {
        ...
        use echo_poseidon2b::native::domain::{TAG_EXSTSLT, capacity_iv};
