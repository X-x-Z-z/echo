...
/// proof pool. This is a low-level executor hook used by `echo_miner` worker
...
/// `echo_ivc_prover::prover::prove_fast_core`; relation wrappers reuse this then call
...
/// mirror of `echo_ivc_prover::prover::open_claims`. Relation wrappers (e.g. the hash
...
///
/// This mirrors
/// `echo_ivc_prover::field_prover::prove_field_with_public_io_and_post_commit`:
...
/// `echo_ivc_prover::field_prover::prove_field_with_public_io_and_post_commit_context`.
...
mod tests {
    ...
    /// in `echo-ivc-prover`'s `tests/verifier_roundtrip.rs`, since they need the
    /// prove path.)
    #[test]
