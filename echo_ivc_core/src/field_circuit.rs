...
//! is bit-for-bit the `echo_ivc_core` [`F128`] field (`clmul_gcm` ==
...
//! isomorphism `echo_core::hardware::tower_to_flat_u128` and back via
...
//! evaluates in the flat basis internally (`echo_poseidon2b`'s
//! `permute_mut`).
//!
...
use echo_core::hardware::tower_to_flat_u128;
use echo_poseidon2b::native::permutation::{
...
/// sponge — the LaneExpr pattern of `echo_recursive::fs_transcript`, lifted
...
impl LayoutRecordingChallenger {
    ...
    fn record_squeeze(&mut self, value: Option<F128>) {
        if self.filled == 1 {
            ...
            let pad_tower =
                echo_core::hardware::flat_to_tower_u128(f128_to_u128(fs_pad_lane_flat()));
...
impl FsChannelUnionRecorder {
    ...
    fn permute(&mut self) {
        ...
        echo_poseidon2b::native::permutation::permute_flat_u128(&mut lanes);
    ...
    fn absorb_const(&mut self, c: F128) {
        self.cur_absorb
            .push(Some(echo_core::hardware::flat_to_tower_u128(
                Self::flat_bits(c),
            )));
...
/// (`echo_poseidon2b::channel::Poseidon2bChannel` — the `FiatShamir<Block128>`
/// implementation every `echo_gkr` verifier runs on). Unlike
...
mod tests {
    ...
    use echo_core::hardware::clmul_gcm;
    use echo_core::{Block128, TowerField};
    use echo_poseidon2b::native::permutation::Poseidon2bPermutation;
    ...
    /// the echo_ivc F128 multiplication on the same u128 bit layout, and the
    ...
    fn pow7_matches_native_sbox() {
        use echo_poseidon2b::native::permutation::sbox_x7;
    ...
    fn raw_channel_trace_lockstep_1000_random_schedules() {
        use echo_core::transcript::FiatShamir;
        use echo_poseidon2b::channel::Poseidon2bChannel;
