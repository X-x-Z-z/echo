...
use echo_mempool::{AsyncMempool, MempoolEvent};
...
impl Default for MinerConfig {
    fn default() -> Self {
        Self {
            ...
            refresh_interval_secs: echo_chain::consensus::params::BLOCK_TIME * 5,
        }
    }
}
...
pub type BlockAppliedHook = Arc<dyn Fn(&echo_chain::block::Block) + Send + Sync>;
...
pub struct BlockMiner {
    ...
    history_step_runtime: Arc<echo_recursive::acceptance::history_step::HistoryStepRuntime>,
    ghost_authorization:
...
impl BlockMiner {
    ...
    pub async fn run(mut self) {
        ...
        if echo_chain::consensus::params::ISOLATED_V1_1_TESTNET {
            if let Some(path) = std::env::var_os("ECHO_ISOLATED_MINER_START_GATE") {
                ...
                pow_res = &mut pow_handle => {
                    match pow_res {
                        Ok(Ok(Some(sol))) => {
                            ...
                            let diff_lz = echo_chain::consensus::target_leading_zero_bits(
    ...
    async fn apply_found_block(
        ...
        let (new_view, committed) = {
            ...
            tokio::task::spawn_blocking(move || {
                ...
                Ok::<_, echo_chain::storage::MdbxContextError>((view, committed))
            })
            .await
...
mod tests {
    ...
    fn payout_resolver_is_dynamic_while_configured_address_is_fixed() {
        let configured = echo_poseidon2b::primitives::Address([0x11; 32]);
        ...
        let resolver: MiningPayoutResolver = Arc::new(move || {
            echo_poseidon2b::primitives::Address([resolver_marker.load(Ordering::Relaxed); 32])
        });
        assert_eq!(
        ...
        assert_eq!(
            resolve_mining_payout(configured, Some(&resolver)),
            echo_poseidon2b::primitives::Address([0x33; 32])
        );
    }
    ...
    async fn previous_height_confirmation_does_not_cancel_fresh_pow() {
        ...
        sender
            .send(MempoolEvent::TxConfirmed {
                hash: echo_poseidon2b::primitives::TxBodyHash([0x11; 32]),
                block_height: 1,
            })
            .unwrap();
        ...
        sender
            .send(MempoolEvent::TxAdmitted {
                hash: echo_poseidon2b::primitives::TxBodyHash([0x22; 32]),
                fee: 1,
    ...
    async fn previous_template_admissions_do_not_cancel_fresh_pow() {
        ...
        for index in 0..191 {
            sender
                .send(MempoolEvent::TxAdmitted {
                    hash: echo_poseidon2b::primitives::TxBodyHash([index as u8; 32]),
                    fee: 9_000,
                    intent_bytes: Arc::from(Vec::<u8>::new().into_boxed_slice()),
                })
                .unwrap();
        ...
        sender
            .send(MempoolEvent::TxAdmitted {
                hash: echo_poseidon2b::primitives::TxBodyHash([255; 32]),
                fee: 9_000,
                intent_bytes: Arc::from(Vec::<u8>::new().into_boxed_slice()),
            })
            .unwrap();
    ...
    async fn previous_template_lag_is_cleared_but_new_lag_still_refreshes() {
        ...
        let event = || MempoolEvent::TxAdmitted {
            hash: echo_poseidon2b::primitives::TxBodyHash([7; 32]),
            fee: 9_000,
            intent_bytes: Arc::from(Vec::<u8>::new().into_boxed_slice()),
        };
        for _ in 0..32 {
