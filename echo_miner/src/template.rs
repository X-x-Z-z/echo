...
use echo_chain::consensus::difficulty::next_target;
use echo_chain::consensus::pow::block_id;
use echo_chain::consensus::template::BlockTemplate as ChainTemplate;
use echo_chain::consensus::AnchorInfo;
use echo_chain::state::ChainState;
use echo_chain::storage::{MdbxChainContext, MdbxContextError, MdbxStore};
...
pub struct BlockTemplate {
    ...
    pub(crate) prepared_state_commit: echo_chain::consensus::template::PreparedBlockStateCommit,
}
...
fn template_epoch_anchor_heights(parent_height: u64) -> Option<TemplateEpochAnchorHeights> {
    ...
    Some(TemplateEpochAnchorHeights {
        parent_terminal: echo_chain::consensus::tx_epoch_anchor_height_for_child(parent_height),
        child_transactions: echo_chain::consensus::tx_epoch_anchor_height_for_child(child_height),
    })
}
...
impl TemplateBuilder {
    ...
    pub async fn build_from_snapshot(
        ...
        self.build_from_snapshot_with_limit(
            ...
            miner_address,
            now_unix,
            echo_chain::consensus::paged_spend::BlockProofClass::B25.page_capacity(),
        )
        .await
    }
    ...
    pub async fn build_from_snapshot_with_limit(
        ...
        use echo_chain::consensus::median_time_past;
        ...
        //   See validate_timestamp in echo_chain::consensus::timestamps.
        ...
        let (authorization_bytes, groups): (Vec<_>, Vec<_>) = authorization_bytes
            ...
            .unzip();
        let mut proof_by_hash: HashMap<echo_poseidon2b::primitives::TxBodyHash, Option<Vec<u8>>> =
            authorization_bytes
                .into_iter()
        ...
        let txs: Vec<_> = groups
            ...
            .map(|page| echo_tx::Transaction::new(page.body))
            .collect();
        ...
        let template_cpu_result = install_history_step_phase_cpu(|| {
            match echo_chain::consensus::template::build_node_owned_block_template_avoiding_outputs(
                &parent,
                &state,
        ...
        let selected_stream =
            echo_chain::consensus::validate_paged_spend_transaction_stream(&inner.txs)
...
fn user_page_limit_for_child(parent_height: u64, max_effective_pages: usize) -> Option<usize> {
    user_page_limit_with_activation(
        parent_height,
        max_effective_pages,
        echo_chain::consensus::params::V1_1_ACTIVATION_HEIGHT,
    )
}
...
fn user_page_limit_with_activation(
    ...
    let child_height = parent_height.checked_add(1)?;
    let consensus_max = echo_chain::consensus::params::BLOCK_MAX_USER_PAGES;
    ...
    let system_positions = usize::from(
        echo_chain::consensus::development_allocation::development_payout_due(child_height),
    );
    Some(
        max_effective_pages
            .min(producer_max)
...
mod tests {
    ...
    fn payout_position_stays_inside_the_selected_proof_class() {
        use echo_chain::consensus::development_allocation::{
