    ...
    pub fn update(&mut self, message: Message) -> Task<Message> {
        match message {
            ...
            Message::SubmitSend => {
                ...
                let amount_microecho = match parse_echo_amount(&self.send_amount) {
                    Ok(amount) => amount,
                    Err(error) => {
                ...
                let fee_microecho = match parse_optional_echo_fee(&self.send_fee) {
                    Ok(fee) => fee,
                    Err(error) => {
                        self.send_error = Some(error);
                        return Task::none();
                ...
                return Task::perform(
                    async move {
                        backend
                            .send_payment(recipient, amount_microecho, fee_microecho)
                            .await
                    },
...
fn parse_echo_amount(input: &str) -> Result<u64, String> {
    let normalized = input.trim().replace(',', ".");
    if normalized.is_empty() {
        return Err("Enter an amount.".into());
    }
    if normalized.starts_with('-') || normalized.starts_with('+') {
        return Err("Amount must be a positive ECHO value.".into());
    }
    ...
    {
        return Err("Use a decimal ECHO amount, for example 12.500000.".into());
    }
    if fractional.len() > 6 {
        return Err("ECHO supports at most 6 decimal places.".into());
    }
    let whole = whole
        .parse::<u64>()
    ...
    let amount = whole
        ...
        .ok_or_else(|| "Amount is too large.".to_string())?;
    if amount == 0 {
        return Err("Amount must be at least 0.000001 ECHO.".into());
    }
    Ok(amount)
}

fn parse_optional_echo_fee(input: &str) -> Result<u64, String> {
    if input.trim().is_empty() {
        return Ok(0);
    }
    parse_echo_amount(input).map_err(|error| format!("Invalid network fee: {error}"))
}
...
mod tests {
    use super::{
        normalize_utxo_page_after_refresh, parse_echo_amount, parse_optional_echo_fee,
        record_snapshot_refresh_failure, utxo_page_count_for, BackendState,
    };

    #[test]
    fn parses_echo_without_floating_point() {
        assert_eq!(parse_echo_amount("1").unwrap(), 1_000_000);
        assert_eq!(parse_echo_amount("0.000001").unwrap(), 1);
        assert_eq!(parse_echo_amount("12,5").unwrap(), 12_500_000);
        assert!(parse_echo_amount("0").is_err());
        assert!(parse_echo_amount("1.0000001").is_err());
        assert!(parse_echo_amount("1.2.3").is_err());
    }

    #[test]
    fn parses_blank_fee_as_automatic_and_explicit_fee_exactly() {
        assert_eq!(parse_optional_echo_fee("").unwrap(), 0);
        assert_eq!(parse_optional_echo_fee("   ").unwrap(), 0);
        assert_eq!(parse_optional_echo_fee("0.01").unwrap(), 10_000);
        assert!(parse_optional_echo_fee("0").is_err());
        assert!(parse_optional_echo_fee("fee").is_err());
    }
