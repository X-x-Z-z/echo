...
//! It supervises the `lostecho` daemon and talks to its loopback JSON-RPC
...
pub struct PaymentSubmission {
    ...
    pub txid: String,
    pub amount_microecho: u64,
    pub fee_microecho: u64,
...
pub struct ConsolidationPlan {
    pub input_value_microecho: u64,
    pub fee_microecho: u64,
    pub output_value_microecho: u64,
    pub balance_before_microecho: u64,
    pub balance_after_microecho: u64,
    pub input_count: usize,
...
fn mock_consolidation_plan() -> ConsolidationPlan {
    ...
    let mut spendable = snapshot
        ...
        .collect::<Vec<_>>();
    spendable.sort_by_key(|utxo| (utxo.value_microecho, utxo.slot_index));
    ...
    let input_value_microecho = selected
        .iter()
        .fold(0u64, |sum, utxo| sum.saturating_add(utxo.value_microecho));
    let fee_microecho = 5_000u64
    ...
    ConsolidationPlan {
        ...
        fee_microecho,
        output_value_microecho: input_value_microecho.saturating_sub(fee_microecho),
        balance_before_microecho: address.balance_microecho,
        balance_after_microecho: address.balance_microecho.saturating_sub(fee_microecho),
        input_count,
        untouched_count,
...
impl Backend {
    ...
    pub async fn node_log_tail(&self, max_bytes: u64, max_lines: usize) -> Result<String, String> {
        let log_path = self.config_snapshot()?.data_dir.join("lostecho-node.log");
    ...
    pub async fn send_payment(
        ...
        if self.is_mock() {
            return Ok(PaymentSubmission {
                ...
                amount_microecho,
                fee_microecho: if fee_microecho == 0 {
                    5_800
                } else {
                    fee_microecho
                },
        ...
        let result = self
            .rpc_with_timeout::<WalletSendResult>(
                "walletSend",
                json!([recipient.clone(), amount_microecho, fee_microecho]),
        ...
        Ok(PaymentSubmission {
            recipient,
            txid: result.txid,
            amount_microecho: result.amount_microecho,
            fee_microecho: result.fee_microecho,
    ...
    pub async fn snapshot(&self, mining_page: u32) -> Result<BackendSnapshot, String> {
        ...
        let circulating_supply_microecho = chain
            .circulating_supply_microecho
            .parse::<u128>()
        ...
        let mut address_snapshots = addresses
            ...
            .map(|address| {
                ...
                AddressSnapshot {
                    ...
                    label,
                    balance_microecho: if is_active {
                        balance.balance_microecho
                    } else {
                        0
                    },
                    ...
                    pending_outbound_microecho: if is_active {
                        balance.pending_outbound_microecho
                    } else {
                        0
                    },
                    incoming_microecho: if is_active {
        ...
        if address_snapshots.is_empty() {
            ...
            address_snapshots.push(AddressSnapshot {
                ...
                label,
                balance_microecho: balance.balance_microecho,
                utxo_count: balance.utxo_count,
                reserved_utxo_count: wallet_utxos.iter().filter(|utxo| utxo.reserved).count(),
                pending_outbound_microecho: balance.pending_outbound_microecho,
                incoming_microecho: balance.pending_incoming_microecho,
            });
        }
        ...
        let utxos = wallet_utxos
            ...
            .map(|utxo| {
                ...
                UtxoSnapshot {
                    ...
                    value_microecho: utxo.value_microecho,
        ...
        let snapshot = AppSnapshot {
            network: NetworkSnapshot {
                ...
                memory_total_bytes,
                circulating_supply_microecho,
                block_reward_microecho: mining.block_reward_microecho,
            ...
            mined_blocks: MinedBlocksSnapshot {
                ...
                blocks: mined_blocks
                    ...
                    .map(|block| MinedBlockSnapshot {
                        ...
                        reward_microecho: block.reward_microecho,
    ...
    pub async fn explorer_search(
        ...
        if query.to_ascii_lowercase().starts_with("o1") {
            ...
            let slots = self
                ...
                .collect::<Vec<_>>();
            let balance_microecho = slots
                .iter()
                .map(|slot| u128::from(slot.value_microecho))
                .sum();
    ...
    async fn rpc_with_timeout<T: DeserializeOwned>(
        ...
        let body = RpcRequest {
            jsonrpc: "2.0",
            id,
            method: format!("lostecho_{method}"),
            params,
        };
    ...
    fn spawn(&self, mode: NodeMode, selected_threads: usize, genesis: bool) -> Result<(), String> {
        ...
        let config_path = config.data_dir.join("lostecho-gui.toml");
        let log_path = config.data_dir.join("lostecho-node.log");
                ...
                if let Some(child) = state.child.as_mut() {
                        ...
                        .map_err(|error| format!("inspect node process: {error}"))?
                    {
                        ...
                        return Err(format!(
                            ...
                            config.data_dir.join("lostecho-node.log").display()
                        ));
                    }
                }
            }
...
impl BackendConfig {
    fn from_env() -> Self {
        let settings_path = std::env::var_os("ECHO_GUI_SETTINGS_FILE")
            .map(PathBuf::from)
        ...
        let data_dir = std::env::var_os("ECHO_GUI_DATA_DIR")
            .map(PathBuf::from)
            .or_else(|| persisted.as_ref().map(|settings| settings.data_dir.clone()))
            .unwrap_or_else(default_data_dir);
        let rpc_url = std::env::var("ECHO_RPC").unwrap_or_else(|_| DEFAULT_RPC_URL.into());
        let rpc_listen = std::env::var("ECHO_GUI_RPC_LISTEN").unwrap_or_else(|_| {
            rpc_listen_from_url(&rpc_url)
                .unwrap_or(DEFAULT_RPC_LISTEN)
                .into()
        });
        let p2p_listen = std::env::var("ECHO_GUI_P2P_LISTEN").unwrap_or_else(|_| {
            persisted
                .as_ref()
                .map(|settings| settings.p2p_listen.clone())
                .unwrap_or_else(|| DEFAULT_P2P_LISTEN.into())
        });
        let node_binary = std::env::var_os("ECHO_GUI_NODE_BIN")
            .map(PathBuf::from)
            .unwrap_or_else(find_node_binary);
        let seeds = std::env::var("ECHO_GUI_SEEDS")
            .ok()
            .map(|seeds| {
                seeds
                    .split(',')
        ...
        let mock = std::env::var("ECHO_GUI_MOCK")
...
async fn run_node_maintenance(
    ...
    command
        .arg("--config")
        .arg(config.data_dir.join("lostecho-gui.toml"))
        .arg("--data-dir")
        .arg(&config.data_dir)
        .arg("--log")
        .arg("error")
...
#[cfg(target_os = "windows")]
const SIBLING_NODE_NAMES: &[&str] = &["lostecho-node.exe", "lostecho.exe"];
#[cfg(target_os = "macos")]
const SIBLING_NODE_NAMES: &[&str] = &["lostecho-node", "lostecho"];
#[cfg(not(any(target_os = "windows", target_os = "macos")))]
const SIBLING_NODE_NAMES: &[&str] = &["lostecho"];
...
struct ChainInfo {
    ...
    log_slots: u32,
    circulating_supply_microecho: String,
}
...
struct RpcRecentTransaction {
    ...
    address_spent_microecho: Option<String>,
    address_received_microecho: Option<String>,
}
...
impl RpcRecentTransaction {
    fn into_snapshot(self) -> RecentTransactionSnapshot {
        RecentTransactionSnapshot {
            ...
            fee_microecho: self.fee_microecho,
            coinbase: self.coinbase,
            development_payout: self.development_payout,
            address_spent_microecho: self.address_spent_microecho,
            address_received_microecho: self.address_received_microecho,
        }
    }
}
...
struct WalletBalance {
    balance_microecho: u64,
    utxo_count: usize,
    pending_outbound_microecho: u64,
    #[serde(default)]
    pending_incoming_microecho: u64,
}
...
struct WalletConsolidationPlan {
    input_value_microecho: u64,
    fee_microecho: u64,
    output_value_microecho: u64,
    balance_before_microecho: u64,
    balance_after_microecho: u64,
    input_count: usize,
...
impl From<WalletConsolidationPlan> for ConsolidationPlan {
    fn from(plan: WalletConsolidationPlan) -> Self {
        Self {
            ...
            fee_microecho: plan.fee_microecho,
            output_value_microecho: plan.output_value_microecho,
            balance_before_microecho: plan.balance_before_microecho,
            balance_after_microecho: plan.balance_after_microecho,
            input_count: plan.input_count,
...
impl From<WalletConsolidationResult> for ConsolidationSubmission {
    fn from(result: WalletConsolidationResult) -> Self {
        Self {
            txid: result.txid,
            input_value_microecho: result.input_value_microecho,
            fee_microecho: result.fee_microecho,
            output_value_microecho: result.output_value_microecho,
            input_count: result.input_count,
            output_count: result.output_count,
...
impl RpcWalletReceipt {
    fn into_snapshot(self) -> ReceiptSnapshot {
        ReceiptSnapshot {
            ...
            height: self.height,
            timestamp: self.timestamp,
            amount_microecho: self.amount_microecho,
            fee_microecho: self.fee_microecho,
        ...
        ReceiptSummarySnapshot {
            ...
            outputs: self
                ...
                .map(|output| ReceiptOutputSnapshot {
                    ...
                    amount_microecho: output.amount_microecho,
                    owner: output.owner,
                })
                .collect(),
        }
...
impl RpcRetainedBlock {
    fn into_snapshot(self) -> RetainedBlockSnapshot {
        RetainedBlockSnapshot {
            ...
            live_inputs: self.live_inputs,
            live_outputs: self.live_outputs,
            reward_microecho: self.reward_microecho,
            total_fees_microecho: self.total_fees_microecho,
            ...
            transactions: self
                ...
                .map(|transaction| BlockTransactionSnapshot {
                    ...
                    fee_microecho: transaction.fee_microecho,
                    ...
                    input_sum_microecho: transaction.input_sum_microecho,
                    output_sum_microecho: transaction.output_sum_microecho,
                    ...
                    inputs: transaction
                        ...
                        .map(|input| BlockTransactionInputSnapshot {
                            ...
                            lane: input.lane,
                            slot_index: input.slot_index,
                            amount_microecho: input.amount_microecho,
...
struct RpcBlockTransaction {
    ...
    input_sum_microecho: String,
    output_sum_microecho: String,
...
fn mock_block_details(height: u64) -> BlockDetailsSnapshot {
    ...
    BlockDetailsSnapshot {
        ...
        retained: Some(RetainedBlockSnapshot {
            ...
            live_inputs: 9,
            live_outputs: 5,
            reward_microecho: 50_018_500,
            total_fees_microecho: "18500".into(),
            ...
            transactions: vec![
                BlockTransactionSnapshot {
                    ...
                    output_sum_microecho: "50018500".into(),
                ...
                BlockTransactionSnapshot {
                    ...
                    live_inputs: 9,
                    live_outputs: 4,
                    fee_microecho: 12_000,
                    coinbase: false,
                    development_payout: false,
                    epoch_anchor: format!("{:064x}", height.saturating_sub(1)),
                    input_owner: Some(miner.into()),
                    input_sum_microecho: "9000000".into(),
                    output_sum_microecho: "8988000".into(),
                    page_hashes: vec![
        ...
        for transaction in retained.transactions {
            let (spent, received) = if let Some(owner) = address {
                let spent = transaction
                    ...
                    .map(|_| transaction.input_sum_microecho.clone())
                    .unwrap_or_else(|| "0".into());
                let received = transaction
                    .outputs
                    .iter()
                    .filter(|output| output.owner == owner)
                    .map(|output| u128::from(output.amount_microecho))
                    .sum::<u128>()
                    .to_string();
            ...
            transactions.push(RecentTransactionSnapshot {
                ...
                coinbase: transaction.coinbase,
                development_payout: transaction.development_payout,
                address_spent_microecho: spent,
                address_received_microecho: received,
            });
        }
    }
        ...
        let slots = if preview.active_address().address == query {
            preview
                ...
                .map(|slot| ExplorerSlotSnapshot {
                    ...
                    value_microecho: slot.value_microecho,
...
fn mock_receipt_records() -> Vec<ReceiptSnapshot> {
    ...
    (0..12u64)
        .map(|index| ReceiptSnapshot {
            ...
            amount_microecho: 12_500_000 + index * 750_000,
            fee_microecho: 9_000 + index * 100,
...
fn mock_receipt_hex(txid: &str) -> String {
    ...
    let mut payload = format!("LOSTECHO_RECEIPT:{txid}:").into_bytes();
...
fn mock_verify_receipt_hex(receipt_hex: &str) -> ReceiptVerificationSnapshot {
    let txid = hex::decode(receipt_hex)
        ...
        .and_then(|text| {
            let payload = text.strip_prefix("LOSTECHO_RECEIPT:")?;
...
mod tests {
    ...
    async fn conditional_wallet_rpc_serializes_refreshes_and_falls_back_only_for_old_nodes() {
        ...
        let utxo = json!({
            "slot_index": 5, "value_microecho": 7, "creation_id": 9,
        ...
        let server = tokio::spawn(async move {
            for (method, params, mut response) in exchanges {
                ...
                assert_eq!(request["method"], format!("lostecho_{method}"));
    ...
    async fn node_log_tail_reads_only_the_latest_complete_lines() {
        let directory = tempfile::tempdir().unwrap();
        let path = directory.path().join("lostecho-node.log");
        let contents = (0..120)
    ...
    fn mock_consolidation_quote_obeys_the_b25_boundary() {
        ...
        assert_eq!(
            plan.input_value_microecho,
            plan.output_value_microecho + plan.fee_microecho
        );
        assert_eq!(
            plan.balance_after_microecho,
            plan.balance_before_microecho - plan.fee_microecho
        );
    }
    ...
    fn gui_node_settings_round_trip_through_owner_only_file() {
        ...
        let config = BackendConfig {
            ...
            data_dir: directory.path().join("node-data"),
            node_binary: PathBuf::from("lostecho"),
            seeds: vec!["seed-a.example:9600".into(), "dnsaddr:echo.network".into()],
