...
fn header(app: &App, _compact: bool) -> Element<'_, Message> {
    ...
    let identity = row![coin, text("Lostecho").size(15).color(theme::MUTED),]
        .spacing(0)
        .align_y(Alignment::Center);
...
fn node_log_terminal(app: &App) -> Element<'_, Message> {
    ...
    let output = text_editor(&app.node_log)
        .placeholder(translate("No lostecho-node.log output yet…"))
    ...
    let mut body = column![
        row![
            ...
            text("lostecho-node.log").size(12).color(theme::DIM),
            Space::new().width(Length::Fill),
