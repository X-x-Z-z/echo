    ...
    let body: Element<'_, Message> = if let Some(retained) = &details.retained {
        let summary = row![
            ...
            metric(
                ...
                format!(
                    "{} ↻",
                    crate::model::format_microecho(retained.reward_microecho)
                ),
                theme::ACCENT,
            ),
        ]
        ...
        let sizes = row![
            ...
            detail("FEES", format!("{} μECHO", retained.total_fees_microecho)),
        ]
        .spacing(18);
        let tx_header = container(
            row![
                ...
                table_cell("FEE / μECHO".into(), 4, theme::INK),
                text("OPEN")
                    .size(13)
                    .color(theme::INK)
            ...
            tx_rows = tx_rows.push(
                button(
                    row![
                        ...
                        table_cell(transaction.fee_microecho.to_string(), 4, theme::WARNING),
                        text("VIEW")
                            .size(12)
                            .color(theme::CYAN)
                            .width(Length::Fixed(52.0)),
...
fn transaction_details<'a>(
    ...
    let metrics = row![
        ...
        metric(
            ...
            format!(
                "{} ↻",
                crate::model::format_microecho(transaction.fee_microecho)
            ),
            if transaction.fee_microecho == 0 {
                theme::DIM
            } else {
                theme::WARNING
            },
    ...
    let flow = if transaction.development_payout {
        ...
        column![
            ...
            text(format!(
                ...
                format_microecho_string(&transaction.output_sum_microecho)
            ))
            .size(13)
            .color(theme::ACCENT),
        ]
        ...
        column![
            ...
            row![
                detail(
                    ...
                    format!(
                        "{} ↻",
                        format_microecho_string(&transaction.input_sum_microecho)
                    ),
                ),
                detail(
                    "OUTPUT TOTAL",
...
fn transaction_inputs(
    ...
    let header = container(
        row![
            ...
            table_cell("AMOUNT / ECHO".into(), 5, theme::INK),
        ]
        .spacing(6),
    )
    .padding([6, 8])
        ...
        rows = rows.push(
            container(
                row![
                    ...
                    table_cell(
                        crate::model::format_microecho(input.amount_microecho),
                        5,
                        theme::TEXT,
                    ),
                ]
            ...
            container(
                column![
                    row![
                        ...
                        text(format!(
                            "{} ↻{}",
                            crate::model::format_microecho(output.amount_microecho),
...
fn format_microecho_string(value: &str) -> String {
