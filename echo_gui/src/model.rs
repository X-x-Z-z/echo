...
pub struct NetworkSnapshot {
    ...
    pub circulating_supply_microecho: u128,
    pub block_reward_microecho: u64,
...
pub struct AddressSnapshot {
    ...
    pub address: String,
    pub label: String,
    pub balance_microecho: u64,
    pub utxo_count: usize,
    pub reserved_utxo_count: usize,
    pub pending_outbound_microecho: u64,
    pub incoming_microecho: u64,
}

impl AddressSnapshot {
    pub fn balance(&self) -> String {
        format_microecho(self.balance_microecho)
    }

    pub fn pending_outbound(&self) -> String {
        format_microecho(self.pending_outbound_microecho)
    }

    pub fn incoming(&self) -> String {
        format_microecho(self.incoming_microecho)
    }
...
impl UtxoSnapshot {
    pub fn value(&self) -> String {
        format_microecho(self.value_microecho)
    }
}
...
impl MinedBlockSnapshot {
    pub fn reward(&self) -> String {
        format_microecho(self.reward_microecho)
    }
...
pub struct RetainedBlockSnapshot {
    ...
    pub reward_microecho: u64,
    pub total_fees_microecho: String,
    pub block_bytes: u64,
...
pub struct BlockTransactionSnapshot {
    ...
    pub input_sum_microecho: String,
    pub output_sum_microecho: String,
...
pub struct RecentTransactionSnapshot {
    ...
    pub coinbase: bool,
    pub development_payout: bool,
    pub address_spent_microecho: Option<String>,
    pub address_received_microecho: Option<String>,
}
...
impl AppSnapshot {
    ...
    pub fn offline(available_threads: usize) -> Self {
        Self {
            network: NetworkSnapshot {
                ...
                circulating_supply_microecho: 0,
                block_reward_microecho: 50_000_000,
    ...
    pub fn design_preview() -> Self {
        ...
        const PREVIEW_BALANCE_MICROECHO: u64 = 100_000_000_000;
        ...
        let base_value = PREVIEW_BALANCE_MICROECHO / PREVIEW_UTXO_COUNT as u64;
        let remainder = PREVIEW_BALANCE_MICROECHO % PREVIEW_UTXO_COUNT as u64;
        let utxos = (0..PREVIEW_UTXO_COUNT)
            .rev()
            .map(|index| UtxoSnapshot {
                slot_index: 73 + index as u32 * 73,
                value_microecho: base_value + u64::from((index as u64) < remainder),
        ...
        let mut addresses = vec![
            AddressSnapshot {
                ...
                label: "Main".into(),
                balance_microecho: PREVIEW_BALANCE_MICROECHO,
                utxo_count: PREVIEW_UTXO_COUNT,
                reserved_utxo_count: 0,
                pending_outbound_microecho: 100_000_000_000,
                incoming_microecho: 100_000_000_000,
            },
            AddressSnapshot {
                key_index: 1,
                address: "o17z7pfmh09rjztwga8y9pzpy05ncznl5teqe23a48d0sumjcnrlaszlk2vj".into(),
                label: "Savings".into(),
                balance_microecho: 312_000_000,
                utxo_count: 6,
                reserved_utxo_count: 0,
                pending_outbound_microecho: 0,
                incoming_microecho: 0,
        ...
        Self {
            network: NetworkSnapshot {
                ...
                circulating_supply_microecho: 118_982_430_000,
                block_reward_microecho: 50_000_000,
...
pub fn format_microecho(value: u64) -> String {
    let whole = value / 1_000_000;
    let fractional = value % 1_000_000;
    format!("{whole}.{fractional:06}")
}

pub fn format_microecho_trimmed(value: u64) -> String {
    let mut formatted = format_microecho(value);
...
///
/// The caller supplies μECHO; the returned text deliberately omits the final
/// `ECHO` suffix so views can retain it unconditionally at every window size.
pub fn format_compact_microecho(value: u128) -> String {
    const UNITS: [(&str, u128); 5] = [
        ("", 1_000_000),
        ("K", 1_000_000_000),
        ("M", 1_000_000_000_000),
...
mod tests {
    use super::{
        ...
        format_compact_microecho, format_creation_origin, format_expected_pow_hashes,
        format_hashrate, format_microecho_trimmed, format_pow_work_change, AppSnapshot,
        SensitiveString,
    };
    ...
    fn compacts_network_supply_without_losing_the_unit_scale() {
        assert_eq!(format_compact_microecho(0), "0");
        assert_eq!(format_compact_microecho(950_000_000), "950");
        assert_eq!(format_compact_microecho(999_995_000), "1K");
        assert_eq!(format_compact_microecho(118_982_430_000), "118.98K");
        assert_eq!(format_compact_microecho(105_120_000_000_000), "105.12M");
        assert_eq!(format_compact_microecho(1_230_000_000_000_000), "1.23B");
        assert_eq!(
            format_compact_microecho(12_350_000_000_000_000_000),
            "12.35T"
        );
    }
    ...
    fn trims_only_insignificant_reward_decimals() {
        assert_eq!(format_microecho_trimmed(50_000_000), "50");
        assert_eq!(format_microecho_trimmed(12_500_000), "12.5");
        assert_eq!(format_microecho_trimmed(3_125_000), "3.125");
        assert_eq!(format_microecho_trimmed(1_562_500), "1.5625");
        assert_eq!(format_microecho_trimmed(1_000_001), "1.000001");
    }
