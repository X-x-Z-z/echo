...
// Copyright (C) 2026 Lostecho. All rights reserved.
...
//! architecture's binary. `ECHO_CPU_BACKEND` is a diagnostic/test hook which
...
fn backend_request() -> BackendRequest {
    ...
    *REQUEST.get_or_init(|| {
        let Ok(value) = std::env::var("ECHO_CPU_BACKEND") else {
            return BackendRequest::Auto;
        };
        parse_backend_request(&value).unwrap_or_else(|| {
            panic!(
                "invalid ECHO_CPU_BACKEND={value:?}; expected auto, scalar, pclmul, avx2, \
...
fn select_backend(caps: CpuCapabilities, request: BackendRequest) -> CpuBackend {
    #[cfg(target_arch = "x86_64")]
    {
        ...
        return match request {
            ...
            BackendRequest::Neon | BackendRequest::NeonPmull => {
                panic!("ECHO_CPU_BACKEND requests an AArch64 backend on x86_64")
            }
            forced => panic!(
                "ECHO_CPU_BACKEND={forced:?} is not supported by this x86_64 CPU; detected \
                 capabilities: {caps:?}"
            ),
        };
    }
    ...
    #[cfg(target_arch = "aarch64")]
    {
        ...
        return match request {
            ...
            BackendRequest::Pclmul | BackendRequest::Avx2 | BackendRequest::Avx512 => {
                panic!("ECHO_CPU_BACKEND requests an x86_64 backend on AArch64")
            }
            forced => panic!(
                "ECHO_CPU_BACKEND={forced:?} is not supported by this AArch64 CPU; detected \
                 capabilities: {caps:?}"
            ),
        };
    }
    ...
    #[cfg(not(any(target_arch = "x86_64", target_arch = "aarch64")))]
    {
        match request {
            ...
            forced => panic!(
                "ECHO_CPU_BACKEND={forced:?} is not supported on architecture {}",
                std::env::consts::ARCH
            ),
        }
    }
