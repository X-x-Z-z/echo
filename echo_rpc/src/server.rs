...
use echo_chain::consensus::params::RECENT_BLOCK_RETENTION_DEPTH;
use echo_chain::consensus::pow::{
    block_id, pow_header_fields, validate_pow, POW_NONCE_FIELD_INDEX,
};
use echo_chain::consensus::wire_limits::{
    hex_chars_for_bytes, MAX_RPC_RECEIPT_BYTES, MAX_RPC_SALT_BYTES, MAX_TX_INTENT_BYTES_GLOBAL,
};
use echo_chain::storage::MdbxChainContext;
use echo_mempool::AsyncMempool;
use echo_miner::template::{TemplateBuilder, TemplateChainSnapshot};
use echo_miner::{AdaptiveProofCapacity, PreparedBlockAttempt};

use crate::api::LostechoApiServer;
use crate::types::{
...
fn mined_record_is_canonical(
    ...
    canonical_header: &echo_chain::block_header::BlockHeader,
) -> bool {
...
fn block_header_info(header: &echo_chain::BlockHeader) -> BlockHeaderInfo {
...
fn retained_transaction_summaries(
    ...
    bundle_bytes: &[u8],
    address: Option<&echo_poseidon2b::primitives::Address>,
) -> Result<Vec<RecentTransactionInfo>, String> {
    let bundle = echo_chain::AcceptedBlockBundle::decode(bundle_bytes)
        .map_err(|error| format!("decode retained block bundle: {error}"))?;
    let block = echo_chain::Block::from_bytes(bundle.block_bytes())
    ...
    let stream = echo_chain::validate_block_page_stream(&block.transactions)
        .map_err(|error| format!("decode retained transaction stream: {error}"))?;
    let logical_txids = echo_chain::try_compute_logical_txids(&block.transactions)
    ...
    if coinbase_received.is_none_or(|received| received > 0) {
        transactions.push(RecentTransactionInfo {
            ...
            input_owner: None,
            input_sum_microecho: "0".into(),
            output_sum_microecho: coinbase_output_sum.to_string(),
            address_spent_microecho: address.map(|_| "0".into()),
            address_received_microecho: coinbase_received.map(|value| value.to_string()),
        });
    }
    ...
    if stream.has_development_payout {
        ...
        if payout_received.is_none_or(|received| received > 0) {
            transactions.push(RecentTransactionInfo {
                ...
                input_owner: None,
                input_sum_microecho: "0".into(),
                output_sum_microecho: payout_output_sum.to_string(),
                address_spent_microecho: address.map(|_| "0".into()),
                address_received_microecho: payout_received.map(|value| value.to_string()),
            });
        }
    }
    ...
    for (index, group) in stream.groups.iter().enumerate() {
        ...
        transactions.push(RecentTransactionInfo {
            ...
            fee_microecho: group.spend.fee,
            coinbase: false,
            development_payout: false,
            input_owner: Some(group.spend.input_owner.to_bech32()),
            input_sum_microecho: group.spend.input_sum.to_string(),
            output_sum_microecho: group.spend.output_sum.to_string(),
            address_spent_microecho: address_spent,
            address_received_microecho: address_received,
        });
    }
    Ok(transactions)
}
...
fn read_canonical_slot(
    ...
    slot_index: u32,
    segment_cache: &mut HashMap<u16, echo_chain::segmented_state::SegmentColumns>,
) -> Result<echo_chain::fri_state::SlotValue, String> {
    use echo_chain::fri_state::SlotValue;
...
fn encode_pow_fields_hex(header: &echo_chain::BlockHeader) -> String {
...
fn mix_slot_hint_seed(tip_seed: u64, salt: u64) -> u64 {
    ...
    echo_chain::consensus::allocator::splitmix64(&mut s)
}

fn fee_breakdown_info(
    breakdown: echo_chain::consensus::FeeBreakdown,
    relay_floor: u64,
    paid_total: u64,
) -> FeeBreakdownInfo {
...
fn mempool_tx_info(entry: echo_mempool::MempoolEntryMetadata) -> MempoolTxInfo {
    use echo_chain::consensus::paged_spend::BlockProofClass;
    ...
    MempoolTxInfo {
        ...
        fee_microecho: entry.fee_microecho,
...
fn validate_tx_counts(n_inputs: usize, n_outputs: usize) -> Result<(), ErrorObject<'static>> {
    if (1..=echo_tx::MAX_PAGED_SPEND_INPUTS).contains(&n_inputs)
        && (1..=echo_tx::MAX_PAGED_SPEND_OUTPUTS).contains(&n_outputs)
    {
        Ok(())
    } else {
        Err(rpc_err(format!(
            "canonical PagedSpend supports 1..={} inputs and 1..={} outputs, got {n_inputs} and {n_outputs}",
            echo_tx::MAX_PAGED_SPEND_INPUTS,
            echo_tx::MAX_PAGED_SPEND_OUTPUTS,
        )))
    }
}
...
fn seed_from_salt_hex(salt_hex: &str) -> Result<u64, ErrorObject<'static>> {
    ...
    for chunk in bytes.chunks(8) {
        ...
        acc = echo_chain::consensus::allocator::splitmix64(&mut acc);
    }
    Ok(acc)
}
...
pub struct RpcHandler {
    ...
    pub p2p_cmd: echo_p2p::NetworkCommandSender,
    ...
    pub p2p_health: tokio::sync::watch::Receiver<echo_p2p::P2PHealthSnapshot>,
    ...
    pub canonical_tip_changes: tokio::sync::watch::Sender<echo_p2p::object_protocol::ChainPoint>,
    ...
    pub mining_payout_address: Option<echo_poseidon2b::primitives::Address>,
    ...
    pub history_step_runtime:
        Option<Arc<echo_recursive::acceptance::history_step::HistoryStepRuntime>>,
...
struct WalletSubmissionRequest {
    to_address: [u8; 32],
    amount_microecho: u64,
    fee_microecho: u64,
    /// Only an Auto payment may be replanned after a pre-admission fee error.
    automatic_fee: bool,
    expected_input_count: usize,
    expected_output_count: usize,
    pending_history_amount_microecho: u64,
    build: WalletSubmissionBuild,
    failure_label: &'static str,
}

fn is_wallet_fee_rejection(error: &echo_mempool::SubmitError) -> bool {
    matches!(
        error,
        echo_mempool::SubmitError::Consensus(
            echo_chain::consensus::ConsensusError::BelowMinFee { .. }
        )
    )
}
...
impl RpcHandler {
    ...
    fn resolved_mining_payout(&self) -> RpcResult<echo_poseidon2b::primitives::Address> {
    ...
    async fn submit_wallet_transaction(
        ...
        let WalletSubmissionRequest {
            ...
            pending_history_amount_microecho,
            build,
            failure_label,
        } = request;
        ...
        for attempt in 0..3u32 {
            ...
            if replan_fee {
                ...
                let plan = self
                    .wallet
                    .plan_send(amount_microecho, None, active_slot_count, log_slots, floor)
                    .map_err(wallet_plan_error)?;
                fee_microecho = plan.fee_microecho;
            ...
            let (intent_bytes, input_slots) = match tokio::task::spawn_blocking(move || {
                echo_miner::install_wallet_proof_cpu(|| match build_attempt {
            ...
            let intent = match echo_tx::PagedSpendIntent::from_bytes(&intent_bytes) {
                Ok(intent) => intent,
                Err(error) => {
            ...
            let facts = echo_tx::validate_paged_spend(&intent.pages)
            ...
            let output_slots: Vec<u32> = intent
                ...
                .collect();
            if actual_fee != fee_microecho
                || input_count != expected_input_count
                || output_count != expected_output_count
            {
                last_error = format!(
                    "wallet builder diverged from plan: expected fee/counts {fee_microecho}/{expected_input_count}/{expected_output_count}, got {actual_fee}/{input_count}/{output_count}"
                );
                break;
            }
    ...
    async fn prepare_external_mining_attempt(
        self,
        requested: Option<echo_poseidon2b::primitives::Address>,
        ...
        let tx_output_counts: Vec<usize> = tmpl
            ...
            .collect();
        let coinbase_value_microecho: u64 = tmpl
            .inner
            .coinbase
            .body
            .live_outputs()
        ...
        let claimable_fees_microecho = tmpl
            .inner
            .txs
            .iter()
            .map(|tx| {
                echo_chain::consensus::claimable_fee_for_tx_body(
                    &tx.body,
        ...
        let (prepared, prepare_elapsed) = tokio::task::spawn_blocking(move || {
            ...
            let result = echo_miner::install_history_step_phase_cpu(|| {
        ...
        let (previous_page_limit, next_page_limit, b25_prepare_ms_ewma, b255_prepare_ms_ewma) = {
            ...
            (
                previous,
                capacity.page_limit(),
                capacity.prepare_ms_ewma(echo_chain::consensus::paged_spend::BlockProofClass::B25),
                capacity.prepare_ms_ewma(echo_chain::consensus::paged_spend::BlockProofClass::B255),
            )
        };
        tracing::info!(
            mining = "external",
        ...
        Ok(BlockTemplateResponse {
            ...
            tx_input_counts,
            tx_output_counts,
            coinbase_value_microecho,
            claimable_fees_microecho,
        })
    }
    ...
    async fn finalize_node_owned_template_after_nonce(
        ...
        let proved = tokio::task::spawn_blocking(move || {
            echo_miner::install_history_step_phase_cpu(|| prepared.prove(&runtime, nonce))
                .map_err(|error| format!("HistoryStep CPU admission failed: {error}"))?
        })
        .await
        ...
        let (committed, new_view) = tokio::task::spawn_blocking(move || {
            ...
            let view = echo_mempool::ChainView::from_mdbx(&ctx);
            canonical_tip_changes.send_replace(echo_p2p::object_protocol::ChainPoint::new(
        ...
        let confirmed = echo_chain::try_compute_logical_txids(&block.transactions)
...
#[async_trait]
impl LostechoApiServer for RpcHandler {
    // -----------------------------------------------------------------------
    ...
    async fn get_chain_info(&self) -> RpcResult<ChainInfo> {
        ...
        Ok(ChainInfo {
            ...
            circulating_supply_microecho: chain.state.circulating_supply_microecho.to_string(),
        })
    }
    ...
    async fn get_state_info(&self) -> RpcResult<StateInfo> {
        use echo_chain::consensus::params::{EXPAND_DENOM, EXPAND_NUM, LOG_SLOTS_MAX};
        ...
        let max_encoded_segment_bytes =
            echo_chain::storage::max_encoded_segment_len_for_eff_log(LOG_SEGMENT_SIZE as u8)
    ...
    async fn get_block_details(&self, height: u64) -> RpcResult<Option<BlockDetailsInfo>> {
        ...
        for (lane, output) in coinbase.body.live_outputs() {
            ...
            coinbase_output_details.push(BlockTransactionOutputInfo {
                ...
                amount_microecho: output.amount,
                owner: output.owner.to_bech32(),
                creation_id: echo_chain::consensus::params::coinbase_creation_id(header.height),
            });
        }
        ...
        transactions.push(BlockTransactionInfo {
            ...
            input_owner: None,
            input_sum_microecho: "0".into(),
            output_sum_microecho: reward_microecho.to_string(),
        ...
        let total_fees_microecho = stream
            .groups
            .iter()
        ...
        let proof_class = match stream.proof_class {
            echo_chain::consensus::BlockProofClass::B25 => "B25 / m22",
            echo_chain::consensus::BlockProofClass::B255 => "B255 / m24",
        }
        .to_string();
        let retained = RetainedBlockInfo {
            ...
            reward_microecho,
            reward_echo: crate::types::microecho_to_echo(reward_microecho),
            total_fees_microecho,
    ...
    async fn get_mining_info(&self) -> RpcResult<MiningInfo> {
        ...
        let diff_bits = echo_chain::consensus::target_leading_zero_bits(&diff);
        let reward = block_reward(tip.log_slots);
        Ok(MiningInfo {
            height,
            difficulty_bits: diff_bits,
            difficulty_target: hex::encode(diff),
            block_reward_microecho: reward,
            block_reward_echo: reward as f64 / 1_000_000.0,
            active_slot_count: tip.active_slot_count,
        })
    }

    async fn get_peer_count(&self) -> RpcResult<usize> {
        let count = echo_p2p::P2PNetwork::peer_count_via(&self.p2p_cmd).await;
        Ok(count)
    }
    ...
    async fn estimate_fee(&self, n_outputs: u32) -> RpcResult<u64> {
        ...
        let floor = self.mempool.fee_floor().await;
        Ok(
            echo_chain::consensus::fee_breakdown(1, n_outputs as u64, active_slot_count, log_slots)
                .required_total
                .max(floor),
        )
    }
    ...
    async fn estimate_fee_detailed(&self, n_inputs: u32, n_outputs: u32) -> RpcResult<FeeEstimate> {
        ...
        let (active_slot_count, log_slots) = self.mempool.fee_context().await;
        let floor = self.mempool.fee_floor().await;
        let breakdown = echo_chain::consensus::fee_breakdown(
            n_inputs as u64,
            n_outputs as u64,
            active_slot_count,
            log_slots,
        ...
        Ok(FeeEstimate {
            ...
            active_slot_count,
            log_slots,
            fee_microecho: info.relay_total,
            breakdown: info,
        })
    }
    ...
    async fn submit_tx_intent(&self, hex_str: String) -> RpcResult<String> {
        ...
        let intent = echo_tx::PagedSpendIntent::from_bytes(&bytes)
    ...
    async fn get_mempool_entry(&self, txhash: String) -> RpcResult<Option<MempoolTxInfo>> {
        let hash_bytes = decode_32_byte_hex("txhash", &txhash)?;
        let hash = echo_poseidon2b::primitives::TxBodyHash(hash_bytes);
    ...
    async fn verify_receipt(&self, receipt_hex: String) -> RpcResult<ReceiptVerifyResult> {
        use echo_chain::consensus::receipt::{verify_against_header, verify_merkle_inclusion};

        let bytes = decode_bounded_hex("receipt", &receipt_hex, MAX_RPC_RECEIPT_BYTES)?;

        let receipt = echo_chain::consensus::receipt::LostechoReceipt::from_bytes(&bytes)
        ...
        let authenticated_summary = merkle_valid.then(|| ReceiptSummaryInfo {
            ...
            fee_microecho: receipt.summary.fee_microecho,
            inputs: receipt
                .summary
                .inputs
                .iter()
            ...
            outputs: receipt
                ...
                .outputs
                .iter()
                .map(|(slot_index, amount_microecho, owner)| ReceiptOutputInfo {
                    slot_index: *slot_index,
                    amount_microecho: *amount_microecho,
                    owner: owner.to_bech32(),
                })
                .collect(),
        });
    ...
    async fn wallet_get_balance(&self) -> RpcResult<WalletBalance> {
        ...
        if let Some((_, address)) = self.wallet.active_address() {
            let owner = echo_poseidon2b::primitives::Address::parse(&address)
                .map_err(|error| rpc_err(format!("active wallet address: {error}")))?;
            balance.pending_incoming_microecho =
    ...
    async fn wallet_receipts(&self, page: u32, page_size: u32) -> RpcResult<WalletReceiptsPage> {
        ...
        let receipts = slice
            ...
            .map(|receipt| WalletReceiptInfo {
                ...
                amount_microecho: receipt.amount_microecho,
                fee_microecho: receipt.fee_microecho,
    ...
    async fn wallet_mined_blocks(
        ...
        for record in slice.blocks {
            ...
            blocks.push(WalletMinedBlockInfo {
                ...
                reward_microecho: record.reward_microecho,
                reward_echo: crate::types::microecho_to_echo(record.reward_microecho),
    ...
    async fn wallet_send(
        ...
        let plan = self
            ...
            .plan_send(
                amount_microecho,
                (fee_microecho != 0).then_some(fee_microecho),
        ...
        self.submit_wallet_transaction(WalletSubmissionRequest {
            ...
            amount_microecho,
            fee_microecho: plan.fee_microecho,
            automatic_fee: fee_microecho == 0,
            expected_input_count: plan.input_count,
            expected_output_count: plan.output_count,
            pending_history_amount_microecho: amount_microecho,
    ...
    async fn wallet_consolidate(
        ...
        expected_fee_microecho: u64,
        expected_output_value_microecho: u64,
        ...
        if selected_input_slots != plan.selected_input_slots
            || expected_fee_microecho != plan.fee_microecho
            || expected_output_value_microecho != plan.output_value_microecho
        {
            return Err(rpc_err(
        ...
        tracing::info!(
            input_count = plan.input_count,
            input_value_microecho = plan.input_value_microecho,
            fee_microecho = plan.fee_microecho,
            output_value_microecho = plan.output_value_microecho,
        ...
        let submitted = self
            .submit_wallet_transaction(WalletSubmissionRequest {
                ...
                amount_microecho: plan.output_value_microecho,
                fee_microecho: plan.fee_microecho,
                automatic_fee: false,
                expected_input_count: plan.input_count,
                expected_output_count: 1,
                pending_history_amount_microecho: plan.fee_microecho,
        ...
        Ok(WalletConsolidationResult {
            txid: submitted.txid,
            input_value_microecho: plan.input_value_microecho,
            fee_microecho: submitted.fee_microecho,
            output_value_microecho: submitted.amount_microecho,
...
fn parse_address_param(s: &str) -> RpcResult<echo_poseidon2b::primitives::Address> {
    if s.is_empty() {
...
mod tests {
    ...
    fn wallet_fee_rejection_is_distinct_from_other_retry_errors() {
        use echo_chain::consensus::ConsensusError;
        use echo_mempool::SubmitError;
    ...
    fn mined_block_identity_marks_exact_and_legacy_reorganizations() {
        let header = echo_chain::consensus::genesis_header();
        ...
        assert!(!mined_record_is_canonical(
            None,
            header.timestamp,
            &echo_poseidon2b::primitives::Address([1; 32]).to_bech32(),
            &header,
        ));
    }
    ...
    fn transaction_count_validation_matches_paged_spend() {
        ...
        assert!(validate_tx_counts(echo_tx::MAX_PAGED_SPEND_INPUTS + 1, 1).is_err());
        assert!(validate_tx_counts(1, echo_tx::MAX_PAGED_SPEND_OUTPUTS + 1).is_err());
    }

    #[test]
    fn mempool_status_exposes_the_exact_b25_b255_boundary() {
        let metadata = |page_count| echo_mempool::MempoolEntryMetadata {
            tx_hash: echo_poseidon2b::primitives::TxBodyHash([page_count as u8; 32]),
            fee_microecho: 7,
            fee_rate: 3,
            n_inputs: 1,
            n_outputs: 1,
    ...
    fn block_template_response_serializes_canonical_counts() {
        let response = BlockTemplateResponse {
            ...
            coinbase_value_microecho: 50,
            claimable_fees_microecho: 9,
        };
    ...
    fn required_address_parameters_reject_the_empty_mining_sentinel() {
        ...
        let address = echo_poseidon2b::primitives::Address([0x42; 32]);
...
/// `stop_rx` fires when `lostecho_stop` is called via RPC.
...
pub async fn start_rpc_server(
    ...
    p2p_health: tokio::sync::watch::Receiver<echo_p2p::P2PHealthSnapshot>,
    canonical_tip_changes: tokio::sync::watch::Sender<echo_p2p::object_protocol::ChainPoint>,
    ...
    mining_payout_address: Option<echo_poseidon2b::primitives::Address>,
    ...
    let wallet_submission_mempool =
        mempool
            ...
            .with_authorization_verification_executor(Arc::new(
                |task: echo_mempool::AuthorizationVerificationTask| {
                    echo_miner::install_wallet_verifier_cpu(task).map_err(|error| {
...
fn mining_credential_method_allowed(method: &str) -> bool {
    matches!(method, "lostecho_getBlockTemplate" | "lostecho_submitBlock")
}

fn operator_credential_method_allowed(method: &str) -> bool {
    matches!(
        method,
        "lostecho_walletMinedBlocks"
            | "lostecho_walletGetBalance"
            | "lostecho_walletStatus"
            | "lostecho_walletPlanSend"
            | "lostecho_walletSend"
            | "lostecho_walletPlanConsolidation"
            | "lostecho_walletConsolidate"
            | "lostecho_walletReceipts"
            | "lostecho_walletExportReceipt"
            | "lostecho_getTx"
            | "lostecho_getMempoolEntry"
            | "lostecho_verifyReceipt"
            | "lostecho_submitTxIntent"
            | "lostecho_validateAddress"
            | "lostecho_getChainInfo"
            | "lostecho_estimateFee"
            | "lostecho_estimateFeeDetailed"
    )
}
...
mod access_control_tests {
    ...
    async fn start_access_test_server(
        ...
        module
            .register_method("lostecho_getBlockTemplate", |_, _, _| "template")
            .unwrap();
        module
            .register_method("lostecho_walletStatus", |_, _, _| "status")
            .unwrap();
        module
            .register_method("lostecho_walletSend", |_, _, _| "sent")
            .unwrap();
        module
            .register_method("lostecho_walletHistory", |_, _, _| "history")
            .unwrap();
        module
            .register_method("lostecho_walletUtxoSnapshot", |_, _, _| "utxo-snapshot")
            .unwrap();
    ...
    async fn mining_credential_is_limited_to_external_mining_methods() {
        ...
        for method in ["lostecho_getBlockTemplate", "lostecho_submitBlock"] {
            let response = service
    ...
    async fn operator_credential_is_limited_to_accounting_and_payout_methods() {
        ...
        for method in [
            "lostecho_getBlockTemplate",
            "lostecho_submitBlock",
            "lostecho_stop",
            "lostecho_walletListUtxos",
            "lostecho_walletUtxoSnapshot",
            "lostecho_walletHistory",
            "lostecho_walletScan",
            "lostecho_walletNextAddress",
            "lostecho_walletSetActiveAddress",
            "lostecho_getNodeStatus",
        ] {
            let response = service
                .call(rpc_request(method, Some(RpcAccess::Operator)))
                .await;
    ...
    async fn ordinary_loopback_rpc_is_not_restricted_without_a_credential() {
        ...
        let response = service
            .call(rpc_request("lostecho_walletSend", Some(RpcAccess::Local)))
            .await;
    ...
    async fn missing_access_scope_fails_closed() {
        ...
        let response = service.call(rpc_request("lostecho_walletSend", None)).await;
    ...
    async fn live_server_keeps_mining_and_operator_scopes_disjoint() {
        ...
        let mining_wallet = post_rpc(addr, "lostecho_walletSend", "mining-integration-token").await;
        ...
        let operator_wallet =
            post_rpc(addr, "lostecho_walletSend", "operator-integration-token").await;
        assert!(
        ...
        let operator_status =
            post_rpc(addr, "lostecho_walletStatus", "operator-integration-token").await;
        assert!(
        ...
        let operator_other_wallet =
            post_rpc(addr, "lostecho_walletHistory", "operator-integration-token").await;
        assert!(
            operator_other_wallet.contains(r#""code":-32002"#),
            "{operator_other_wallet}"
        );

        let wrong = post_rpc(addr, "lostecho_walletSend", "wrong-token").await;
        ...
        for (token, denied_code) in [
            ...
            let snapshot = post_rpc(addr, "lostecho_walletUtxoSnapshot", token).await;
            assert!(
        ...
        let batch = post_rpc_body(
            addr,
            r#"[{"jsonrpc":"2.0","id":1,"method":"lostecho_walletStatus","params":[]},{"jsonrpc":"2.0","id":2,"method":"lostecho_walletHistory","params":[]}]"#,
            "operator-integration-token",
        )
        .await;
    ...
    async fn live_server_grants_local_scope_from_accepted_loopback_peer() {
        ...
        let wallet = post_rpc_without_authorization(addr, "lostecho_walletSend").await;
        assert!(wallet.starts_with("HTTP/1.1 200"), "{wallet}");
        assert!(wallet.contains(r#""result":"sent""#), "{wallet}");

        let history = post_rpc_without_authorization(addr, "lostecho_walletHistory").await;
        assert!(history.contains(r#""result":"history""#), "{history}");

        let snapshot = post_rpc_without_authorization(addr, "lostecho_walletUtxoSnapshot").await;
        assert!(
