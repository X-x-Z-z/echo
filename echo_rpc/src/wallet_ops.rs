...
//!
//! Implemented by `WalletHandle` in `echo_node/src/wallet/mod.rs`.
...
pub enum WalletSendPlanError {
    #[error("InsufficientFunds: need {needed_microecho} μECHO, have {available_microecho} μECHO spendable")]
    InsufficientFunds {
        needed_microecho: u64,
        available_microecho: u64,
    },
...
pub trait WalletOps: Send + Sync {
    ...
    ///
    /// `explicit_fee_microecho = Some(fee)` applies that exact fee and rejects
    ...
    /// - `amount_microecho`: payment amount in μECHO
    /// - `fee_microecho`: transaction fee in μECHO
