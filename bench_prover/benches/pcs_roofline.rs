...
//!
//! ```text
//! ECHO_PCS_ROOFLINE_DOMAINS=22 cargo bench -p bench_prover --bench pcs_roofline
//! ECHO_PCS_ROOFLINE_DOMAINS=23 cargo bench -p bench_prover --bench pcs_roofline
//! ECHO_PCS_ROOFLINE_DOMAINS=24 cargo bench -p bench_prover --bench pcs_roofline
//! ```
//!
...
//! enabled automatically via `ECHOH_COMMIT_TIMING=1`.
...
use echo_core::mem_profile::{current_mem_snapshot, MemSnapshot};
use echo_ivc_prover::challenger::FsLaneChallenger;
use echo_ivc_prover::field::F128;
use echo_ivc_prover::pcs::{self, PcsParams, QuirkyDirectClaim, QuirkyDirectClaimRef};
use echo_ivc_prover::proof::bind_statement_field_parts;
use echo_ivc_prover::zerocheck::multilinear::lagrange_weights_naive;
use rayon::prelude::*;
...
fn requested_domains() -> Vec<usize> {
    let raw =
        env::var("ECHO_PCS_ROOFLINE_DOMAINS").unwrap_or_else(|_| DEFAULT_LOGICAL_M.to_string());
...
fn print_manifest(domains: &[usize]) {
    ...
    println!("  commit split    : production ECHOH_COMMIT_TIMING NTT/Merkle timers");
    println!();
}
...
fn run_case(logical_m: usize) {
    ...
    drop(claim);
    drop(witness);
    echo_ivc_prover::scratch::clear();
}

fn main() {
    // Read by pcs::commit; set before rayon or any other worker thread starts.
    if env::var_os("ECHOH_COMMIT_TIMING").is_none() {
        env::set_var("ECHOH_COMMIT_TIMING", "1");
    }
    echo_ivc_prover::init_perf_thread_pool();
