...
//!
//! Defaults to B25. Use `ECHO_ACTION_TIERS=25,255`; B255 intentionally
//! builds the real 4,096-row sorting network and may be expensive.

use std::time::Instant;

use bench_prover::legal_block_scenarios;
use echo_core::mem_profile::current_mem_snapshot;
use echo_core::Block128;
use echo_recursive::acceptance::shape::ShapeClass;
use echo_recursive::acceptance::trace::action_compaction::{
    bind_mint_packed_values_body_order, compact_action_rows,
};
use echo_recursive::acceptance::trace::action_surface::ActionRowTrace;
use echo_recursive::acceptance::trace::{alloc_block, FieldR1csBuilder, LinExpr};

fn requested_tiers() -> Vec<usize> {
    let raw = std::env::var("ECHO_ACTION_TIERS").unwrap_or_else(|_| "25".into());
    let tiers: Vec<_> = raw
        .split(',')
    ...
    assert!(tiers
        .iter()
        .all(|tier| echo_chain::consensus::params::BLOCK_PAGE_CLASS_TIERS.contains(tier)));
    tiers
}
...
fn main() {
    println!("LOSTECHO bitmap action-compactor gate");
    println!("ECHO_ACTION_TIERS=25,255 selects class runs.\n");
    ...
    for tier in requested_tiers() {
        ...
        for scenario in &scenarios {
            for input in 0..echo_tx::TX_INPUTS {
                pattern.push(scenario.body.input_is_live(input));
                kinds.push(false);
            }
            for output in 0..echo_tx::TX_OUTPUTS {
        ...
        let verify = std::env::var_os("ECHO_ACTION_VERIFY").is_some() || tier != 255;
        ...
        if verify {
            ...
            println!("    satisfy:          skipped (set ECHO_ACTION_VERIFY=1)");
        }
