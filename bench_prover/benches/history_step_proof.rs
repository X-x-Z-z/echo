...
//! export ECHO_HISTORY_STEP_RUNTIME_METADATA_RELEASE_DIGEST
//! export ECHO_HISTORY_STEP_PACK_LEAF_DIGESTS
//! export ECHO_HISTORY_STEP_PACK_DIR="$PACK_ROOT"
//! cargo bench -p bench_prover --bench history_step_proof
//! ```
//!
//! Set `ECHO_HISTORY_STEP_BENCH_FILTER=B255` (or `c01`) to run B255 after a
//! B25 parent. Use `B255-B255` for the B255-parent case. With no filter, both
//! launch classes run. `ECHO_HISTORY_STEP_BENCH_SAMPLES=N` reuses the proved
...
//! `ECHO_HISTORY_STEP_BENCH_WIRE_AUDIT=1` additionally checks exact round trips
...
//! `ECHO_HISTORY_STEP_BENCH_ALL_PARENTS=1` runs both child classes after each
...
use echo_ivc_core::field_r1cs::CompactFieldR1cs;
use echo_miner::history_step_artifacts::{
...
use echo_poseidon2b::native::poseidon2b_hash_byte_slices;
use echo_recursive::{
...
const PACK_DIRECTORY_ENV: &str = "ECHO_HISTORY_STEP_PACK_DIR";
const BENCH_FILTER_ENV: &str = "ECHO_HISTORY_STEP_BENCH_FILTER";
const BENCH_SAMPLES_ENV: &str = "ECHO_HISTORY_STEP_BENCH_SAMPLES";
const METADATA_DIGEST_ENV: &str = "ECHO_HISTORY_STEP_RUNTIME_METADATA_RELEASE_DIGEST";
const LEAF_DIGESTS_ENV: &str = "ECHO_HISTORY_STEP_PACK_LEAF_DIGESTS";
...
) -> Result<(echo_chain::Block, HistoryStepBlockInput<TIER>), String> {
...
fn finish_fixture_template<const TIER: usize>(
    fixture: PreparedHistoryStepTierFixture<TIER>,
) -> Result<(echo_chain::Block, HistoryStepBlockInput<TIER>, u128), String> {
    let (witness, nonce, start, end) = fixture.into_parts();
...
fn prove_parent_step<const TIER: usize>(
    ...
    fixture: PreparedHistoryStepTierFixture<TIER>,
) -> Result<(echo_chain::Block, HistoryStepTerminal), String> {
...
fn build_parent(
    ...
    let mut expected = echo_recursive::genesis_accumulator();
    let mut parent = None;
    loop {
...
fn benchmark_tier<const TIER: usize>(
    ...
    let epoch_anchor = echo_chain::consensus::genesis_header();
    ...
    if std::env::var_os("ECHO_HISTORY_STEP_BENCH_WIRE_AUDIT").is_some() {
        let audit =
            echo_recursive::acceptance::history_step::audit_history_step_terminal_encodings(
                runtime,
                &terminal,
                &block.header,
                &epoch_anchor,
...
fn run_on_production_pool() -> Result<(), String> {
    ...
    let all_parents = std::env::var_os("ECHO_HISTORY_STEP_BENCH_ALL_PARENTS").is_some();
    ...
    for target_parent_slot in parent_slots {
        ...
        let accepted_parent = decode_verify_history_step_terminal(
            ...
            &parent_wire,
            &parent_block.header,
            &echo_chain::consensus::genesis_header(),
        )
...
fn run() -> Result<(), String> {
    let cpu_plan =
        echo_miner::configure_process_cpu_budget(echo_miner::ProcessCpuBudgetMode::ProofOnly)
            .map_err(|error| format!("configure production CPU pool: {error}"))?;
    let hardware = echo_core::cpu::ProductionHardwareReport::detect();
    println!(
        "HistoryStep benchmark backend={} threads={}",
        hardware.backend, cpu_plan.history_step_phase_threads,
    );
    echo_miner::install_history_step_phase_cpu(run_on_production_pool)
