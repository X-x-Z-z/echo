...
//!
//! Usage: `echo_matrix_gen <pack-root>`
//!
...
fn parse_zstd_level() -> i32 {
    match std::env::var("ECHO_ARTIFACT_ZSTD_LEVEL") {
        Ok(value) => value
            .parse::<i32>()
            .unwrap_or_else(|error| panic!("ECHO_ARTIFACT_ZSTD_LEVEL must be an integer: {error}")),
        Err(std::env::VarError::NotPresent) => DEFAULT_ZSTD_LEVEL,
        Err(error) => panic!("read ECHO_ARTIFACT_ZSTD_LEVEL: {error}"),
    }
}
...
fn main() {
    ...
    let root = std::env::args()
        .nth(1)
        .expect("usage: echo_matrix_gen <pack-root>");
    assert!(
    ...
    let debug = std::env::var_os("ECHO_HISTORY_STEP_GENERATOR_DEBUG").is_some();
    println!("LOSTECHO canonical HistoryStep v1 freezer");
