...
use echo_core::Block128;
use echo_gkr::zk_authorization::ZkAuthorizationProof;
use echo_gkr::{
...
use echo_poseidon2b::primitives::{derive_address, SpendSecret, TxBodyHash};
use echo_tx::{
...
pub fn poseidon_chain_field_instance(
    ...
    echo_ivc_prover::field_r1cs::FieldR1cs,
    Vec<echo_ivc_prover::field::F128>,
) {
    use echo_ivc_prover::field_circuit::{flat_const, poseidon2b_permute, LinExpr};
    use echo_poseidon2b::native::permutation::{Poseidon2bPermutation, STATE_SIZE};
    use echo_recursive::acceptance::trace::FieldR1csBuilder;
...
pub fn legal_block_scenarios(
    ...
    assert!(echo_chain::consensus::params::BLOCK_PAGE_CLASS_TIERS.contains(&user_txs));
    if user_txs == echo_chain::consensus::params::BLOCK_MAX_USER_PAGES {
...
fn maximally_dispersed_b255_touched_slots() -> Vec<u32> {
    let mut slots = Vec::with_capacity(echo_chain::consensus::params::BLOCK_MAX_ACTIONS);
    for segment_rank in 0..echo_chain::consensus::params::BLOCK_MAX_DISTINCT_SEGMENTS {
        ...
        for local_rank in 0..local_count {
            ...
            slots.push((segment << echo_chain::consensus::params::LOG_SEGMENT_SIZE) | local);
        }
    }
    assert_eq!(
        slots.len(),
        echo_chain::consensus::params::BLOCK_MAX_ACTIONS
    );
    slots
}
...
fn tx8x2_scenario_with_layout(
    ...
    let fee = echo_chain::consensus::fees::fee_breakdown(
        input_slots.len() as u64,
        TX_OUTPUTS as u64,
        echo_chain::consensus::params::BLOCK_MAX_LIVE_INPUTS as u64,
        BENCH_LOG_SLOTS,
    )
    .required_total;
...
struct HistoryStepFixtureCheckpoint {
    parent_header: echo_chain::BlockHeader,
    tx_epoch_anchor_header: echo_chain::BlockHeader,
    parent_state: echo_chain::state::ChainState,
    start_accumulator: echo_recursive::ChainAccumulator,
    previous_timestamps: Vec<u64>,
    finalized_active_counts: Vec<u64>,
    asert_anchor: echo_chain::consensus::AnchorInfo,
...
pub struct PreparedHistoryStepTierFixture<const TIER: usize> {
    witness: echo_block::PreparedHistoryStepInputWitness<TIER>,
    nonce: u128,
    start_accumulator: echo_recursive::ChainAccumulator,
    end_accumulator: echo_recursive::ChainAccumulator,
...
impl<const TIER: usize> PreparedHistoryStepTierFixture<TIER> {
    ...
    pub fn start_accumulator(&self) -> &echo_recursive::ChainAccumulator {
        &self.start_accumulator
    }

    pub fn end_accumulator(&self) -> &echo_recursive::ChainAccumulator {
    ...
    pub fn into_parts(
        ...
        echo_block::PreparedHistoryStepInputWitness<TIER>,
        u128,
        echo_recursive::ChainAccumulator,
        echo_recursive::ChainAccumulator,
    ) {
        (
            self.witness,
    ...
    fn into_history_step_input(
        self,
    ) -> Result<echo_recursive::HistoryStepBlockInput<TIER>, String> {
...
pub struct HonestHistoryStepFixtureProvider {
    seed: u128,
    ghost: echo_recursive::PreparedHistoryStepGhostAuthorization,
    ...
    checkpoints:
        [Option<HistoryStepFixtureCheckpoint>; echo_recursive::HISTORY_STEP_TIER_SLOT_COUNT],
}

impl HonestHistoryStepFixtureProvider {
    pub fn new(seed: u128) -> Result<Self, String> {
        let ghost = echo_gkr::ghost_tx::prove_selected_ghost_authorization()
            .map_err(|error| format!("prove canonical ghost authorization: {error}"))?;
        let ghost = echo_recursive::prepare_history_step_ghost_authorization(ghost)
    ...
    pub fn next_backbone(
        &mut self,
        expected_start: &echo_recursive::ChainAccumulator,
        ...
        let input = match echo_chain::consensus::params::block_page_class_tier(user_count) {
            Some(25) => self
    ...
    pub fn b25(
        &self,
        class_id: echo_recursive::CanonicalHistoryStepClassId,
        expected_start: &echo_recursive::ChainAccumulator,
    ...
    pub fn parent_accumulator(
        &self,
        parent_slot: usize,
    ) -> Option<&echo_recursive::ChainAccumulator> {
        self.checkpoints
            .get(parent_slot)?
            .as_ref()
    ...
    fn build_child_from<const TIER: usize>(
        ...
        if echo_chain::consensus::params::block_page_class_tier(user_count) != Some(TIER) {
        ...
        let timestamp = checkpoint
            .parent_header
            .timestamp
            .checked_add(echo_chain::consensus::params::BLOCK_TIME)
            .ok_or_else(|| "fixture timestamp overflow".to_owned())?;
        let target = echo_chain::consensus::next_target(
        ...
        let nonce_key = echo_chain::hash_block_header(&block.header);
        ...
        let context = echo_block::HistoryStepPreparationContext {
        ...
        let authorization_proofs = authorization_bytes
            ...
            .collect::<Result<Vec<_>, _>>()?;
        let witness = echo_block::prepare_history_step_input_witness::<TIER>(
            block,
            context,
            authorization_proofs,
            &self.ghost,
...
impl echo_recursive::HistoryStepFreezeInputProvider for HonestHistoryStepFixtureProvider {
    type Error = String;
    ...
    fn next_backbone(
        &mut self,
        expected_start: &echo_recursive::ChainAccumulator,
    ) -> Result<Option<echo_recursive::HistoryStepFreezeInput>, Self::Error> {
        HonestHistoryStepFixtureProvider::next_backbone(self, expected_start)?
            .map(|step| match step.input {
                PreparedHistoryStepBackboneInput::B25(input) => input
                    ...
                    .map(echo_recursive::HistoryStepFreezeInput::B25),
                PreparedHistoryStepBackboneInput::B255(input) => input
                    .into_history_step_input()
                    .map(echo_recursive::HistoryStepFreezeInput::B255),
            })
            .transpose()
    }
    ...
    fn b25(
        &mut self,
        class: echo_recursive::CanonicalHistoryStepClassId,
        expected_start: &echo_recursive::ChainAccumulator,
    ) -> Result<echo_recursive::HistoryStepBlockInput<25>, Self::Error> {
    ...
    fn b255(
        ...
        class: echo_recursive::CanonicalHistoryStepClassId,
        expected_start: &echo_recursive::ChainAccumulator,
    ) -> Result<echo_recursive::HistoryStepBlockInput<255>, Self::Error> {
    ...
    ($tier:literal, $variant:ident) => {
        impl AdvanceHonestBackbone for BuiltFixtureChild<$tier> {
            fn map_into(
                ...
                echo_chain::consensus::validate_block_checks(
                    &sealed_block,
                    &live.parent_header,
                ...
                echo_chain::materialize_accepted_block_state(&mut live.parent_state, &sealed_block)
                    .map_err(|error| {
...
fn genesis_fixture_checkpoint() -> HistoryStepFixtureCheckpoint {
    ...
    let state = echo_chain::state::ChainState::with_log_slots(genesis.log_slots as usize);
    ...
    HistoryStepFixtureCheckpoint {
        ...
        parent_state: state,
        start_accumulator: echo_recursive::genesis_accumulator(),
...
> {
    ...
    for (tx_index, source) in checkpoint.spendables.iter().take(user_count).enumerate() {
        ...
        body.fee = echo_chain::consensus::fees::required_fee_for_tx_body(
            &body,
...
fn mine_history_step_fixture_header(header: &echo_chain::BlockHeader) -> u128 {
    use rayon::prelude::*;
    ...
    loop {
        if let Some(nonce) = (0..lanes)
            .into_par_iter()
            .filter_map(|lane| {
                echo_chain::consensus::pow::search_pow(
                    header,
...
mod two_class_history_step_fixture_tests {
    ...
    fn freezer_page_counts_select_only_b25_and_b255() {
        let backbone = HISTORY_STEP_FREEZER_BACKBONE_USER_COUNTS
            .map(|count| echo_chain::consensus::params::block_page_class_tier(count).unwrap());
    ...
    fn first_backbone_step_is_a_coinbase_only_b25_block() {
        ...
        let genesis = echo_recursive::genesis_accumulator();
