...
BASE = Path(
    os.environ.get(
        "ECHO_LIVE_WALLET_ACTIVE_ADDRESS_DIR",
        str(RUN_PARENT / f"wallet-active-address-clean-{STAMP}"),
    )
)
BASE_PORT = int(os.environ.get("ECHO_LIVE_WALLET_ACTIVE_ADDRESS_BASE_PORT", "22400"))
FUNDING_HEIGHT = 4
PAYMENT_MICROECHO = 450_000
...
def main():
    ...
    try:
        ...
        require(balance(node)["balance_microecho"] == 0, "active balance changed at h0")
        ...
        require(
            int(index0_before["spendable_microecho"]) > PAYMENT_MICROECHO,
        ...
        sent = rpc(
            node.rpc_port,
            "walletSend",
            [address1["address"], PAYMENT_MICROECHO, 0],
            timeout=300,
        )
        txid = sent["txid"]
        ...
        require(
            int(index1_balance["balance_microecho"]) == PAYMENT_MICROECHO
        ...
        require(
            ...
            and int(index1_utxos[0]["value_microecho"]) == PAYMENT_MICROECHO,
