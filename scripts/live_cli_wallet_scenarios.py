...
def main():
    if not NODE_BIN.exists() or not CLI_BIN.exists():
        raise LiveTestError(
            "release binaries missing; run cargo build --release -p echo_node --bin lostecho --bin lostecho-cli"
        )
    if BASE.exists():
        shutil.rmtree(BASE)
    LOGS.mkdir(parents=True, exist_ok=True)
    ...
    try:
        ...
        assert_contains(out, "Lostecho node status", "status output")
        ...
        assert_true(
            bal1["spendable_microecho"] >= 100_000_000,
        ...
        scan3 = cli_json(n3, ["scan"], timeout=180)
        assert_true(
            scan3["balance_microecho"] == 750_000
            and scan3["found_utxos"] == 1,
            f"second recipient scan did not find the exact funds: {scan3}",
        )
        bal3 = cli_json(n3, ["balance"])
        assert_true(
            bal3["balance_microecho"] == 750_000 and bal3["utxo_count"] == 1,
