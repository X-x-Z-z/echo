...
BASE = Path(
    os.environ.get(
        "ECHO_LIVE_MULTI_TX_DIR", str(RUN_PARENT / f"multi-tx-mempool-clean-{STAMP}")
    )
)
BASE_PORT = int(os.environ.get("ECHO_LIVE_MULTI_TX_BASE_PORT", "20500"))
...
def main():
    ...
    summary = {
        ...
        "amounts_microecho": list(AMOUNTS),
        "status": "running",
    }
    error = None
    try:
        ...
        require(rpc(observer.rpc_port, "walletGetBalance")["balance_microecho"] == 0, "recipient pre-funded")
        ...
        require(
            recipient_balance["balance_microecho"] == sum(AMOUNTS),
