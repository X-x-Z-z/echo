...
NODE_BIN = ROOT / "target" / "release" / "lostecho"
RUN_PARENT = ROOT / "target" / "live-tests"
STAMP = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
SCENARIO = os.environ.get("ECHO_LIVE_TX_SCENARIO", "single-input").strip()
BASE = Path(
    os.environ.get(
        "ECHO_LIVE_SINGLE_TX_DIR",
        str(RUN_PARENT / f"transaction-{SCENARIO}-clean-{STAMP}"),
    )
)
P2P_PORT = int(os.environ.get("ECHO_LIVE_SINGLE_TX_P2P_PORT", "20400"))
RPC_PORT = P2P_PORT + 1
MINE_TO_HEIGHT = int(os.environ.get("ECHO_LIVE_TX_MINE_TO_HEIGHT", "3"))
PAYMENT_MICROECHO = int(os.environ.get("ECHO_LIVE_TX_PAYMENT_MICROECHO", "1000000"))
EXPECTED_INPUTS = int(os.environ.get("ECHO_LIVE_TX_EXPECTED_INPUTS", "1"))
EXPECTED_OUTPUTS = int(os.environ.get("ECHO_LIVE_TX_EXPECTED_OUTPUTS", "2"))
EXPECTED_PAGES = int(
    os.environ.get("ECHO_LIVE_TX_EXPECTED_PAGES", str((EXPECTED_INPUTS + 7) // 8))
)
EXPECTED_PROOF_CLASS = os.environ.get("ECHO_LIVE_TX_EXPECTED_PROOF_CLASS", "B25")
EXPECT_CHANGE = os.environ.get("ECHO_LIVE_TX_EXPECT_CHANGE", "1") != "0"
    ...
    payload = json.dumps(
        {
            "jsonrpc": "2.0",
            "id": 1,
            "method": method if method.startswith("lostecho_") else f"lostecho_{method}",
            "params": params or [],
        }
    ).encode()
...
class Node:
    def __init__(self):
        ...
        self.config = self.root / "lostecho.toml"
        self.proc = None
...
def main():
    ...
    summary = {
        ...
        "configuration": {
            ...
            "payment_microecho": PAYMENT_MICROECHO,
    ...
    try:
        ...
        require(before["spendable_microecho"] > PAYMENT_MICROECHO, f"sender not funded: {before}")
        require(
            before["utxo_count"] >= EXPECTED_INPUTS,
            f"sender has too few UTXOs for requested shape: {before}",
        )

        plan = rpc("walletPlanSend", [recipient["address"], PAYMENT_MICROECHO, 0], timeout=60)
        require(
        ...
        require(
            (plan["change_microecho"] > 0) == EXPECT_CHANGE,
            f"payment change shape is wrong: {plan}",
        )

        submit_height = int(rpc("getChainInfo")["height"])
        proof_started = time.monotonic()
        sent = rpc("walletSend", [recipient["address"], PAYMENT_MICROECHO, 0], timeout=300)
        ...
        require(sent["fee_microecho"] == plan["fee_microecho"], f"send fee changed: {sent} vs {plan}")
        ...
        require(recipient_slots[0]["value"] == PAYMENT_MICROECHO, f"recipient amount wrong: {recipient_slots}")
        ...
        require(recipient_balance["balance_microecho"] == PAYMENT_MICROECHO, f"recipient balance wrong: {recipient_balance}")
        ...
        require(sender_after["pending_outbound_microecho"] == 0, f"sender reservation survived confirmation: {sender_after}")
