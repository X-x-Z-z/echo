...
BASE = Path(
    os.environ.get(
        "ECHO_LIVE_LARGE_MEMPOOL_SINGLE_DIR",
        str(RUN_PARENT / f"large-mempool-single-miner-clean-{STAMP}"),
    )
)
BASE_PORT = int(os.environ.get("ECHO_LIVE_LARGE_MEMPOOL_SINGLE_BASE_PORT", "21800"))
INITIAL_HEIGHT = 4
        ...
        summary.update(
            {
                ...
                "fee_counts": dict(Counter(send["fee_microecho"] for send in spam_sends)),
            }
        )
