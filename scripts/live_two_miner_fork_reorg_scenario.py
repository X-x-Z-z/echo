...
NODE_BIN = Path(
    os.environ.get("ECHO_LIVE_NODE_BIN", str(ROOT / "target" / "release" / "lostecho"))
).resolve()
RUN_PARENT = ROOT / "target" / "live-tests"
STAMP = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
BASE = Path(
    os.environ.get(
        "ECHO_LIVE_FORK_REORG_DIR",
        str(RUN_PARENT / f"two-miner-fork-reorg-clean-{STAMP}"),
    )
)
BASE_PORT = int(os.environ.get("ECHO_LIVE_FORK_REORG_BASE_PORT", "20600"))
COMMON_HEIGHT = 2
