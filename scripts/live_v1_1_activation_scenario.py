...
Build with `--features echo_chain/isolated-v1-1-testnet` into a separate target
...
class Node(live.Node):
    ...
    def start(self, label, **kwargs):
        ...
        original_force = os.environ.get("ECHO_ISOLATED_FORCE_B255")
        live.NODE_BIN = self.binary
        if self.producer and kwargs.get("mode") == "miner":
            os.environ["ECHO_ISOLATED_FORCE_B255"] = "1"
        else:
            os.environ.pop("ECHO_ISOLATED_FORCE_B255", None)
        try:
            ...
            if original_force is None:
                ...
                os.environ["ECHO_ISOLATED_FORCE_B255"] = original_force
...
def boundary():
    ...
    evidence = {
        ...
        "recipient": recipient, "amount": 10_000, "fee": int(sent["fee_microecho"]),
    }
...
def finish_workloads(a, b, fresh, evidence):
    ...
    checked = subprocess.run(
        [str(TEST_BIN.with_name("lostecho-cli")), "--rpc", f"http://127.0.0.1:{fresh.rpc_port}",
...
def capacity():
    ...
    for count in (1, 2, 4, 8, 16, 32, 64, 128, 256):
        ...
        minimum = min(int(u["value_microecho"]) for u in utxos)
        ...
        if count == 256:
            ...
            previous_gate = os.environ.get("ECHO_ISOLATED_MINER_START_GATE")
            os.environ["ECHO_ISOLATED_MINER_START_GATE"] = str(start_gate)
            try:
                a.start("a-waits-for-full-capacity", mode="miner", genesis=True,
                        seeds=[b.seed, wallet.seed])
            finally:
                if previous_gate is None:
                    os.environ.pop("ECHO_ISOLATED_MINER_START_GATE", None)
                else:
                    os.environ["ECHO_ISOLATED_MINER_START_GATE"] = previous_gate
        txids = []
        for index in range(count):
            ...
            require(int(sent["fee_microecho"]) == 9_000, "split fee changed")
