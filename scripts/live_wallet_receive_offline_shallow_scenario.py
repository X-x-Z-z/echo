...
BASE = Path(
    os.environ.get(
        "ECHO_LIVE_WALLET_RECEIVE_OFFLINE_SHALLOW_DIR",
        str(RUN_PARENT / f"wallet-receive-offline-shallow-clean-{STAMP}"),
    )
)
BASE_PORT = int(
    os.environ.get("ECHO_LIVE_WALLET_RECEIVE_OFFLINE_SHALLOW_BASE_PORT", "22200")
)
FUNDING_HEIGHT = 4
TRAILING_BLOCKS = 3
PAYMENT_MICROECHO = 250_000
RETAINED_DEPTH = 18
...
def main():
    ...
    try:
        ...
        require(
            ...
            "shallow offline recipient unexpectedly used ↻ snapshot",
        )
        require(
