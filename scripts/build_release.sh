...
Environment:
  ECHO_MACOS_SIGN_IDENTITY        Optional Developer ID identity; defaults to
...
case "$HOST_TRIPLE" in
  x86_64-unknown-linux-gnu)
    ...
    BINARY_SUFFIX=
    ARCHIVE_KIND=tar
    GUI_ARTIFACT_NAME="lostecho-gui-v$RELEASE_VERSION-linux-x86_64.deb"
    ;;
  aarch64-unknown-linux-gnu)
    ...
    BINARY_SUFFIX=
    ARCHIVE_KIND=tar
    GUI_ARTIFACT_NAME="lostecho-gui-v$RELEASE_VERSION-linux-aarch64.deb"
    ;;
  x86_64-pc-windows-msvc)
    ...
    BINARY_SUFFIX=.exe
    ARCHIVE_KIND=zip
    GUI_ARTIFACT_NAME="lostecho-gui-v$RELEASE_VERSION-windows-x86_64-setup.exe"
    ;;
  aarch64-apple-darwin)
    ...
    BINARY_SUFFIX=
    ARCHIVE_KIND=tar
    GUI_ARTIFACT_NAME="lostecho-gui-v$RELEASE_VERSION-macos-aarch64.dmg"
    ;;
  x86_64-apple-darwin)
    ...
    BINARY_SUFFIX=
    ARCHIVE_KIND=tar
    GUI_ARTIFACT_NAME="lostecho-gui-v$RELEASE_VERSION-macos-x86_64.dmg"
    ;;
...
if [[ $ARCHIVE_KIND == zip ]]; then
  ARCHIVE_NAME="lostecho-core-v$RELEASE_VERSION-$PLATFORM.zip"
  release_require_command 7z
else
  ARCHIVE_NAME="lostecho-core-v$RELEASE_VERSION-$PLATFORM.tar.gz"
fi
...
unset ECHO_HISTORY_STEP_PACK_DIR
unset ECHO_HISTORY_STEP_RUNTIME_METADATA_RELEASE_DIGEST
unset TAR_OPTIONS GZIP GZIP_OPT
export CARGO_TARGET_DIR="$RELEASE_ROOT_DIR/target"

printf 'Lostecho self-contained release build\n'
printf '  source:       %s\n' "$RELEASE_ROOT_DIR"
...
export ECHO_HISTORY_STEP_PACK_DIR="$PACK_DIR"
export ECHO_HISTORY_STEP_RUNTIME_METADATA_RELEASE_DIGEST="$RELEASE_METADATA_DIGEST"

CURRENT_STAGE='self-contained binary build'
printf '\n==> Building matrix-embedded native binaries\n'
cargo build --locked --release --target "$HOST_TRIPLE" -p echo_node --bins
cargo build --locked --release --target "$HOST_TRIPLE" \
  -p echo-extminer --bin lostecho-miner
cargo build --locked --release --target "$HOST_TRIPLE" \
  -p echo_gui --bin lostecho-gui

TARGET_BIN_DIR="$CARGO_TARGET_DIR/$HOST_TRIPLE/release"
for binary in lostecho lostecho-cli lostecho-miner; do
  [[ -f $TARGET_BIN_DIR/$binary$BINARY_SUFFIX ]] || \
    release_die "release binary is missing: $TARGET_BIN_DIR/$binary$BINARY_SUFFIX"
done
[[ -f $TARGET_BIN_DIR/lostecho-gui$BINARY_SUFFIX ]] || \
  release_die "release GUI is missing: $TARGET_BIN_DIR/lostecho-gui$BINARY_SUFFIX"

CURRENT_STAGE='native smoke test'
printf '\n==> Smoke-testing native executables\n'
"$TARGET_BIN_DIR/lostecho$BINARY_SUFFIX" --check-hardware >/dev/null
"$TARGET_BIN_DIR/lostecho$BINARY_SUFFIX" --help >/dev/null
"$TARGET_BIN_DIR/lostecho-cli$BINARY_SUFFIX" --help >/dev/null
"$TARGET_BIN_DIR/lostecho-miner$BINARY_SUFFIX" --check-hardware >/dev/null
"$TARGET_BIN_DIR/lostecho-miner$BINARY_SUFFIX" --help >/dev/null
"$TARGET_BIN_DIR/lostecho-gui$BINARY_SUFFIX" --release-self-check >/dev/null
...
printf '\n==> Packaging %s\n' "$GUI_ARTIFACT_NAME"
mkdir -- "$GUI_BIN_DIR"
cp -- "$TARGET_BIN_DIR/lostecho-gui$BINARY_SUFFIX" \
  "$GUI_BIN_DIR/lostecho-gui$BINARY_SUFFIX"
cp -- "$TARGET_BIN_DIR/lostecho$BINARY_SUFFIX" \
  "$GUI_BIN_DIR/lostecho$BINARY_SUFFIX"
chmod 0755 "$GUI_BIN_DIR/lostecho-gui$BINARY_SUFFIX" 2>/dev/null || true
chmod 0755 "$GUI_BIN_DIR/lostecho$BINARY_SUFFIX" 2>/dev/null || true
case "$PLATFORM" in
  linux-*)
...
archive_entries=(
  ...
  LICENSE
  NOTICE
  "lostecho$BINARY_SUFFIX"
  "lostecho-cli$BINARY_SUFFIX"
  "lostecho-miner$BINARY_SUFFIX"
)
