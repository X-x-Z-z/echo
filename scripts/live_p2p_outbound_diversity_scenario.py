...
BASE = Path(
    os.environ.get(
        "ECHO_LIVE_OUTBOUND_DIVERSITY_DIR",
        str(RUN_PARENT / f"p2p-outbound-diversity-clean-{STAMP}"),
    )
)
BASE_PORT = int(os.environ.get("ECHO_LIVE_OUTBOUND_DIVERSITY_BASE_PORT", "21600"))
...
def enter_network_namespace():
    if os.environ.get("ECHO_LIVE_ECLIPSE_NETNS") == "1":
        return
    env = os.environ.copy()
    env["ECHO_LIVE_ECLIPSE_NETNS"] = "1"
    os.execvpe(
        "unshare",
