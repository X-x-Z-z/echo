...
BASE = Path(
    os.environ.get(
        "ECHO_LIVE_STATE_SLOT_DIR",
        str(RUN_PARENT / f"state-slot-lifecycle-clean-{STAMP}"),
    )
)
BASE_PORT = int(os.environ.get("ECHO_LIVE_STATE_SLOT_BASE_PORT", "22700"))
INITIAL_HEIGHT = int(os.environ.get("ECHO_LIVE_STATE_SLOT_INITIAL_HEIGHT", "3"))
PAYMENT_MICROECHO = int(os.environ.get("ECHO_LIVE_STATE_SLOT_PAYMENT", "1000000"))
SEGMENT_LOG = 16
...
def main():
    ...
    try:
        ...
        plan = rpc(
            node.rpc_port,
            "walletPlanSend",
            [recipient["address"], PAYMENT_MICROECHO, 0],
        )
        ...
        require(
            int(recipient_slots[0]["value"]) == PAYMENT_MICROECHO,
