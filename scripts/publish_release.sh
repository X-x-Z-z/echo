...
Examples:
  ./scripts/publish_release.sh 0.0.4 \
    /home/neo/rust/lostecho-artifacts/history-step-pack-v1 \
    --channel prerelease
...
[[ $(git rev-parse --show-toplevel) == "$RELEASE_ROOT_DIR" ]] || \
  release_die "run this command from the Lostecho repository"
...
TEMP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/lostecho-release.XXXXXX")
cleanup() {
  local status=$?
  if [[ -d $TEMP_DIR && $TEMP_DIR == "${TMPDIR:-/tmp}"/lostecho-release.* ]]; then
    rm -r -- "$TEMP_DIR" || true
  fi
  exit "$status"
}
...
EXPECTED_ASSETS=(
  "lostecho-core-$TAG-linux-x86_64.tar.gz"
  "lostecho-core-$TAG-linux-aarch64.tar.gz"
  "lostecho-core-$TAG-windows-x86_64.zip"
  "lostecho-core-$TAG-macos-aarch64.tar.gz"
  "lostecho-core-$TAG-macos-x86_64.tar.gz"
  "lostecho-gui-$TAG-linux-x86_64.deb"
  "lostecho-gui-$TAG-linux-aarch64.deb"
  "lostecho-gui-$TAG-windows-x86_64-setup.exe"
  "lostecho-gui-$TAG-macos-aarch64.dmg"
  "lostecho-gui-$TAG-macos-x86_64.dmg"
