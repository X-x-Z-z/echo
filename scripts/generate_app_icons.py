...
"""Generate deterministic Lostecho application icons without external tools."""
...
OUTPUT = ROOT / "echo_gui" / "assets" / "app-icons"
...
def main() -> int:
    ...
    for size in PNG_SIZES:
        ...
        encoded[size] = png
        (OUTPUT / f"Lostecho-{size}.png").write_bytes(png)

    ico = encode_ico([(size, encoded[size]) for size in ICO_SIZES])
    (OUTPUT / "Lostecho.ico").write_bytes(ico)
