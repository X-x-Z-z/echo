...
# Shared, source-only helpers for Lostecho release tooling.
...
release_write_pin_file() {
  ...
  printf 'ECHO_HISTORY_STEP_RUNTIME_METADATA_RELEASE_DIGEST=%s\n' \
    "$RELEASE_METADATA_DIGEST" > "$temporary"
  printf 'ECHO_HISTORY_STEP_PACK_LEAF_DIGESTS=%s\n' \
...
release_build_pack_tools() {
  ...
  local build_args=(--bin echo_pack_pins)
  ...
  RELEASE_TOOL_TARGET_DIR=${ECHO_RELEASE_TOOL_TARGET_DIR:-$RELEASE_ROOT_DIR/target/release-tools}
  if [[ $include_generator == 1 ]]; then
    build_args+=(--bin echo_matrix_gen)
  fi
  case "$(rustc -vV | sed -n 's/^host: //p' | tr -d '\r')" in
    *-windows-*)
  ...
  (
    ...
    unset ECHO_HISTORY_STEP_PACK_LEAF_DIGESTS
  ...
  RELEASE_PIN_TOOL=$(release_tool_executable echo_pack_pins)
  [[ -x $RELEASE_PIN_TOOL ]] || release_die "pin tool was not built: $RELEASE_PIN_TOOL"
  if [[ $include_generator == 1 ]]; then
    RELEASE_MATRIX_GENERATOR=$(release_tool_executable echo_matrix_gen)
...
release_compute_pack_pins() {
  ...
  metadata_digest=$(
    printf '%s\n' "$output" |
      sed -n 's/^ECHO_HISTORY_STEP_RUNTIME_METADATA_RELEASE_DIGEST=//p'
  )
  leaf_digests=$(
    printf '%s\n' "$output" |
      sed -n 's/^ECHO_HISTORY_STEP_PACK_LEAF_DIGESTS=//p'
  )
  [[ $metadata_digest =~ ^[0-9a-f]{64}$ ]] || \
...
release_workspace_version() {
  ...
  release_require_command tr
  node_pkg=$(cd "$RELEASE_ROOT_DIR" && cargo pkgid -p echo_node | tr -d '\r')
  extminer_pkg=$(cd "$RELEASE_ROOT_DIR" && cargo pkgid -p echo-extminer | tr -d '\r')
  if [[ $node_pkg == *@* ]]; then
  ...
  [[ $node_version =~ ^[0-9]+\.[0-9]+\.[0-9]+([+-][0-9A-Za-z.-]+)?$ ]] || \
    release_die "cannot determine echo_node version from: $node_pkg"
