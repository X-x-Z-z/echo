...
BASE = Path(
    os.environ.get(
        "ECHO_LIVE_WALLET_RECEIVE_ONLINE_DIR",
        str(RUN_PARENT / f"wallet-receive-online-clean-{STAMP}"),
    )
)
BASE_PORT = int(os.environ.get("ECHO_LIVE_WALLET_RECEIVE_ONLINE_BASE_PORT", "22100"))
FUNDING_HEIGHT = 4
...
def main():
    ...
    try:
        ...
        require(
            balance(recipient)["balance_microecho"] == 0,
        ...
        require(
            int(sender_before["spendable_microecho"]) > sum(PAYMENTS),
        ...
        for index, amount in enumerate(PAYMENTS, start=1):
            ...
            require(
                int(before["balance_microecho"]) == expected_balance,
            ...
            updated = live.wait_value(
                ...
                lambda expected=expected_balance, count=index: (
                    current
                    if int(current["balance_microecho"]) == expected
            ...
            print(
                ...
                f"balance={updated['balance_microecho']} utxos={updated['utxo_count']}",
                flush=True,
            )
        ...
        require(
            int(final_balance["balance_microecho"]) == sum(PAYMENTS),
