...
Environment knobs:
  ECHO_LIVE_TX_ROUNDS   default 2  (each round submits 6 txs: A->B,A->C,B->C,B->A,C->A,C->B)
  ECHO_LIVE_START_BLOCKS default 20
  ECHO_LIVE_BASE_PORT   default 19600
"""
...
START_BLOCKS = int(os.environ.get("ECHO_LIVE_START_BLOCKS", "20"))
TX_ROUNDS = int(os.environ.get("ECHO_LIVE_TX_ROUNDS", "2"))
BASE_PORT = int(os.environ.get("ECHO_LIVE_BASE_PORT", "19600"))
AMOUNT_BASE = 100_000  # 0.1 ECHO in microecho; small enough for many rounds
...
def rpc(url, method, params=None, timeout=8):
    method_full = method if method.startswith("lostecho_") else f"lostecho_{method}"
    body = json.dumps(
...
def main():
    if not NODE_BIN.exists():
        raise LiveTestError(
            f"release binary missing: {NODE_BIN}; run cargo build --release -p echo_node --bin lostecho"
        )
    if BASE.exists():
        shutil.rmtree(BASE)
    LOGS.mkdir(parents=True, exist_ok=True)
    ...
    try:
        ...
        assert_true(
            n1.balance()["spendable_microecho"] >= 100_000_000,
        ...
        for i in range(4):
            ...
            print(
                f"[send funding] A->{dst.name} amount={amount} tx={tx_hash[:12]} fee={send['fee_microecho']}",
                flush=True,
            )
            wait_until(
                "funding tx gossiped",
        ...
        wait_until(
            ...
            lambda: (
                ...
                if n2.balance()["total_microecho"] > 0
                and n3.balance()["total_microecho"] > 0
                else False
            ),
            timeout=120,
            interval=2,
        ...
        for r in range(TX_ROUNDS):
            for j, (src, dst) in enumerate(pairs):
                ...
                tx_hashes.append(tx_hash)
                print(
                    f"[send r{r}] {src.name}->{dst.name} amount={amount} tx={tx_hash[:12]} fee={send['fee_microecho']} hints={hints_before[:3]}",
                    flush=True,
                )
                wait_until(
                ...
                wait_until(
                    ...
                    lambda: (
                        dst.balance()["total_microecho"]
                        if dst.balance()["total_microecho"] >= pre_dst + amount
                        else False
                    ),
                    timeout=120,
                    interval=2,
