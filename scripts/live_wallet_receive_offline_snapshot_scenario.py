#!/usr/bin/env python3
"""Offline recipient recovers a payment at an ↻ snapshot boundary."""
...
BASE = Path(
    os.environ.get(
        "ECHO_LIVE_WALLET_RECEIVE_OFFLINE_SNAPSHOT_DIR",
        str(RUN_PARENT / f"wallet-receive-offline-snapshot-clean-{STAMP}"),
    )
)
BASE_PORT = int(
    os.environ.get("ECHO_LIVE_WALLET_RECEIVE_OFFLINE_SNAPSHOT_BASE_PORT", "22300")
)
FUNDING_HEIGHT = 4
RETAINED_DEPTH = 18
SNAPSHOT_BOUNDARY_INTERVAL = 6
PAYMENT_MICROECHO = 350_000
...
def main():
    ...
    try:
        ...
        require(balance(recipient)["balance_microecho"] == 0, "recipient funded at h0")
        recipient.stop()
        ...
        require(
            int(sender_before["spendable_microecho"]) > PAYMENT_MICROECHO,
        ...
        sent = rpc(
            sender.rpc_port,
            "walletSend",
            [recipient_address["address"], PAYMENT_MICROECHO, 0],
            timeout=300,
        )
        ...
        require(
            ...
            f"sender overshot ↻ source tip: {mined_tip}",
        )
        ...
        require(
            int(recipient_balance["balance_microecho"]) == PAYMENT_MICROECHO,
        ...
        require(
            len(owned) == 1 and int(owned[0]["value"]) == PAYMENT_MICROECHO,
        ...
        require(
            re.search(
                rf"wallet active address reloaded .*height={snapshot_boundary_height}.*utxos=1.*balance={PAYMENT_MICROECHO}.*reason=\"snapshot sync\"",
                recipient_log,
            )
            is not None,
        ...
        print("[PASS] offline recipient recovered payment through ↻ sync", flush=True)
