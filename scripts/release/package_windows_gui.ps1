...
foreach ($Binary in @("lostecho-gui.exe", "lostecho.exe")) {
...
$Definition = Join-Path $ScriptDir "gui\windows\Lostecho.iss"
$Icon = Join-Path $ReleaseRoot "echo_gui\assets\app-icons\Lostecho.ico"
$License = Join-Path $ReleaseRoot "LICENSE"
$Notice = Join-Path $ReleaseRoot "NOTICE"
$OutputBaseFilename = "lostecho-gui-v$Version-$Platform-setup"
...
$InstallDir = Join-Path ([System.IO.Path]::GetTempPath()) (
    "Lostecho-release-smoke-" + [guid]::NewGuid().ToString("N")
)
try {
    ...
    $Wallet = Join-Path $InstallDir "Lostecho.exe"
    $Node = Join-Path $InstallDir "lostecho-node.exe"
    ...
    foreach ($ForbiddenBinary in @("lostecho-cli.exe", "lostecho-miner.exe")) {
