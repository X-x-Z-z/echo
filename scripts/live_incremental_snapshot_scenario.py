#!/usr/bin/env python3
"""Fresh-chain incremental state-snapshot publication and ↻ sync.
...
BASE = Path(
    os.environ.get(
        "ECHO_LIVE_INCREMENTAL_SNAPSHOT_DIR",
        str(RUN_PARENT / f"incremental-snapshot-clean-{STAMP}"),
    )
)
BASE_PORT = int(os.environ.get("ECHO_LIVE_INCREMENTAL_SNAPSHOT_BASE_PORT", "22900"))
SERVING_DEPTH = 42
