...
BASE = Path(
    os.environ.get(
        "ECHO_LIVE_SNAPSHOT_FAILOVER_DIR",
        str(RUN_PARENT / f"snapshot-source-failover-clean-{STAMP}"),
    )
)
BASE_PORT = int(os.environ.get("ECHO_LIVE_SNAPSHOT_FAILOVER_BASE_PORT", "25300"))
...
def last_sync_run():
    explicit = os.environ.get("ECHO_LIVE_SNAPSHOT_FAILOVER_FIXTURE")
    if explicit:
