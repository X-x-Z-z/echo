# Lostecho soundness certificate

`echo_soundness` instantiates the security analysis for the current Lostecho
...
Their published comparison and the production Lostecho row are:
...
| **Lostecho** | History B25 / B255 | **128** | **127** | **127** |

Both Lostecho values lie in the exact interval `[127, 128)`. Their whole-bit
...
theorems and derive every Lostecho-specific term. Separate Rust types preserve
...
This repository provides a Category 1 resource assessment for the Lostecho
soundness game. It does not claim that NIST reviewed or certified Lostecho.
...
From the root of a downloaded Lostecho repository:
...
```sh
cargo run --release --locked -p echo_soundness -- --exact
```

To verify the calculator and production correspondence tests:

```sh
cargo test --release --locked -p echo_soundness
```
