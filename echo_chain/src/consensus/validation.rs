...
pub fn validate_mandatory_coinbase(
    ...
    // cheap preflight.
    echo_tx::validate_body_semantics_no_hash(&coinbase.body)
    ...
    match (allocation.payout_each, stream.has_development_payout) {
        ...
        (Some(expected_amount), true) => {
            ...
            echo_tx::validate_body_semantics_no_hash(&payout.body).map_err(|_| {
            ...
            if first.owner != crate::consensus::development_allocation::O1_NETWORK_FUND_ADDRESS
                || second.owner != crate::consensus::development_allocation::LOSTECHO_LAB_ADDRESS
