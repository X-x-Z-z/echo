...
use echo_ivc_core::deep_chain::relations::{claimed_refs, ColRef, RELATION_DEGREE};
use echo_ivc_core::deep_chain::schedule::{
    carry_selection_terms, duplex_substitution_terms, DuplexFamilyRefs,
};
use echo_ivc_core::deep_chain::WALK_DEGREE;
...
/// Exact shape of [`echo_ivc_core::deep_chain::MultiDeepChainWalkProof`].
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
pub(super) fn duplex_like_proof_shape(
    ...
    let shifts = substitution_refs
        ...
        .filter(|reference| {
            matches!(
                reference,
                echo_ivc_core::deep_chain::relations::ColRef::CommittedShift(_)
                    | echo_ivc_core::deep_chain::relations::ColRef::CommittedShift2(_)
            )
        })
        .count();
    FixedProofShape {
