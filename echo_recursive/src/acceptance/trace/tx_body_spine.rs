...
//!   `echo_gkr::block_spine::verify_block_spine_killshot` (the final
...
use echo_gkr::block_spine::{num_vars_for, BlockSpineProof, BLOCK_SPINE_TX_HASH_PIN_TAG};
use echo_gkr::spine_sumcheck::N_SPINE_SLOTS;
use echo_gkr::tx_body_layout::InstanceRole;
use echo_gkr::{SpineCircuit, SpineInputs};
...
pub use echo_tx::body_hash::BODY_HASH_LEAVES as TX_BODY_RAW_LEAVES;
...
mod tests {
    ...
    fn tx_body_proof_mutator_kills_all() {
        ...
        let stride: usize = std::env::var("ECHO_TRACE_MUTATE_STRIDE")
            .ok()
    ...
    fn tx_body_native_trace_equivalence() {
        let cases: usize = std::env::var("ECHO_TRACE_CROSS_CASES")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(100);
