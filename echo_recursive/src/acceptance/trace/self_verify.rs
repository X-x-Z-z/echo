...
//! The self-verification slot: the `echo_ivc_core` FieldR1cs verifier
...
//! the verified proof's field elements are `echo_ivc_core::F128` values
...
//! primitives are the flat-basis constructions of `echo_ivc_core::merkle`.
...
use echo_ivc_core::field::PHI_8_TABLE;
use echo_ivc_core::field_circuit::{f128_from_u128, FsChannelOps};
use echo_ivc_core::field_r1cs::FieldR1cs;
use echo_ivc_core::merkle::{self, Hash};
use echo_ivc_core::ntt::AdditiveNttF128;
use echo_ivc_core::pcs::{self, compute_fri_arities, default_fri_queries, PcsParams, LOG_PACKING};
use echo_ivc_core::public_io::POST_COMMIT_CLASS_BINDING_LABEL;
use echo_ivc_core::zerocheck::{self, K_SKIP};
use echo_poseidon2b::native::{capacity_iv_flat, DomainTag};
...
/// The `IVCPCSL_` / `IVCPCSN_` tags of `echo_ivc_core::merkle`, duplicated
...
// ---------------------------------------------------------------------------
// Merkle primitives (trace twins of echo_ivc_core::merkle)
// ---------------------------------------------------------------------------
...
/// Trace twin of `echo_ivc_core::merkle::hash_pair` — ONE feed-forward
...
/// Trace twin of `echo_ivc_core::merkle::hash_leaf` for a lane-aligned leaf
...
/// Trace twin of `echo_ivc_core::proof::bind_statement_field`. The instance
...
pub fn bind_statement_field_trace(
    ...
    ch.observe_bytes_const(
        b,
        &echo_ivc_core::proof::pcs_params_statement_bytes(pcs_params),
    );
...
impl LincheckProofTrace {
    pub fn alloc(
        b: &mut FieldR1csBuilder,
        native: &echo_ivc_core::lincheck::LincheckProof,
        k_log: usize,
        k_skip: usize,
    ) -> Self {
...
impl C1LincheckProofTrace {
    pub fn alloc(
        b: &mut FieldR1csBuilder,
        native: &echo_ivc_core::lincheck::c1::C1LincheckProof,
        k_log: usize,
        k_skip: usize,
    ) -> Self {
        assert_eq!(native.rounds.len(), k_log - k_skip, "rounds off shape");
...
impl FieldR1csProofTrace {
    pub fn alloc(
        b: &mut FieldR1csBuilder,
        native: &echo_ivc_core::proof::FieldR1csProof,
        r1cs: &FieldR1cs,
        pcs_params: &PcsParams,
    ) -> Self {
        Self::alloc_shape(
            b,
            native,
            &echo_ivc_core::proof::FieldShape::of(r1cs),
            pcs_params,
        )
    }
    ...
    pub fn alloc_shape(
        b: &mut FieldR1csBuilder,
        native: &echo_ivc_core::proof::FieldR1csProof,
        shape: &echo_ivc_core::proof::FieldShape,
        pcs_params: &PcsParams,
    ) -> Self {
...
impl C1FieldR1csProofTrace {
    pub fn alloc(
        b: &mut FieldR1csBuilder,
        native: &echo_ivc_core::proof::C1FieldR1csProof,
        r1cs: &FieldR1cs,
        pcs_params: &PcsParams,
    ) -> Self {
...
pub fn bind_public_io_trace(
    b: &mut FieldR1csBuilder,
    ch: &mut impl FsChannelOps,
    spec: &echo_ivc_core::public_io::PublicIoSpec,
    io: &[LinExpr],
    total_vars: usize,
    ...
    let prefix = |slice: &echo_ivc_core::public_io::WitnessSlice| -> Vec<LinExpr> {
        slice
...
pub fn bind_public_io_c1_trace(
    ...
    let prefix = |slice: &echo_ivc_core::public_io::WitnessSlice| {
        slice
            .prefix_coords(total_vars)
            .into_iter()
...
fn verify_field_trace_inner(
    ...
    commitment_root: &FlatDigestExpr,
    proof: &FieldR1csProofTrace,
    public_io: Option<(&echo_ivc_core::public_io::PublicIoSpec, &[LinExpr])>,
) -> R1csClaimTrace {
    assert_eq!(
        pcs_params.m,
        r1cs.m + LOG_PACKING,
...
pub(crate) fn shape_only_field_r1cs_proof_c1(
    shape: &echo_ivc_core::proof::FieldShape,
    pcs_params: &PcsParams,
) -> (echo_ivc_core::proof::C1FieldR1csProof, Hash) {
    ...
    let proof = echo_ivc_core::proof::C1FieldR1csProof {
        ...
        lincheck: echo_ivc_core::lincheck::c1::C1LincheckProof {
...
pub(crate) fn patch_shape_only_query_positions_c1(
    proof: &mut echo_ivc_core::proof::C1FieldR1csProof,
    pcs_params: &PcsParams,
    ...
    for lane in query_lane_values {
        let raw = echo_ivc_core::field_circuit::f128_to_u128(*lane);
...
mod tests {
    use super::*;
    use echo_ivc_core::challenger::{fs_pack_bytes_lanes, Challenger, FsLaneChallenger};
    use echo_ivc_core::deep_chain::schedule::{compile_duplex, DuplexLayout};
    use echo_ivc_core::field_circuit::{f128_to_u128, FsChannelUnionRecorder};
    ...
    use echo_ivc_core::field_circuit::FsChannelTrace;
    ...
    /// The module's private tag copies match the ones `echo_ivc_core::merkle`
    ...
    fn duplicated_tags_match_native_merkle() {
        use echo_poseidon2b::native::{compress_flat_feed_forward_with_tag, Poseidon2bFlatSponge};
    ...
    fn lagrange_windows_match_native() {
        use echo_ivc_core::zerocheck::multilinear::{
    ...
    fn lincheck_fixture(
        ...
        echo_ivc_core::lincheck::LincheckProof,
    ) {
        use echo_ivc_core::field_r1cs::synthetic_satisfiable;
        use echo_ivc_core::lincheck::{self, QuirkyPoint};
    ...
    fn c1_lincheck_fixture(
        ...
        echo_ivc_core::lincheck::c1::C1LincheckProof,
    ) {
        use echo_ivc_core::field_r1cs::synthetic_satisfiable;
        use echo_ivc_core::lincheck::c1::{self, C1QuirkyPoint};
    ...
    fn pcs_fixture(
        ...
        use echo_ivc_core::lincheck::build_eq_table;
        use echo_ivc_core::zerocheck::multilinear::lagrange_weights_naive;
    ...
    fn field_proof_fixture(
        ...
        PcsParams,
        pcs::Commitment,
        echo_ivc_core::proof::FieldR1csProof,
        echo_ivc_core::proof::R1csClaim,
    ) {
        use echo_ivc_core::field_r1cs::synthetic_satisfiable;
        use echo_ivc_prover::field_prover::prove_field;
    ...
    fn c1_field_proof_fixture(
        ...
        PcsParams,
        pcs::Commitment,
        echo_ivc_core::proof::C1FieldR1csProof,
        echo_ivc_core::proof::C1R1csClaim,
    ) {
        use echo_ivc_core::field_r1cs::synthetic_satisfiable;
        use echo_ivc_prover::field_prover::prove_field_c1;
    ...
    fn build_field_verify_trace(
        ...
        params: &PcsParams,
        commitment: &pcs::Commitment,
        proof: &echo_ivc_core::proof::FieldR1csProof,
    ) -> (
        FieldR1cs,
        Vec<F128>,
        std::ops::Range<usize>,
        ...
        echo_ivc_core::verifier::verify_field(r1cs, commitment, proof, &mut ch_native)
    ...
    fn verify_field_c1_replay_lockstep_e2e() {
        ...
        echo_ivc_core::verifier::verify_field_c1(&r1cs, &commitment, &proof, &mut native)
    ...
    fn verify_field_c1_deferred_capture_lockstep_e2e() {
        use echo_ivc_core::field_r1cs::synthetic_satisfiable;
        use echo_ivc_prover::field_prover::prove_field_c1_capturing_fresh;
        ...
        let shape = echo_ivc_core::proof::FieldShape::of(&r1cs);
    ...
    fn verify_field_c1_production_envelope_lockstep_e2e() {
        use echo_ivc_core::field_r1cs::synthetic_satisfiable;
        use echo_ivc_core::public_io::{PublicIoSpec, WitnessSlice};
        use echo_ivc_core::verifier::VerifyError;
        use echo_ivc_prover::field_prover::prove_field_c1_with_public_io_and_post_commit_context;
            ...
            prove_field_c1_with_public_io_and_post_commit_context(
                ...
                |context| {
                    ...
                    let value = context
                        .witness()
                        .iter()
                        .zip(echo_ivc_core::lincheck::build_eq_table(&point))
        ...
        let (_, native_fresh) =
            echo_ivc_core::verifier::verify_field_c1_deferred_matrix_with_post_commit_context(
                &shape,
    ...
    fn verify_field_replay_with_public_io_lockstep_and_mutations() {
        use echo_ivc_core::field_r1cs::SparseFieldMatrix;
        use echo_ivc_core::public_io::{IoClaimSpec, PublicIoSpec, WitnessSlice};
        use echo_ivc_prover::field_prover::prove_field_with_public_io;
        ...
        let eq = echo_ivc_core::lincheck::build_eq_table(&p);
        let mut v = F128::ZERO;
        ...
        echo_ivc_core::verifier::verify_field_with_public_io(
            &inner,
            &commitment,
            &proof,
            &spec,
    ...
    fn verify_field_deferred_lockstep_and_mutations() {
        use echo_ivc_core::field_r1cs::SparseFieldMatrix;
        use echo_ivc_core::matrix_claim::{
            prove_matrix_claim_fold, stacked_matrix_mle_eval, MatrixAccClaim,
        };
        use echo_ivc_core::proof::FieldShape;
        use echo_ivc_core::public_io::{PublicIoSpec, WitnessSlice};
        use echo_ivc_prover::field_prover::prove_field_with_public_io;
        ...
        let (_, fresh_native) = echo_ivc_core::verifier::verify_field_deferred_matrix(
            &shape,
            &digest,
            &commitment,
            &proof,
    ...
    fn verify_field_deferred_post_commit_auxiliary_lockstep() {
        ...
        use echo_ivc_core::proof::FieldShape;
        use echo_ivc_core::public_io::{PublicIoSpec, WitnessSlice};
        use echo_ivc_core::verifier::{verify_field_deferred_matrix_with_post_commit, VerifyError};
        use echo_ivc_prover::field_prover::prove_field_with_public_io_and_post_commit;
    ...
    fn verify_field_deferred_post_commit_context_lockstep() {
        ...
        use echo_ivc_core::proof::FieldShape;
        use echo_ivc_core::public_io::{PublicIoSpec, WitnessSlice};
        use echo_ivc_core::verifier::{
            verify_field_deferred_matrix_with_post_commit_context, VerifyError,
        };
        use echo_ivc_prover::field_prover::prove_field_with_public_io_and_post_commit_context;
