...
use echo_ivc_core::field_circuit::{f128_from_u128, ExtExpr};
pub const HISTORY_STEP_WIRE_VERSION: u8 = 4;

const _: () = assert!(
    HISTORY_STEP_WIRE_VERSION == echo_chain::history_step::HISTORY_STEP_TERMINAL_VERSION,
...
fn auxiliary_sidecar_error(label: &str, error: RegionSidecarError) -> VerifyError {
    if std::env::var_os("ECHO_HISTORY_STEP_AUX_DEBUG").is_some() {
    ...
    pub fn new(
        ...
        for (slot, vk) in direct_block_vks.iter().enumerate() {
            if crate::region_sidecar::selected_zk_block_geometry(
                echo_chain::consensus::params::BLOCK_PAGE_CLASS_TIERS[slot],
            )
            .is_none()
...
fn history_step_query_lane_count(params: &PcsParams) -> usize {
    let log_dim = params.m - echo_ivc_core::pcs::LOG_PACKING - params.log_batch_size;
    let k_code = log_dim + params.log_inv_rate;
    let per_lane = 128 / k_code;
    echo_ivc_core::pcs::default_fri_queries(params.log_dim(), params.log_inv_rate)
        .div_ceil(per_lane)
}
...
impl HistoryStepTerminal {
    ...
    pub const fn wire_version(&self) -> u8 {
        echo_chain::history_step::history_step_terminal_wire_version(self.height)
    }
...
fn prepare_history_step_assembly<const TIER: usize>(
    ...
    let timing = std::env::var_os("ECHOH_HISTORY_ASSEMBLY_TIMING").is_some();
    ...
    if echo_chain::consensus::paged_spend::BlockProofClass::for_page_count(effective_pages)
    ...
    // header, pinned through both derived ids.
    {
        ...
        let genesis_id = digest_lanes(&echo_chain::hash_block_header(&genesis));
        let genesis_semantic =
            digest_lanes(&echo_chain::block_header::semantic_header_id(&genesis));
        for lane in 0..2 {
    ...
    Ok(match r1cs {
        ...
        None => {
            ...
            let built = BuiltHistoryStep {
                ...
                useful_rows: used,
                class_id: current_class,
                semantic_id: echo_chain::block_header::semantic_header_id(sealed_header),
...
mod tests {
    ...
    fn seal_fixture_parent(height: u64) -> echo_chain::BlockHeader {
    ...
    fn parent_seal_replay_rejects_wrong_prev_block_hash() {
        ...
        let link = echo_chain::hash_block_header(&parent);
        let tip = echo_chain::block_header::semantic_header_id(&parent);
        ...
        assert!(seal_glue_satisfies(
            &renonced,
            echo_chain::hash_block_header(&renonced),
            tip,
        ));
        ...
        assert!(!seal_glue_satisfies(
            &tampered,
            echo_chain::hash_block_header(&tampered),
            tip,
        ));
    }
    ...
    fn epoch_lane_updates_from_derived_parent_id_at_boundary() {
        ...
        for parent_height in [143u64, 144] {
            ...
            let parent_id = echo_chain::hash_block_header(&parent);
            ...
            let child = echo_chain::BlockHeader {
            ...
            let satisfies = |end: &ChainAccumulator| {
                ...
                let child_wires = DirectChildWires {
                    semantic_id: digest_lanes(&echo_chain::block_header::semantic_header_id(
                        &child,
                    ))
                    .map(|lane| {
                        LinExpr::from_wire(
                    ...
                    height: LinExpr::from_wire(builder.alloc_f128(
                        crate::acceptance::trace::flat_of(echo_core::Block128::from(
    ...
    fn base_case_selector_rejects_non_genesis_parent() {
        ...
        let base_pins_satisfy = |parent: &echo_chain::BlockHeader| {
            let genesis = echo_chain::consensus::genesis_header();
            let genesis_id = digest_lanes(&echo_chain::hash_block_header(&genesis));
            let genesis_semantic =
                digest_lanes(&echo_chain::block_header::semantic_header_id(&genesis));
        ...
        assert!(base_pins_satisfy(&echo_chain::consensus::genesis_header()));
        assert!(!base_pins_satisfy(&seal_fixture_parent(0)));
        let mut renonced_genesis = echo_chain::consensus::genesis_header();
