...
use echo_ivc_core::pcs::basefold::{C1QueryOpening, C1RoundMessage};
use echo_ivc_core::pcs::{
...
use echo_ivc_core::proof::{C1FieldR1csProof, FieldShape};
...
fn decode_field_proof(
    ...
    let skip = 1usize << shape.k_skip;
    let zerocheck = echo_ivc_core::zerocheck::field_c1::C1ZerocheckProof {
    ...
    let lincheck = echo_ivc_core::lincheck::c1::C1LincheckProof {
...
fn decode_terminal_in_format(
    ...
    let allowed_length = if shared {
        ...
        expected <= echo_chain::consensus::wire_limits::MAX_HISTORY_STEP_TERMINAL_TRANSPORT_BYTES
            && (minimum..=expected).contains(&bytes.len())
            && bytes.len()
                <= echo_chain::consensus::wire_limits::V1_1_MAX_HISTORY_STEP_TERMINAL_BYTES
    } else {
        bytes.len() == expected
    };
    if !allowed_length {
