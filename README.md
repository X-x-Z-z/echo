...
Lostecho removes this requirement.

In Lostecho, validity is established once, where the complete information
...
| | Conventional blockchain | Lostecho |
|---|---|---|
...
Lostecho is transparent, not a privacy chain. Current values and owners are
...
[wallet derivation](echo_soundness/docs/wallet-johnson.md) gives the finite
...
[Block–Tiwari derivation](echo_soundness/docs/block-tiwari.md).
...
claim. See the [response accounting](echo_soundness/docs/response-accounting.md)
and the [end-to-end QROM and Category 1 derivation](echo_soundness/docs/category-one.md).

## How It Works

### Execution Is Local

When sending ECHO, the wallet selects its UTXOs and creates one atomic
...
Lostecho is history-stateless, not state-free. State transfer scales with the
...
reward halves when the state domain actually expands, with a permanent 1 ECHO
floor.
...
For Lostecho, we developed FROST-GKR (Frobenius Reduction Over Shifted
...
in the official binary and can be regenerated from source. The Lostecho Lab
[FROST-GKR research article](https://lab.lostecho.org/research/frost-gkr-global-trace-protocol/)
...
Lostecho has no premine. For blocks 1 through 4,730,400 — exactly three
...
The ↻ Network Fund finances operation, maintenance, security and adoption
of the live network. Lostecho Lab supports the founders, core developers,
...
Lostecho uses libp2p GossipSub for blocks and transaction intents, typed
...
```sh
lostecho --seed <host>:9600
```

External nonce search keeps transaction selection and proving inside the node:

```sh
lostecho --extminer --mining-key-file ~/.lostecho/mining.key
lostecho-miner --key-file ~/.lostecho/mining.key
```
...
creates `~/.lostecho/lostecho.toml`, the MDBX state and the built-in wallet
under `~/.lostecho/data/`.
...
Addresses use bech32m and begin with `o1`. `1 ECHO = 1,000,000 μECHO`.
`ECHO` is the ticker; the wallet uses `↻` as its interface symbol.
...
lostecho-cli state
lostecho-cli mining
lostecho-cli address
lostecho-cli address --new
lostecho-cli balance
lostecho-cli utxos
lostecho-cli send <o1-address> 10.5 --dry-run
lostecho-cli send <o1-address> 10.5
lostecho-cli mempool
lostecho-cli history
lostecho-cli receipt <txid> > receipt.hex
lostecho-cli verify "$(tr -d '\n' < receipt.hex)"
lostecho-cli stop
```

Run `lostecho --help`, `lostecho-cli help` or `lostecho-miner --help` for the full
interface.
...
production node. Run `lostecho --check-hardware` before installation to see
...
mkdir -p ../lostecho-artifacts
./scripts/generate_history_step_pack.sh \
  ../lostecho-artifacts/history-step-pack-v1
```
...
- a Core archive containing `lostecho`, `lostecho-cli` and `lostecho-miner`;
- a native GUI Wallet package containing `lostecho-gui` and its private,
  locally supervised `lostecho` node.

```sh
./scripts/build_release.sh \
  --pack ../lostecho-artifacts/history-step-pack-v1
...
[`echo_soundness`](echo_soundness/README.md) and run:

```sh
cargo run --release --locked -p echo_soundness
```
