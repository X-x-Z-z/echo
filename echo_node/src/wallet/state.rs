...
use echo_poseidon2b::primitives::{Address, SpendSecret};
...
pub struct TxHistoryEntry {
    ...
    /// Net amount in μECHO (sent: net sent, received: total received).
...
pub struct WalletUtxo {
    ...
    /// This must be copied into every spending [`echo_tx::TxInput`].
    pub creation_id: u64,
...
pub struct WalletState {
    ...
    /// Cached different-address receipts: txid → serialized LostechoReceipt.
...
impl WalletState {
    ...
    pub fn select_utxos(&self, target: u64, fee: u64) -> Option<(Vec<&WalletUtxo>, u64)> {
        ...
        let input_limit = echo_tx::MAX_PAGED_SPEND_INPUTS;
        ...
        let mut total = 0u64;
        for utxo in available.into_iter().take(echo_tx::MAX_PAGED_SPEND_INPUTS) {
            selected.push(utxo);
...
mod tests {
    ...
    fn snapshot(owner: [u8; 32], amount: Option<u64>) -> VerifiedOwnerSnapshot {
        VerifiedOwnerSnapshot {
            ...
            alloc_counter: 9,
            utxos: amount
                .map(|amount| echo_chain::storage::VerifiedOwnerUtxo {
                    slot_index: 5,
                    amount,
                    creation_id: 8,
                })
