...
//! Built-in wallet for the `lostecho` daemon.
//!
...
use echo_chain::storage::VerifiedOwnerSnapshot;
use echo_rpc::types::{
    microecho_to_echo, FeeBreakdownInfo, WalletAddressInfo, WalletBalance, WalletConsolidationPlan,
...
fn wallet_utxo_records(wallet: &WalletState) -> Vec<WalletUtxoInfo> {
    ...
    wallet
        ...
        .map(|utxo| WalletUtxoInfo {
            ...
            value_microecho: utxo.value,
            creation_id: utxo.creation_id,
            value_echo: microecho_to_echo(utxo.value),
            address: addresses
...
pub fn update_for_accepted_block(
    ...
    wallet.active_snapshot = Some(state::ActiveWalletSnapshot {
        ...
        tip_hash: echo_chain::consensus::pow::block_id(&block.header),
    ...
    let confirmed_block_hash = echo_chain::block_id(&block.header);
    for txid in echo_chain::try_compute_logical_txids(&block.transactions)
...
fn recover_outgoing_receipts_from_block(
    ...
    let tx_hashes = echo_chain::try_compute_logical_txids(&block.transactions)
    ...
    for (group_index, group) in stream.groups.iter().enumerate() {
        ...
        if !wallet.receipts.contains_key(&tx_hash) {
            wallet.receipts.insert(
                tx_hash,
                echo_chain::consensus::receipt::generate_receipt(
                    &block.header,
                    pages,
                    tx_index,
                    &tx_hashes,
...
pub fn reconcile_receipts_at_startup(
    wallet: &mut WalletState,
    chain: &echo_chain::storage::MdbxChainContext,
) -> Result<(usize, usize), String> {
    use echo_chain::consensus::params::RECENT_BLOCK_RETENTION_DEPTH;
    use echo_chain::consensus::receipt::{
        verify_against_header, verify_merkle_inclusion, LostechoReceipt,
    };
    ...
    for tx_hash in receipt_keys {
        let receipt = wallet
            .receipts
            .get(&tx_hash)
            .and_then(|bytes| LostechoReceipt::from_bytes(bytes).ok());
    ...
    if first <= tip {
        for height in first..=tip {
            ...
            let block = echo_chain::block::Block::from_bytes(bundle.block_bytes())
...
pub fn install_reorg_snapshot_and_artifacts(
    ...
    reclaimed_tx_hashes: &[echo_poseidon2b::primitives::TxBodyHash],
    replacement_blocks: &[&echo_chain::block::Block],
) -> Result<(), String> {
    let mut guard = wallet
        .lock()
        .map_err(|_| "wallet state lock is poisoned".to_string())?;
...
impl WalletOps for WalletHandle {
        ...
        match &*guard {
            ...
            Some(w) => {
                ...
                WalletStatus {
                    ...
                    balance_microecho: balance,
                    balance_echo: microecho_to_echo(balance),
    ...
    fn get_balance(&self) -> WalletBalance {
        ...
        match &*guard {
            None => WalletBalance {
                ...
                balance_echo: 0.0,
                utxo_count: 0,
                pending_outbound_microecho: 0,
                pending_incoming_microecho: 0,
                spendable_microecho: 0,
                spendable_echo: 0.0,
            },
            Some(w) => {
                let total = w.balance();
                ...
                WalletBalance {
                    balance_microecho: total,
                    balance_echo: microecho_to_echo(total),
                    utxo_count: w.utxos.len(),
                    pending_outbound_microecho: pending_out,
                    pending_incoming_microecho: 0,
                    spendable_microecho: spendable,
                    spendable_echo: microecho_to_echo(spendable),
                }
            }
        }
    }
        ...
        match &*guard {
            ...
            Some(w) => w
                ...
                .map(|h| WalletHistoryEntry {
                    ...
                    amount_microecho: h.amount_microecho,
                    amount_echo: microecho_to_echo(h.amount_microecho),
                    peer_address: h
                        .peer_address
                        .map(|a| echo_poseidon2b::primitives::Address(a).to_bech32()),
    ...
    fn receipts(&self, offset: usize, limit: usize) -> Result<WalletReceiptSlice, String> {
        use echo_chain::consensus::receipt::LostechoReceipt;
        ...
        for (txid, bytes) in &wallet.receipts {
            let receipt = LostechoReceipt::from_bytes(bytes).map_err(|error| {
            ...
            receipts.push(WalletReceiptRecord {
                ...
                amount_microecho: sent.map_or(derived_amount, |entry| entry.amount_microecho),
                fee_microecho: receipt.summary.fee_microecho,
                peer_address: sent
                    .and_then(|entry| entry.peer_address)
                    .map(|owner| echo_poseidon2b::primitives::Address(owner).to_bech32())
                    .or(derived_peer),
                own_address: sent
        ...
        let blocks = wallet
            ...
            .filter_map(|entry| {
                Some(WalletMinedBlockRecord {
                    ...
                    reward_microecho: entry.amount_microecho,
    ...
    fn on_accepted_block(&self, block: &echo_chain::block::Block) -> Result<(), String> {
    ...
    fn plan_send(
        ...
        explicit_fee_microecho: Option<u64>,
        active_slot_count: u64,
        log_slots: u32,
        relay_floor: u64,
    ) -> Result<WalletSendPlan, WalletSendPlanError> {
        if amount_microecho == 0 {
        ...
        available.sort_by_key(|utxo| {
            (
                ...
                utxo.slot_index >> echo_chain::consensus::params::LOG_SEGMENT_SIZE,
                utxo.slot_index,
            )
        });
        ...
        let one_input_one_output =
            echo_chain::consensus::fee_breakdown(1, 1, active_slot_count, log_slots);
        let one_input_one_output_minimum = one_input_one_output.required_total.max(relay_floor);
        if let Some(fee) = explicit_fee_microecho {
            if fee < one_input_one_output_minimum {
                return Err(WalletSendPlanError::Other(format!(
                    "fee too low for transaction with 1 input and 1 output: required {one_input_one_output_minimum} μECHO, got {fee} μECHO"
                )));
            }
        }
        let minimum_fee = explicit_fee_microecho.unwrap_or(one_input_one_output_minimum);
        let minimum_total = amount_microecho.checked_add(minimum_fee).ok_or_else(|| {
        ...
        if spendable < minimum_total {
            return Err(WalletSendPlanError::InsufficientFunds {
                needed_microecho: minimum_total,
                available_microecho: spendable,
            });
        }

        let mut selected_value = 0u64;
        let mut planned = explicit_fee_microecho.and_then(|fee| {
            available
                .iter()
        ...
        for input_count in 1..=MAX_PAGED_SPEND_INPUTS {
            ...
            if let Some(fee) = explicit_fee_microecho {
                ...
                if fee < minimum {
                    return Err(WalletSendPlanError::Other(format!(
                        "fee too low for transaction with {input_count} input(s) and {output_count} output(s): required {minimum} μECHO, got {fee} μECHO"
                    )));
                }
            ...
            let one_output_breakdown = echo_chain::consensus::fee_breakdown(
                input_count as u64,
                1,
                active_slot_count,
                log_slots,
            ...
            if selected_value > amount_microecho {
                let two_output_breakdown = echo_chain::consensus::fee_breakdown(
                    input_count as u64,
                    2,
                    active_slot_count,
                    log_slots,
            ...
            if selected_value >= amount_microecho {
                let no_change_fee = selected_value - amount_microecho;
        ...
        let Some((_, fee_microecho, _, _, _)) = planned else {
        ...
        let Some((selected, change_microecho)) =
            wallet.select_utxos(amount_microecho, fee_microecho)
        else {
            if available.len() > MAX_PAGED_SPEND_INPUTS {
                return Err(WalletSendPlanError::InputLimitExceeded {
                    max_inputs: MAX_PAGED_SPEND_INPUTS,
            ...
            return Err(WalletSendPlanError::InsufficientFunds {
                needed_microecho: amount_microecho.saturating_add(fee_microecho),
                available_microecho: spendable,
            });
        };
        let input_count = selected.len();
        let output_count = 1 + usize::from(change_microecho > 0);
        let breakdown = echo_chain::consensus::fee_breakdown(
            input_count as u64,
            output_count as u64,
            active_slot_count,
        ...
        let minimum = breakdown.required_total.max(relay_floor);
        if fee_microecho < minimum {
            return Err(WalletSendPlanError::Other(format!(
                "fee too low for selected transaction with {input_count} input(s) and {output_count} output(s): required {minimum} μECHO, got {fee_microecho} μECHO"
            )));
        }
        let total_spend_microecho =
            amount_microecho.checked_add(fee_microecho).ok_or_else(|| {
                WalletSendPlanError::Other("payment amount plus fee overflows u64".to_string())
            })?;
        Ok(WalletSendPlan {
            amount_microecho,
            fee_microecho,
            total_spend_microecho,
            input_count,
            output_count,
            change_microecho,
            fee_breakdown: fee_breakdown_info(breakdown, relay_floor, fee_microecho),
        })
    }
    fn build_send(
        &self,
        ...
        let (_txid, intent_bytes) =
            builder::build_and_prove_tx(to_address, amount_microecho, fee_microecho, build_data)
                .map_err(|e| e.to_string())?;
    ...
    fn plan_consolidation(
        ...
        let balance_before_microecho = available
            .iter()
            .map(|utxo| utxo.value)
            .try_fold(0u64, u64::checked_add)
            .ok_or_else(|| {
        ...
        let input_value_microecho = selected
            .iter()
            .map(|utxo| utxo.value)
            .try_fold(0u64, u64::checked_add)
            .ok_or_else(|| {
        ...
        let fee_microecho = breakdown.required_total.max(relay_floor);
        let Some(output_value_microecho) = input_value_microecho.checked_sub(fee_microecho) else {
            return Err(WalletSendPlanError::InsufficientFunds {
                needed_microecho: fee_microecho.saturating_add(1),
                available_microecho: input_value_microecho,
            });
        };
        if output_value_microecho == 0 {
            return Err(WalletSendPlanError::InsufficientFunds {
                needed_microecho: fee_microecho.saturating_add(1),
                available_microecho: input_value_microecho,
            });
        ...
        Ok(WalletConsolidationPlan {
            input_value_microecho,
            fee_microecho,
            output_value_microecho,
            balance_before_microecho,
            balance_after_microecho: balance_before_microecho - fee_microecho,
            input_count,
            untouched_count,
    ...
    fn reserve_pending_submission(
        ...
        wallet.record_pending_send(txid, amount_microecho, peer_address)?;
        Ok(())
    }
                    ...
                    entry.tx_hash == tx_hash
                        ...
                        && entry.peer_address.is_some_and(|peer| {
                            entry.own_address.as_deref().is_some_and(|own| {
                                echo_poseidon2b::primitives::Address(peer).to_bech32() == own
                            })
                        })
                });
...
mod tests {
    ...
    fn valid_outgoing_receipt(
        owner: echo_poseidon2b::primitives::Address,
        height: u64,
    ) -> ([u8; 32], Vec<u8>) {
        use echo_chain::block_header::BlockHeader;
        use echo_chain::consensus::receipt::generate_receipt;
        ...
        let logical_txids = echo_chain::try_compute_logical_txids(&transactions).unwrap();
        let logical_hashes = logical_txids.iter().map(|txid| txid.0).collect::<Vec<_>>();
        let header = BlockHeader {
            prev_block_hash: [0; 32],
            state_root: [0; 32],
            tx_root: echo_chain::consensus::receipt::tx_root(&logical_hashes),
    ...
    fn startup_reconciliation_keeps_valid_receipt_until_header_arrives() {
        ...
        let chain =
            echo_chain::storage::MdbxChainContext::open_or_create(chain_dir.path()).unwrap();
    ...
    fn retained_body_rebuilds_missing_outgoing_receipt_without_history_marker() {
        ...
        let header = BlockHeader {
            prev_block_hash: [0; 32],
            state_root: [0; 32],
            tx_root: echo_chain::block::compute_tx_root(&transactions),
            timestamp: 123,
            height: 7,
            miner_address: Address([0x77; 32]),
            nonce: 0,
        ...
        let block = echo_chain::block::Block {
            header,
            transactions,
        };
        ...
        let tx_hash = echo_chain::try_compute_logical_txids(&block.transactions).unwrap()[1].0;
        let receipt = LostechoReceipt::from_bytes(&wallet.receipts[&tx_hash]).unwrap();
    ...
    fn automatic_fee_path_selects_more_than_one_physical_page() {
        ...
        assert!(plan.input_count > echo_tx::TX_INPUTS);
    ...
    fn consolidation_below_limit_merges_every_spendable_utxo() {
        ...
        assert_eq!(plan.input_value_microecho, expected_balance);
        assert_eq!(
            plan.output_value_microecho + plan.fee_microecho,
            expected_balance
        );
        assert_eq!(
            plan.balance_after_microecho,
            expected_balance - plan.fee_microecho
        );
    }
    ...
    fn consolidation_above_limit_selects_exactly_the_64_smallest_utxos() {
        ...
        assert_eq!(plan.input_value_microecho, expected_input_value);
        assert_eq!(plan.balance_before_microecho, expected_balance);
        assert_eq!(
            plan.balance_after_microecho,
            expected_balance - plan.fee_microecho
        );
    ...
    fn explicit_fee_below_one_by_one_minimum_is_rejected_before_selection() {
        ...
        assert_eq!(
            error,
            WalletSendPlanError::Other(
                "fee too low for transaction with 1 input and 1 output: required 5800 μECHO, got 1 μECHO"
                    .to_string()
            )
        );
    }
    ...
    fn explicit_fee_prefers_exact_single_before_greedy_change() {
        ...
        assert_eq!(plan.output_count, 1);
        assert_eq!(plan.change_microecho, 0);
        assert_eq!(plan.fee_microecho, 5_800);
    ...
    fn building_is_side_effect_free_until_pending_reservation_is_installed() {
        ...
        let intent =
            echo_tx::PagedSpendIntent::from_bytes(&intent_bytes).expect("decode built intent");
    ...
    fn planner_excludes_pending_inputs_from_spendable_balance() {
        ...
        assert_eq!(
            ...
            WalletSendPlanError::InsufficientFunds {
                needed_microecho: 15_000,
                available_microecho: 4_000,
            }
        );
    }
    ...
    fn status_and_pending_balance_cover_cached_active_utxos_only() {
        ...
        assert_eq!(balance.balance_microecho, 4_000);
        assert_eq!(balance.pending_outbound_microecho, 2_000);
        assert_eq!(balance.spendable_microecho, 2_000);
    ...
    fn discovery_derives_twenty_addresses_without_loading_inactive_balances() {
        ...
        assert_eq!(
            balance_after.balance_microecho,
            balance_before.balance_microecho
        );
    ...
    fn address_list_keeps_new_address_inactive() {
        ...
        let slots_before = handle
            .list_utxos()
            .into_iter()
            .map(|utxo| (utxo.slot_index, utxo.value_microecho, utxo.creation_id))
            .collect::<Vec<_>>();
        ...
        assert_eq!(
            (
                ...
                balance_after.spendable_microecho,
            ),
            (
                balance_before.balance_microecho,
                balance_before.utxo_count,
                balance_before.spendable_microecho,
            )
        );
        assert_eq!(
            handle
    ...
    fn reorg_installs_exact_snapshot_instead_of_replaying_on_old_cache() {
        ...
        let snapshot = VerifiedOwnerSnapshot {
            ...
            active_slot_count: 1,
            alloc_counter: 8,
            utxos: vec![echo_chain::storage::VerifiedOwnerUtxo {
                slot_index: 99,
                amount: 777,
                creation_id: 8,
            }],
    ...
    fn reorg_removes_receipts_bound_to_orphaned_blocks() {
        ...
        install_reorg_snapshot_and_artifacts(
            ...
            &std::collections::HashSet::new(),
            &std::collections::HashSet::new(),
            &[echo_poseidon2b::primitives::TxBodyHash(orphan_hash)],
            &[],
        )
        .unwrap();
    ...
    fn reorg_confirms_pending_send_by_logical_transaction_id() {
        ...
        let logical_txid = echo_chain::try_compute_logical_txids(&transactions).unwrap()[1].0;
        assert_ne!(logical_txid, physical_txid);
        let header = BlockHeader {
            prev_block_hash: [0; 32],
            state_root: [0; 32],
            tx_root: echo_chain::compute_tx_root(&transactions),
            timestamp: 123,
            height: 7,
            miner_address: Address([0x77; 32]),
            nonce: 0,
        ...
        install_reorg_snapshot_and_artifacts(
            ...
            &std::collections::HashSet::new(),
            &std::collections::HashSet::new(),
            &[echo_poseidon2b::primitives::TxBodyHash(logical_txid)],
            &[&block],
        )
        .unwrap();
    ...
    fn reorg_preserves_displaced_coinbase_as_mining_history() {
        ...
        install_reorg_snapshot_and_artifacts(
            ...
            &std::collections::HashSet::new(),
            &std::collections::HashSet::new(),
            &[echo_poseidon2b::primitives::TxBodyHash(coinbase_txid)],
            &[],
        )
        .unwrap();
    ...
    fn rejected_incremental_update_does_not_advance_snapshot_or_cache() {
        ...
        let mut outputs = [echo_tx::TxOutput::dummy(); echo_tx::TX_OUTPUTS];
        outputs[0] = echo_tx::TxOutput {
            slot_index: 9,
            amount: 50,
            owner,
        };
        let body = echo_tx::TxBody {
            epoch_anchor: [0; 32],
            fee: 0,
            input_owner: echo_poseidon2b::primitives::Address([0; 32]),
            inputs: [echo_tx::TxInput::dummy(); echo_tx::TX_INPUTS],
            outputs,
            validity_bitmap: echo_tx::output_bitmap_bit(0),
            is_coinbase: true,
        };
        let malformed = echo_chain::block::Block {
            ...
            transactions: vec![echo_tx::Transaction::new(body)],
        };
    ...
    fn canonical_send_selects_up_to_eight_inputs() {
        ...
        assert_eq!(plan.amount_microecho, amount);
    ...
    fn plan_keeps_small_payment_fee_at_baseline() {
        ...
        assert_eq!(plan.fee_microecho, 9_000);
        assert_eq!(plan.input_count, 1);
    ...
    fn automatic_fee_tracks_local_relay_floor_and_its_reset() {
        ...
        for floor in [5_000, 25_000, 90_000, 5_000] {
            ...
            assert_eq!(plan.fee_microecho, expected);
            ...
            assert_eq!(plan.total_spend_microecho, 100_000 + expected);
        }
    }
    ...
    fn automatic_fee_keeps_state_pressure_after_relay_floor_resets() {
        ...
        for (occupancy_bps, expected_fee) in [
            ...
            assert_eq!(plan.fee_microecho, expected_fee);
    ...
    fn automatic_fee_selects_enough_inputs_for_an_elevated_floor() {
        ...
        assert_eq!(plan.fee_microecho, 90_000);
    ...
    fn consolidation_fee_tracks_the_live_relay_floor() {
        ...
        for floor in [90_000, 5_000] {
            let plan = handle
                ...
                .unwrap();
            assert_eq!(plan.fee_microecho, floor.max(6_100));
            assert_eq!(plan.fee_breakdown.burned, 0);
        }
    }
    ...
    fn plan_charges_actual_io_for_five_input_send() {
        ...
        assert_eq!(plan.input_count, 5);
        assert_eq!(plan.output_count, 2);
        assert_eq!(plan.fee_microecho, 6_900);
