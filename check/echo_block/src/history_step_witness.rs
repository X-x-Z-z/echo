...
use echo_chain::consensus::validation::validate_block_checks_template;
use echo_chain::consensus::{
...
use echo_chain::exact_state_hash::{slot_leaf_hash, StateHash};
use echo_chain::sparse_merkle::{
    derive_structural_frontier_plan, evaluate_structural_frontier, SparseMerkleError,
};
use echo_chain::state::{ChainState, ExactFrontierError};
use echo_chain::{
...
use echo_core::{Block128, TowerField};
use echo_gkr::zk_authorization::ZkAuthorizationProof;
use echo_gkr::{spine_inputs_from_body, MerklePathInputs, SlotLeafInputs, MAX_MERKLE_DEPTH};
use echo_poseidon2b::native::compress;
use echo_recursive::acceptance::history_step::{
...
impl<const TIER: usize> PreparedHistoryStepWitness<TIER> {
    ...
    pub fn user_page_count(&self) -> usize {
        usize::from(
            echo_chain::validate_block_page_stream(&self.template.transactions)
    ...
    ) -> Result<(Block, echo_recursive::BuiltHistoryStep, ChainAccumulator), HistoryStepWitnessError>
    {
        let Self {
            template,
            parent_header,
        ...
        echo_chain::consensus::validation::validate_block_checks_template(
            &template,
            &parent_header,
            &previous_timestamps,
            &finalized_active_counts,
...
fn prepare_native_history_step<const TIER: usize>(
    ...
    let actual_tier =
        echo_chain::consensus::paged_spend::BlockProofClass::for_page_count(effective_pages)
...
fn validate_nonce_independent_block(
    ...
    if block.header.prev_block_hash != echo_chain::hash_block_header(context.parent_header)
        || context
            .start_accumulator
            .height
            .checked_add(1)
    ...
    echo_chain::consensus::checks::validate_block_slot_conflicts(&block.transactions)?;
    ...
    let parent_block_id = echo_chain::hash_block_header(context.parent_header);
    let user_epoch_anchor = if context
        .start_accumulator
        .height
        .is_multiple_of(echo_chain::consensus::params::TX_EPOCH_BLOCKS)
    {
        parent_block_id
    } else {
...
fn build_history_step_components(
    ...
    for (group_index, group) in stream.groups.iter().enumerate() {
        ...
        let public =
            echo_gkr::OwnerAuthPublicInputs::new(tx_body_hash, group.spend.input_owner.as_fields());
...
fn slot_leaf_input(slot: echo_chain::SlotValue) -> SlotLeafInputs {
    SlotLeafInputs {
...
fn tx_root_merkle_inputs(
    ...
    let leaf_capacity = echo_chain::tx_tree::TX_TREE_LEAVES;
    let depth = echo_chain::tx_tree::TX_TREE_DEPTH;
    ...
    if echo_chain::tx_tree::bind_tx_count(root, logical_hashes.len()) != block.header.tx_root {
        return Err(HistoryStepWitnessError::TransactionRootMismatch);
    }
...
mod tests {
    ...
    use echo_chain::consensus::template::build_block_template;
    ...
    fn expansion_frontier_binds_parent_as_left_child_of_the_new_domain() {
        use echo_chain::block_id;
        use echo_chain::consensus::fees::required_fee_for_tx_body;
        use echo_chain::exact_state_hash::{state_node_hash, zero_slot_roots};
    ...
    fn tx_root_paths_bind_every_real_body_to_the_count_wrapped_root() {
        ...
        assert_eq!(paths[0].active_depth, echo_chain::tx_tree::TX_TREE_DEPTH);
    }
}
