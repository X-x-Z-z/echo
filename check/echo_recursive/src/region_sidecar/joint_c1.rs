...
use echo_poseidon2b::native::permutation::N_ROUNDS;
    ...
    let walk = C1MultiDeepChainWalkProof {
        layers: (0..N_ROUNDS)
            .map(|_| C1MultiWalkLayerProof {
                round_coeffs: vec![[F256::ZERO; echo_ivc_core::deep_chain::WALK_DEGREE]; max_w_log],
...
pub(crate) fn prove_joint_c1_region_sidecar<Ch: Challenger>(
    ...
    let timing = std::env::var_os("ECHOH_C1_JOINT_TIMING").is_some();
