...
use echo_ivc_core::field::{F128, F256};
...
use echo_ivc_core::matrix_claim::c1::{C1FreshLincheckClaim, C1MatrixFoldProof};
use echo_ivc_core::pcs::{Commitment, PcsParams};
use echo_ivc_core::proof::{pcs_params_statement_bytes, C1FieldR1csProof, FieldShape};
use echo_ivc_core::public_io::PublicIoSpec;
use echo_ivc_core::verifier::{
    verify_field_c1_deferred_matrix_with_post_commit_context, VerifyError,
};
use echo_ivc_prover::field_prover::{
...
pub struct AuthorizationComponentInput {
    ...
    pub tx_index: usize,
    pub tx_body_hash: [echo_core::Block128; 2],
    pub public: echo_gkr::OwnerAuthPublicInputs,
}
...
pub struct ExactStateStructuralFrontierInputs {
    ...
    pub old_slot_leaves: Vec<echo_gkr::SlotLeafInputs>,
    pub new_slot_leaves: Vec<echo_gkr::SlotLeafInputs>,
    pub live_sibling_digests: Vec<echo_chain::exact_state_hash::StateHash>,
    pub old_combine_digests: Vec<echo_chain::exact_state_hash::StateHash>,
    pub new_combine_digests: Vec<echo_chain::exact_state_hash::StateHash>,
    pub old_root: echo_chain::exact_state_hash::StateHash,
    pub new_root: echo_chain::exact_state_hash::StateHash,
}
...
pub struct HistoryStepBlockComponents {
    ...
    pub tx_body_inputs: Vec<echo_gkr::SpineInputs>,
    pub tx_body_hashes: Vec<[echo_core::Block128; 2]>,
    pub tx_root_inputs: Vec<echo_gkr::MerklePathInputs>,
...
pub fn prepare_history_step_ghost_authorization(
    proof: echo_gkr::zk_authorization::ZkAuthorizationProof,
...
pub fn prepare_history_step_authorizations<const TIER: usize>(
    ...
    proofs: Vec<echo_gkr::zk_authorization::ZkAuthorizationProof>,
    ...
    if inputs.iter().any(|input| {
        input.public.layout != echo_gkr::OwnerAuthLayout::FIXED
    ...
    let statements = inputs
        .iter()
        .map(
            |input| echo_gkr::zk_authorization::ZkAuthCapsuleOwnerStatement {
...
impl<const TIER: usize> HistoryStepBlockInput<TIER> {
    pub fn try_new(
        ...
        let actual_tier = echo_chain::consensus::paged_spend::BlockProofClass::for_page_count(
        ...
        let semantic_id = echo_chain::block_header::semantic_header_id(sealed_header);
