...
use echo_chain::consensus::params::BLOCK_PAGE_CLASS_TIERS;
use echo_core::Block128;
use echo_ivc_core::challenger::{Challenger, FsLaneChallenger};
use echo_ivc_core::field::{F128, F256};
use echo_ivc_core::field_circuit::{f128_from_u128, f128_to_u128, FieldR1csBuilder, FsChannelOps};
use std::sync::Arc;

use echo_ivc_core::field_r1cs::{CompactFieldR1cs, FieldR1cs};
use echo_ivc_core::matrix_claim::c1::{
...
use echo_ivc_core::pcs::{PcsParams, BASEFOLD_RATE_QUARTER_C1_QUERIES, LOG_PACKING};
use echo_ivc_core::proof::{pcs_params_statement_bytes, FieldShape};
...
const _: () = assert!(
    HISTORY_STEP_CURRENT_CLASS_MS[0]
        == echo_chain::consensus::paged_spend::BlockProofClass::B25.outer_m()
        && HISTORY_STEP_CURRENT_CLASS_MS[1]
            == echo_chain::consensus::paged_spend::BlockProofClass::B255.outer_m(),
    "HistoryStep and consensus proof-class dimensions must match"
);

const HISTORY_STEP_BANK_POST_COMMIT_DOMAIN: &[u8] = b"ECHO/HISTORY-STEP/BANK-POST-COMMIT/V1";
const HISTORY_STEP_BANK_DIGEST_DOMAIN: &[u8] = b"ECHO/HISTORY-STEP/CLASS-BANK/V1";
pub(crate) const HISTORY_STEP_BANK_FOLD_TRANSCRIPT_DOMAIN: &[u8] = b"history-step-bank-fold-v1";
const HISTORY_STEP_BANK_FOLD_ROUTE_DOMAIN: &[u8] = b"ECHO/HISTORY-STEP/BANK-FOLD-ROUTE/V1";
...
pub fn canonical_history_step_shape(class_id: CanonicalHistoryStepClassId) -> FieldShape {
    ...
    FieldShape {
        m,
        k_log: m,
        k_skip: echo_ivc_core::zerocheck::K_SKIP,
        const_pin: Some(0),
    }
}
...
pub fn history_step_bank_block_accumulator(
    ...
    let lanes = parsed.block_accumulator.map(|lane| {
        ...
        Block128::from(echo_core::hardware::flat_to_tower_u128(flat))
    });
...
pub(crate) fn observe_history_step_bank_fold_route_trace(
    ...
    selected_parent_class: &echo_ivc_core::field_circuit::LinExpr,
) {
...
mod tests {
    ...
    use echo_ivc_core::field_circuit::{FsChannelOps, LinExpr};
    ...
    fn history_step_soundness_ledger_matches_basefold_target() {
        assert_eq!(
            echo_gkr::zk_auth_qrom::HISTORY_STEP_CLASSICAL_BITS,
            echo_ivc_core::pcs::BASEFOLD_UDR_TARGET_BITS
        );
        assert_eq!(echo_gkr::zk_auth_qrom::HISTORY_STEP_QROM_BITS, 83);
    }
    ...
    fn route_selection_requires_the_authenticated_parent_class() {
        ...
        let matrix = FieldR1cs {
            ...
            k_skip: 6,
            useful_rows: 1,
            a_0: echo_ivc_core::field_r1cs::SparseFieldMatrix::zero(k),
            b_0: echo_ivc_core::field_r1cs::SparseFieldMatrix::zero(k),
            const_pin: Some(0),
