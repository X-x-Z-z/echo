...
//! # lostecho-miner — External Poseidon2b PoW miner for the ParanO(1)d.
//!
//! Connects to any `lostecho` full node via JSON-RPC, fetches a block template,
...
//! lostecho-miner --rpc http://127.0.0.1:9601
//!
//! # Pool (remote node with bearer token)
//! lostecho-miner --rpc https://pool.example.com:9601 --key-file /secure/mining.key
//!
//! # Limit threads
//! lostecho-miner --rpc http://127.0.0.1:9601 --threads 4
//! ```
//!
//! ## Template protocol
//!
...
use echo_poseidon2b::batch::FixedFieldNonceBatch;
use echo_poseidon2b::native::domain::TAG_POWHDR;
use rayon::prelude::*;
...
fn mine(cli: &Cli, key: Option<String>) -> Result<()> {
    ...
    eprintln!(
        "lostecho-miner  rpc={}  threads={}  backend={}  poll={}ms",
        cli.rpc,
        threads,
        echo_core::cpu::selected_backend(),
        cli.poll_ms,
    );
    if authenticated {
...
fn main() {
    ...
    if let Err(error) = echo_core::cpu::ensure_production_hardware() {
...
mod tests {
    ...
    fn worker_accepts_legacy_and_file_credentials() {
        let legacy = Cli::try_parse_from(["lostecho-miner", "--key", "0123456789abcdef"]).unwrap();
        assert_eq!(legacy.key.as_deref(), Some("0123456789abcdef"));
        assert!(legacy.key_file.is_none());

        let file =
            Cli::try_parse_from(["lostecho-miner", "--key-file", "/secure/mining.key"]).unwrap();
