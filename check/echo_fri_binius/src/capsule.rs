...
//!
//! - **Rate 1/32** (`CAPSULE_LOG_RATE = 5`, decoupled from `echo_fri`'s
...
use echo_core::hardware::{flat_to_tower_u128, tower_to_flat_u128};
use echo_core::mle::eq::eq_ind_partial_eval;
use echo_core::mle::evaluate::evaluate_slice;
use echo_core::{AdditiveNTT, Block128, Block256, TowerField};
use echo_fri::hasher::{CryptographicHasher, HashOutput};
use echo_fri::Channel;
use echo_poseidon2b::native::compress_flat_feed_forward_with_tag;
use echo_poseidon2b::native::compression::Poseidon2bFlatSponge;
use echo_poseidon2b::native::domain::{TAG_CAPS256, TAG_CAPSLEAF, TAG_CAPSMIX, TAG_CAPSNODE};
...
/// log2 of the capsule code's inverse rate. Decoupled from `echo_fri`'s
...
/// `ECHO_CAPSULE_PROFILE` is set (same convention as the auth-PCS byte
/// profile). Zero cost when the variable is absent.
fn capsule_profile() -> bool {
    static ON: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *ON.get_or_init(|| std::env::var("ECHO_CAPSULE_PROFILE").is_ok())
}
...
impl CryptographicHasher for CapsuleNodeHasher {
    ...
    fn batch_compress(&self, pairs: &[HashOutput], out: &mut [HashOutput]) {
        echo_poseidon2b::batch::compress_flat_ff_batch_interleaved_with_tag_into(
            TAG_CAPSNODE,
            pairs,
            out,
        );
...
/// construction as `echo_fri::code::Code::new`, at the capsule's own rate.
