...
impl FsChallenger {
    ...
    fn with_profile(domain: &[u8], tag: echo_poseidon2b::native::DomainTag, c1: bool) -> Self {
        let mut c = Self {
...
fn proof_hash_has_leading_zero_bits(state_digest: &[u8; 32], nonce: u64, bits: u32) -> bool {
    ...
    let h: [u8; 32] = {
        ...
        poseidon2b_hash_byte_slices(b"ECHO/IVC/FS-POW", &[state_digest, &nonce_bytes])
    };
...
// `Poseidon2bPermutation` via the `echo_core::hardware` basis isomorphism.
...
pub fn capacity_iv_flat_lanes(tag: echo_poseidon2b::native::DomainTag) -> [F128; 2] {
    let [hi, lo] = capacity_iv(tag);
    [
        f128_of_u128(echo_core::hardware::tower_to_flat_u128(hi.0)),
        f128_of_u128(echo_core::hardware::tower_to_flat_u128(lo.0)),
    ]
}
...
pub fn ks_channel_iv_flat() -> [F128; 2] {
    capacity_iv_flat_lanes(echo_poseidon2b::native::TAG_KSCHANNL)
}
...
pub fn fs_pad_lane_flat() -> F128 {
    ...
    f128_of_u128(echo_core::hardware::tower_to_flat_u128(tower))
}
...
fn permute_flat(state: &mut [F128; 4]) {
    ...
    echo_poseidon2b::native::permutation::Poseidon2bPermutation.permute_mut(&mut tower);
    for i in 0..4 {
