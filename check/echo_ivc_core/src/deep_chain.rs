...
//!
//! Layer structure mirrors `echo_poseidon2b::native::permutation`
...
fn mul_unreduced_pair(a0: F128, b0: F128, a1: F128, b1: F128) -> [F256Unreduced; 2] {
    ...
    {
        if echo_core::cpu::avx2_vpclmul_available() {
...
pub fn prove_ragged_multi_deep_chain_walk<Ch: Challenger>(
    ...
    let timing_enabled = std::env::var_os("ECHOH_DEEP_CHAIN_TIMING").is_some();
    assert!(!s0_instances.is_empty(), "at least one walk instance");
    assert_eq!(
        s0_instances.len(),
        out_groups.len(),
    ...
    if timing_enabled {
        eprintln!(
            "ECHOH deep-chain-ragged widths={w_logs:?} state_init_ms={} setup_ms={} state_fetch_ms={} sumcheck_ms={} finish_ms={} total_ms={}",
...
mod tests {
    ...
    fn rounds_compose_to_the_native_permutation() {
        use echo_poseidon2b::native::permutation::permute_flat_u128;
    ...
    fn ragged_m22_three_child_micro_profile() {
        ...
        eprintln!(
            "ECHOH deep-chain-ragged-m22-micro elapsed_ms={} layers={} terminals={}",
