...
use echo_poseidon2b::primitives::TxBodyHash;
use echo_tx::unpack_amount_creation_id;
...
#[inline]
fn owner_key_from_fields(owner_hi: echo_core::Block128, owner_lo: echo_core::Block128) -> [u8; 32] {
    let mut key = [0u8; 32];
...
mod tests {
    ...
    fn commit_stateful_test_genesis(store: &MdbxStore) -> (ChainState, BlockHeader, [u8; 32]) {
        ...
        assert_eq!(
            ...
            Some(state.circulating_supply_microecho)
        );
        (state, header, hash)
    }
    ...
    fn missing_restart_summaries_are_rebuilt_once_from_dense_state() {
        ...
        assert_eq!(
            restored.circulating_supply_microecho,
            state.circulating_supply_microecho
        );
        assert_eq!(store.segment_summaries().unwrap().len(), 1);
    }
    ...
    fn compact_restart_defers_raw_segment_check_until_hydration() {
        ...
        tampered.values[7] = echo_tx::pack_amount_creation_id(12, 1);
        let txn = store.db.begin_rw_txn().unwrap();
