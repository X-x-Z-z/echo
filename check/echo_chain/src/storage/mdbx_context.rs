...
///
/// `echo_chain` resolves the canonical headers the native verifier must bind
...
impl MdbxChainContext {
    ...
    fn load_dense_chain_state(
        ...
        let mut circulating_supply_microecho = 0u128;
        store.visit_segments(|segment_id, stored_log, columns| {
            ...
            for local in 0..expected_segment_len {
                ...
                circulating_supply_microecho = circulating_supply_microecho
    ...
    pub(crate) fn load_streamed_chain_state(
        ...
        active_slot_count: u64,
        alloc_counter: u64,
        persisted_circulating_supply_microecho: Option<u128>,
        tip_height: u64,
        expected_root: [u8; 32],
    ) -> Result<ChainState, MdbxContextError> {
        if let Some(circulating_supply_microecho) = persisted_circulating_supply_microecho {
    ...
    fn persist_genesis(&self) -> Result<(), MdbxContextError> {
        ...
        self.store.commit_block(
            ...
            self.state.circulating_supply_microecho,
            &meta,
            false,
        )?;
        Ok(())
    ...
    fn restore_from_mdbx(store: MdbxStore) -> Result<Self, MdbxContextError> {
        ...
        let circulating_supply_microecho = store.get_circulating_supply()?;
    ...
    fn reload_hot_state_from_mdbx(&mut self) -> Result<(), MdbxContextError> {
        ...
        let (log_slots, active_slot_count, alloc_counter) = self
            ...
            .get_state_meta()?
            .ok_or(MdbxContextError::Corrupt("missing state_meta"))?;
        let circulating_supply_microecho = self.store.get_circulating_supply()?;
        let tip_header = self
            .store
    ...
    fn rollback_uncommitted_block(
        ...
        self.state.circulating_supply_microecho = circulating_supply_microecho;
...
mod tests {
    ...
    fn test_next_bundle_for_miner(
        ...
        let (template, _) = crate::consensus::template::build_node_owned_block_template(
            ...
            &finalized_active_counts,
            Vec::new(),
            echo_poseidon2b::primitives::Address([miner_byte; 32]),
            timestamp,
            target,
        )
        .unwrap();
    ...
    fn test_context_with_log(path: &Path, log_slots: u32) -> MdbxChainContext {
        ...
        let header = BlockHeader {
            ...
            timestamp: 1_000,
            height: 0,
            miner_address: echo_poseidon2b::primitives::Address([0u8; 32]),
            nonce: 0,
        ...
        store
            .commit_block(
                ...
                &[],
                None,
                state.circulating_supply_microecho,
                &ConsensusMeta {
                    tip_height: 0,
                    tip_hash: hash,
    ...
    fn unsafe_claimed_coinbase_with_impossible_pow(
        ...
        let (template, prepared) = crate::consensus::template::build_node_owned_block_template(
            ...
            vec![],
            echo_poseidon2b::primitives::Address([0x44; 32]),
    ...
    fn occupancy_header(height: u64, active_slot_count: u64) -> BlockHeader {
        BlockHeader {
            ...
            height,
            miner_address: echo_poseidon2b::primitives::Address([0x44; 32]),
            nonce: 0,
            difficulty_target: [0xff; 32],
            log_slots: 8,
            active_slot_count,
