...
use echo_tx::wire::WireError;
use echo_tx::{PagedSpendError, Transaction, TxPage};
...
pub(crate) fn apply_state_delta(
    ...
    let mut circulating_supply_microecho = snap.circulating_supply_microecho;
    ...
    snap.circulating_supply_microecho = circulating_supply_microecho;
...
pub fn try_compute_logical_txids(
    txs: &[Transaction],
) -> Result<Vec<echo_poseidon2b::primitives::TxBodyHash>, BlockPageStreamError> {
    if txs.is_empty() {
        return Ok(Vec::new());
    }
...
pub fn canonical_block_wire_len(tx_count: usize) -> Result<usize, WireError> {
    ...
    tx_count
        .checked_mul(echo_tx::wire::TRANSACTION_WIRE_SIZE)
