...
const _: () = assert!(
    TX_EPOCH_BLOCKS == echo_tx::TX_EPOCH_BLOCKS,
    "echo_chain TX_EPOCH_BLOCKS must equal echo_tx"
);
...
/// Precision: 1 ECHO = 1_000_000 μECHO (microECHO).
pub const MICROECHO_PER_ECHO: u64 = 1_000_000;

/// Starting block reward at zero occupancy: 50 ECHO.
pub const BASE_REWARD_MICROECHO: u64 = 50 * MICROECHO_PER_ECHO;

/// Reward floor: 1 ECHO forever.
pub const FLOOR_REWARD_MICROECHO: u64 = MICROECHO_PER_ECHO;
...
// Slot allocator PRNG
// ---------------------------------------------------------------------------
// splitmix64 constants are embedded in echo_chain::consensus::allocator.
...
/// Base minimum fee in μECHO per non-coinbase transaction.
pub const MIN_FEE_BASE: u64 = 5_000; // 0.005 ECHO
...
pub const FEE_PER_INPUT: u64 = 100; // 0.0001 ECHO per input
...
/// at the historical 9_000 μECHO baseline together with state-growth burn.
pub const FEE_PER_OUTPUT: u64 = 700; // 0.0007 ECHO per output

/// Base fee charged per net-new live UTXO slot at low occupancy.
/// This state-growth component is burned by consensus.
pub const STATE_GROWTH_FEE_BASE: u64 = 2_500; // 0.0025 ECHO per net-new slot
