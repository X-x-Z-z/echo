...
//! This is Lostecho's block-withholding protection.

use std::collections::{BTreeSet, HashSet};

use echo_poseidon2b::primitives::{Address, Digest};
use echo_tx::{PagedSpendFacts, Transaction};
...
fn build_block_template_with_post_state(
    ...
    use crate::consensus::development_allocation::{
        development_allocation, O1_NETWORK_FUND_ADDRESS, LOSTECHO_LAB_ADDRESS,
    };
    use crate::consensus::expected_child_log_slots;
    use crate::consensus::fees::fee_breakdown;
    use echo_tx::{TxBody, TxInput, TxOutput, TX_INPUTS};
        ...
        Transaction::new(TxBody {
            ...
            outputs: [
                ...
                TxOutput {
                    slot_index: lab_slot,
                    amount,
                    owner: LOSTECHO_LAB_ADDRESS,
                },
            ],
            validity_bitmap: echo_tx::output_bitmap_bit(0) | echo_tx::output_bitmap_bit(1),
            is_coinbase: true,
        })
    });
...
mod tests {
    ...
    fn due_template_builds_the_mandatory_two_recipient_payout() {
        use crate::consensus::development_allocation::{
            development_share_each, miner_subsidy, O1_NETWORK_FUND_ADDRESS, LOSTECHO_LAB_ADDRESS,
        ...
        assert_eq!(payout.body.outputs[1].owner, LOSTECHO_LAB_ADDRESS);
    ...
    fn payout_and_user_keep_distinct_anchors_at_epoch_boundary() {
        ...
        state.circulating_supply_microecho = 1_000_000;
        let mut parent = parent(&mut state);
        parent.height = TARGET_BLOCKS_PER_DAY - 1;
    ...
    fn mint_preference_cannot_block_a_payout_with_only_three_free_slots() {
        ...
        state.circulating_supply_microecho = 25_300_000;
        let mut parent = parent(&mut state);
        parent.height = TARGET_BLOCKS_PER_DAY - 1;
    ...
    fn mint_preference_keeps_b25_b255_users_and_their_slots_unchanged() {
        ...
        for (count, payout) in [(25u32, false), (255, false), (24, true), (255, true)] {
            ...
            state.circulating_supply_microecho = u128::from(count) * 1_000_000;
            let mut parent = parent(&mut state);
            if payout {
                parent.height = TARGET_BLOCKS_PER_DAY - 1;
            }
            ...
            for all_soft in [false, true] {
                ...
                assert_eq!(
                    replay.circulating_supply_microecho,
    ...
    fn mint_preference_latency_benchmark() {
        ...
        state.circulating_supply_microecho = 255_000_000;
        let parent = parent(&mut state);
        let seed =
    ...
    fn expansion_block_keeps_parent_mempool_and_grows_exact_domain_once() {
        ...
        state.circulating_supply_microecho = occupied
            .iter()
    ...
    fn candidate_stream_rejects_same_block_slot_chains() {
        ...
        state.alloc_counter = 1;
        state.circulating_supply_microecho = 2_000_000;
        let parent = parent(&mut state);
