...
//! LostechoReceipt — unforgeable proof of transaction inclusion.
//!
...
//!   Must match `echo_chain::block::compute_tx_root` exactly.
...
use echo_tx::{validate_paged_spend, PagedSpendIntent, Transaction, TxPage};
...
impl LostechoReceipt {
    /// Serialize to compact bincode bytes.
    pub fn to_bytes(&self) -> Vec<u8> {
        bincode::serialize(self).expect("LostechoReceipt serialize")
    }
...
///
/// Uses Poseidon2b COMPRESS to match `compute_tx_root` in `echo_chain::block`.
pub fn verify_merkle_inclusion(receipt: &LostechoReceipt) -> bool {
...
pub fn verify_against_header(receipt: &LostechoReceipt, canonical_header: &BlockHeader) -> bool {
...
/// Mirrors `compute_tx_root` in `echo_chain::block`.
...
mod tests {
    ...
    fn all_255_user_positions_have_authenticated_fixed_paths() {
        ...
        for (index, transaction) in groups.iter().enumerate() {
            ...
            assert_eq!(
                LostechoReceipt::from_bytes(&receipt.to_bytes())
                    .unwrap()
                    .paged_spend,
                receipt.paged_spend
            );
