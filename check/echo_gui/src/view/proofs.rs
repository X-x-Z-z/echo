...
use crate::model::{
    format_microecho, grouped, ProofsTab, ReceiptSnapshot, ReceiptSummarySnapshot,
...
fn receipt_row(
    ...
    button(
        column![
            row![
                text(format!("{} ↻", format_microecho(receipt.amount_microecho)))
                    .size(13)
                    .color(theme::ACCENT),
                Space::new().width(Length::Fill),
    ...
    let metrics: Element<'_, Message> = if compact {
        column![
            ...
            row![
                metric(
                    "FEE / ECHO",
                    format_microecho(summary.fee_microecho),
                    theme::WARNING,
                ),
                metric(
                    "CONFIRMED",
            ...
            container(
                column![
                    row![
                        ...
                        text(format!(
                            "{} ↻{}",
                            format_microecho(output.amount_microecho),
