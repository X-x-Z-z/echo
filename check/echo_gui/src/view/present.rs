...
use crate::model::{
    format_compact_count, format_compact_difficulty, format_compact_microecho,
    format_creation_origin, format_hashrate, format_microecho, format_microecho_trimmed,
...
pub fn system_meters(app: &App, compact: bool) -> Element<'_, Message> {
    ...
    let economy_status: Element<'_, Message> = column![
        telemetry_value(
            ...
            format!(
                "{} ECHO",
                format_compact_microecho(network.circulating_supply_microecho)
            ),
            theme::ACCENT,
        ),
        telemetry_value(
            ...
            format!(
                "{} ECHO",
                format_microecho_trimmed(network.block_reward_microecho)
            ),
            theme::ACCENT,
        ),
        telemetry_value(
...
fn active_owner(app: &App, compact: bool) -> Element<'_, Message> {
    ...
    let metrics = row![
        ...
        amount_stat(
            ...
            if address.pending_outbound_microecho == 0 {
                theme::DIM
            } else {
                theme::WARNING
            },
        ...
        amount_stat(
            "INCOMING",
            address.incoming(),
            if address.incoming_microecho == 0 {
                theme::DIM
            } else {
                theme::ACCENT
            },
...
fn utxo_table(app: &App) -> container::Container<'_, Message> {
    ...
    let header = table_columns("SLOT", "VALUE / ECHO", "ORIGIN", "SEGMENT", "STATUS");
    ...
    let selection: Element<'_, Message> = if let Some(utxo) = selected_utxo {
        ...
        let (mine_count, mine_value) = app
            ...
            .fold((0usize, 0u64), |(count, total), utxo| {
                (count + 1, total.saturating_add(utxo.value_microecho))
            });
        ...
        column![
            ...
            row![
                ...
                state_detail("VALUE", format_microecho(mine_value), theme::ACCENT),
            ]
            .spacing(10)
            .align_y(Alignment::Center),
        ]
    ...
    column![
        form_line(
            ...
            format!(
                ...
                plan.input_count,
                original_count,
                format_microecho(plan.input_value_microecho)
            ),
            compact,
        ),
        form_line(
            "NETWORK FEE",
            format!("{} ECHO", format_microecho(plan.fee_microecho)),
            compact,
        ),
        form_line(
            "NEW OUTPUT",
            format!("1 · {} ECHO", format_microecho(plan.output_value_microecho)),
            compact,
        ),
        form_line(
            "STATE",
        ...
        container(
            column![
                ...
                row![
                    text(format_microecho(plan.balance_before_microecho))
                        .size(16)
                        .color(theme::TEXT),
                    text("↻")
                        .size(16)
                    ...
                    text(format_microecho(plan.balance_after_microecho))
                        .size(16)
                        .color(theme::TEXT),
                    text("↻")
                        .size(16)
...
fn consolidation_success<'a>(
    ...
    let metrics = row![
        ...
        send_result_metric(
            "FEE",
            format!("{} ↻", format_microecho(result.fee_microecho)),
        ),
    ...
    column![
        ...
        form_line(
            "NEW OUTPUT",
            format!("{} ECHO", format_microecho(result.output_value_microecho)),
            compact,
        ),
        form_line(
            "INPUT VALUE",
            format!(
                "{} ECHO selected",
                format_microecho(result.input_value_microecho),
            ),
            compact,
        ),
...
fn send_form(app: &App, compact: bool) -> Element<'_, Message> {
    ...
    let spendable = address
        .balance_microecho
        .saturating_sub(address.pending_outbound_microecho);
...
fn send_success<'a>(
    ...
    let metrics = row![
        send_result_metric(
            "AMOUNT",
            format!("{} ↻", format_microecho(result.amount_microecho)),
        ),
        send_result_metric(
            "FEE",
            format!("{} ↻", format_microecho(result.fee_microecho)),
