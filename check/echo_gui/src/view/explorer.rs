...
use crate::model::{
    format_creation_origin, format_microecho, grouped, ExplorerAddressSnapshot,
    ...
    let rows: Element<'a, Message> = if recent.transactions.is_empty() {
        ...
        let header = container(
            row![
                ...
                table_cell("FEE / ECHO", 3, theme::MUTED),
                text("OPEN")
                    .size(13)
                    .color(theme::MUTED)
...
fn transaction_row<'a>(
    ...
    let spent = if address_mode {
        transaction
            .address_spent_microecho
            .as_deref()
            .map(format_microecho_string)
    ...
    container(
        row![
            ...
            table_cell(
                format_microecho(transaction.fee_microecho),
                3,
                theme::WARNING,
            ),
            open,
...
fn address_result<'a>(
    ...
    let balance = metric(
        "BALANCE",
        format!("{} ↻", format_u128_microecho(address.balance_microecho)),
        theme::ACCENT,
    );
    ...
    let rows: Element<'_, Message> = if slots.is_empty() {
            ...
            list = list.push(
                container(
                    row![
                        ...
                        text(format!(
                            "{} ↻",
                            crate::model::format_microecho(slot.value_microecho)
                        ))
                        .size(13)
                        .color(theme::ACCENT),
                    ]
        ...
        let header = container(
            row![
                ...
                table_cell("VALUE / ECHO", 4, theme::MUTED),
...
fn slot_result<'a>(
    ...
    let value_metric = metric(
        "VALUE",
        format!("{} ↻", crate::model::format_microecho(slot.value_microecho)),
        state.1,
    );
...
fn format_u128_microecho(value: u128) -> String {
