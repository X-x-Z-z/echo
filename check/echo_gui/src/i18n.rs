...
fn translate_localized(language: Language, source: &str) -> String {
    ...
    if let Some(value) = source.strip_suffix(" ECHO selected") {
        return match language {
            Language::Russian => format!("ВЫБРАНО {value} ECHO"),
            Language::Chinese => format!("已选择 {value} ECHO"),
            Language::English => source.to_owned(),
        };
    }
...
fn exact_translation(source: &str) -> Option<(&'static str, &'static str)> {
    Some(match source {
        ...
        "SWITCH" => ("СМЕНИТЬ", "切换"),
        "ECHO BALANCE" => ("БАЛАНС ECHO", "ECHO 余额"),
        ...
        "AMOUNT / ECHO" => ("СУММА / ECHO", "金额 / ECHO"),
        ...
        "FEE / ECHO" => ("КОМИССИЯ / ECHO", "手续费 / ECHO"),
        "FEE / μECHO" => ("КОМИССИЯ / μECHO", "手续费 / μECHO"),
        ...
        "Amount must be a positive ECHO value." => (
            "Сумма ECHO должна быть положительной.",
            "ECHO 金额必须大于零。",
        ),
        "Use a decimal ECHO amount, for example 12.500000." => (
            "Введите десятичную сумму ECHO, например 12.500000.",
            "请输入十进制 ECHO 金额，例如 12.500000。",
        ),
        "ECHO supports at most 6 decimal places." => (
            "ECHO поддерживает не более 6 знаков после запятой.",
            "ECHO 最多支持 6 位小数。",
        ),
        "Amount is too large." => ("Сумма слишком велика.", "金额过大。"),
        "Amount must be at least 0.000001 ECHO." => (
            "Минимальная сумма — 0.000001 ECHO.",
            "金额不得低于 0.000001 ECHO。",
        ),
        ...
        "Written locally to lostecho-node.log" => (
            "Записываются локально в lostecho-node.log",
            "在本机写入 lostecho-node.log",
        ),
        "ERROR" => ("ОШИБКИ", "错误"),
        "WARN" => ("ПРЕДУПР.", "警告"),
        "INFO" => ("ИНФО", "信息"),
        ...
        "No lostecho-node.log output yet…" => (
            "В lostecho-node.log пока нет данных…",
            "lostecho-node.log 暂无输出…",
        ),
