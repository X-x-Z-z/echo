...
use echo_chain::consensus::{pow::block_id, tx_epoch_anchor_height_for_child};
use echo_chain::fri_state::SlotValue;
use echo_chain::segmented_state::SegmentedFriState;
use echo_chain::storage::MdbxStore;
...
struct SegmentReadCache {
    entries: VecDeque<(u16, Arc<echo_chain::segmented_state::SegmentColumns>)>,
}
...
impl ChainView {
    ...
    pub fn from_mdbx(ctx: &echo_chain::storage::MdbxChainContext) -> Self {
...
mod tests {
    ...
    fn next_child_anchor_is_exact_at_143_144_145_boundary() {
        let genesis = echo_chain::consensus::genesis::genesis_header();
        ...
        let view = |tip_height| {
            ChainView::new(
                ...
                headers.clone(),
                0,
                echo_chain::state::ChainState::with_log_slots(8).state,
            )
        };
