...
//! Transaction builder for the Lostecho wallet.
//!
...
//!
use echo_gkr::OwnerAuthWitness;
use echo_poseidon2b::primitives::{derive_address, Address};
use echo_rpc::types::WALLET_CONSOLIDATION_INPUT_LIMIT;
use echo_tx::{
    output_bitmap_bit,
...
pub enum BuildError {
    #[error("insufficient funds: need {need} μECHO, have {have} μECHO")]
    ...
    #[error(
        "consolidation value mismatch: selected inputs total {selected_total} μECHO, expected output+fee {expected_total} μECHO"
    )]
...
///   `amount_microecho + fee_microecho`.
...
pub fn extract_build_data(
    ...
    let total_needed = amount_microecho
        .checked_add(fee_microecho)
        .ok_or(BuildError::AmountOverflow)?;

    // Coin selection — largest-first, returns (selected, change_amount).
    let (selected_refs, change_amount) = match wallet.select_utxos(amount_microecho, fee_microecho)
    {
...
pub fn extract_consolidation_build_data(
    ...
    let selected_total = selected_utxos
        ...
        .ok_or(BuildError::AmountOverflow)?;
    let expected_total = output_value_microecho
        .checked_add(fee_microecho)
        .ok_or(BuildError::AmountOverflow)?;
...
pub fn build_and_prove_tx(
    ...
    let change_amount = total_selected
        .checked_sub(amount_microecho)
        .and_then(|remaining| remaining.checked_sub(fee_microecho))
        .ok_or(BuildError::InsufficientFunds {
            need: amount_microecho
                .checked_add(fee_microecho)
                .ok_or(BuildError::AmountOverflow)?,
            have: total_selected,
        })?;
    ...
    for page_index in 0..page_count {
        ...
        pages.push(
            TxPage::new(TxBody {
                ...
                fee: if page_index == 0 { fee_microecho } else { 0 },
...
mod tests {
    ...
    fn multi_page_builder_keeps_secret_only_in_owner_witness() {
        ...
        let bundle = echo_gkr::WalletAuthorizationBundle::from_bytes(&intent.authorization_bytes)
            .expect("decode standard authorization bundle");
        echo_gkr::verify_paged_spend_authorization(&intent.pages, &bundle)
