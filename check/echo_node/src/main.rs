...
//! # lostecho — ParanO(1)d Full Node Binary
//!
//! Startup sequence:
...
use echo_chain::consensus::wire_limits::MAX_TX_INTENT_BYTES_GLOBAL;
use echo_chain::consensus::NetworkConfig;
use echo_chain::storage::snapshot_staging::{
    AuthenticatedSnapshotMetadata, FinalizedSnapshotStaging, SnapshotStagingError,
    SnapshotStagingSession,
};
use echo_chain::storage::{MdbxChainContext, MdbxStore, SnapshotSegmentDescriptor};
use echo_mempool::{AsyncMempool, ChainView, MempoolConfig};
use echo_miner::{BlockMiner, MinerConfig};
use echo_node::snapshot_header_staging::{
...
use echo_p2p::{NetworkEvent, P2PNetwork};
use echo_rpc::types::NodeSyncStage;
use echo_rpc::{start_rpc_server, ExternalMiningAttemptInvalidator, WalletOperationGate};

struct AppliedCompactSuffix {
    height: u64,
    block_hash: [u8; 32],
    confirmed_tx_hashes: Vec<echo_poseidon2b::primitives::TxBodyHash>,
    view: ChainView,
    applied_blocks: u64,
    payload_bytes: u64,
...
struct AppliedReorg {
    result: echo_chain::consensus::ReorgResult,
    confirmed_tx_hashes: Vec<echo_poseidon2b::primitives::TxBodyHash>,
    view: ChainView,
}
...
struct PostSnapshotTail {
    boundary: echo_node::networking::ChainPoint,
    target: Option<echo_node::networking::ChainPoint>,
}
...
impl PostSnapshotTail {
    fn serving_limit(self) -> u64 {
        self.boundary
            .height
            .saturating_add(echo_chain::consensus::params::RETAINED_BLOCK_SERVING_DEPTH)
    }
    ...
    fn pin_selected_target(
        &mut self,
        local: echo_node::networking::ChainPoint,
        selected: echo_node::networking::ChainPoint,
    ) -> bool {
...
fn newest_deterministic_snapshot_boundary(peer_height: u64) -> u64 {
    let finalized_height =
        peer_height.saturating_sub(echo_chain::consensus::params::CONSENSUS_FINALITY_DEPTH);
    finalized_height - finalized_height % echo_p2p::protocol::SNAPSHOT_BOUNDARY_INTERVAL
}
...
fn canonical_tip_supersedes_snapshot_boundary(
    ...
    canonical_boundary: Option<echo_node::networking::ChainPoint>,
    snapshot_boundary: echo_node::networking::ChainPoint,
) -> bool {
...
fn validate_snapshot_install_selection(
    header_dag: &echo_node::networking::header_dag::HeaderDag,
    manifest: &echo_p2p::protocol::GetStateManifestResponse,
    ...
    if manifest.tip_height > selected_tip.height {
        ...
        if !matches!(
            echo_chain::choose_chain_by_work(
                &manifest.cumulative_chainwork,
                &manifest.tip_hash,
                &selected_work,
                &selected_tip.hash,
            ),
            echo_chain::consensus::fork_choice::ChainChoice::A
        ) {
...
fn validate_rebase_snapshot_selection(
    ...
    use echo_node::networking::ChainPoint;
...
fn try_dispatch_header_fetch(
    p2p_cmd: &echo_p2p::NetworkCommandSender,
    ...
    if p2p_cmd
        .try_send(echo_p2p::NetworkCommand::FetchHeaders {
            peer,
            start_height,
            count,
        })
...
fn finalized_header_search_floor(local_height: u64) -> u64 {
    local_height.saturating_sub(echo_chain::consensus::params::CONSENSUS_FINALITY_DEPTH)
}
...
fn unresolved_selected_tip_probe_range(
    dag: &echo_node::networking::header_dag::HeaderDag,
    committed_tip: echo_node::networking::ChainPoint,
    forward_headers: u16,
) -> Result<(u64, u16), echo_node::networking::header_dag::HeaderDagError> {
...
const CONNECTED_TIP_PROBE_HEADERS: u16 = (echo_chain::consensus::params::CONSENSUS_FINALITY_DEPTH
    + echo_p2p::protocol::SNAPSHOT_BOUNDARY_INTERVAL
    + 1) as u16;
...
struct MiningPeerQuorum {
    ...
    readiness: echo_node::networking::mining_readiness::MiningReadiness,
    connected: std::collections::HashMap<libp2p::PeerId, echo_node::networking::FailureDomain>,
...
impl MiningPeerQuorum {
    fn new(
        ...
        let initial = echo_node::networking::ChainPoint::new(0, [0u8; 32]);
        let quorum = Self {
            isolated,
            origin,
            initial_sync_complete: isolated,
            unresolved_better_header: false,
            readiness: echo_node::networking::mining_readiness::MiningReadiness::new(
                isolated,
                MINING_PEER_QUORUM,
                initial,
            ),
    ...
    fn connect(
        &mut self,
        peer: libp2p::PeerId,
        failure_domain: echo_node::networking::FailureDomain,
    ) {
    ...
    fn set_canonical_tip_state(
        ...
        let tip = echo_node::networking::ChainPoint::new(height, hash);
    ...
    fn reconcile_canonical_tip(&mut self, height: u64, hash: [u8; 32], prev_hash: [u8; 32]) {
        ...
        let current = echo_node::networking::ChainPoint::new(height, hash);
        if previous == current {
            return;
        }
    ...
    fn confirm_tip_at(
        ...
        let point = echo_node::networking::ChainPoint::new(height, hash);
        let Some(failure_domain) = self.connected.get(&peer).copied() else {
            return;
        };
...
fn state_manifest_candidate_is_preferred(
    candidate: &echo_p2p::protocol::GetStateManifestResponse,
    current: &echo_p2p::protocol::GetStateManifestResponse,
) -> bool {
    matches!(
        echo_chain::consensus::fork_choice::choose_chain_by_work(
...
fn terminal_transport_can_retry_same_peer(kind: echo_p2p::RequestFailureKind) -> bool {
    matches!(
        kind,
        echo_p2p::RequestFailureKind::Timeout | echo_p2p::RequestFailureKind::Io
    )
}
...
fn embedded_history_step_cache_file(
    ...
    Some(
        history_step_cache_directory(data_dir, pack.runtime_metadata_digest()).join(
            echo_miner::history_step_runtime_image_file_name(class.class_id()),
        ),
    )
}
...
fn embedded_history_step_runtime(
    data_dir: &Path,
) -> Result<Option<Arc<echo_recursive::acceptance::history_step::HistoryStepRuntime>>, String> {
    let Some(pack) = embedded_history_step_pack::embedded_history_step_pack() else {
        return Ok(None);
    };
    let metadata = echo_miner::decode_history_step_runtime_metadata_pinned(
    ...
    let runtime = echo_recursive::acceptance::history_step::HistoryStepRuntime::new(
        bank,
...
fn prepare_history_step_ghost_authorization() -> Result<
    Arc<echo_recursive::acceptance::history_step::PreparedHistoryStepGhostAuthorization>,
    String,
> {
    echo_miner::install_history_step_phase_cpu(|| {
        let proof = echo_gkr::ghost_tx::prove_selected_ghost_authorization()
            .map_err(|error| format!("canonical ghost authorization proof failed: {error}"))?;
        echo_recursive::acceptance::history_step::prepare_history_step_ghost_authorization(proof)
            .map(Arc::new)
...
impl HistoryStepCacheClass {
    fn class_id(self) -> echo_recursive::CanonicalHistoryStepClassId {
        echo_recursive::CanonicalHistoryStepClassId::new(match self {
            Self::B25 => 0,
            Self::B255 => 1,
        })
...
#[command(
    ...
    long_about = "Run a ParanO(1)d node and wallet.\n\nExample:\n  lostecho --miner --data-dir ~/.lostecho/data\n  lostecho --p2p-listen 0.0.0.0:9600 --seed 1.2.3.4:9600",
)]
struct Cli {
    /// Path to TOML config file. A missing file is created with safe defaults.
    /// Default: ~/.lostecho/lostecho.toml
    ...
    ///
    /// Pool example:
    ///   lostecho --mode extminer --mining-key-file ~/.lostecho/mining.key
...
const NETWORK_STORAGE_SCHEMA: &[u8] = b"lostecho/mainnet/network-storage/v1/";

fn network_storage_epoch_bytes() -> Vec<u8> {
    let genesis = echo_chain::consensus::genesis_header();
    let genesis_id = echo_chain::block_header::block_id(&genesis);
...
fn ensure_network_storage_epoch(data_dir: &Path, explicit_purge: bool) -> anyhow::Result<bool> {
    ...
    let mainnet_genesis = echo_chain::consensus::genesis_header();
...
fn validate_isolated_fork_test(cli: &Cli) -> anyhow::Result<()> {
    if !echo_chain::consensus::params::ISOLATED_V1_1_TESTNET {
        return Ok(());
    }
    anyhow::ensure!(
...
async fn main() -> anyhow::Result<()> {
    ...
    if cli.check_hardware {
        let report = echo_core::cpu::ProductionHardwareReport::detect();
        print!("{report}");
        if report.ready() {
            return Ok(());
        }
    ...
    let production_hardware = echo_core::cpu::ensure_production_hardware()?;
    ...
    let config_path = cli
        .config
        .unwrap_or_else(|| expand_tilde(&PathBuf::from("~/.lostecho/lostecho.toml")));
    ...
    let data_dir = if cfg.storage.path == Path::new("~/.lostecho/data") {
        expand_tilde(Path::new("~/.lostecho/data"))
    } else {
    ...
    let cpu_budget_mode = if cli.mode != NodeMode::Node {
        echo_miner::ProcessCpuBudgetMode::InternalMiner
    } else {
        echo_miner::ProcessCpuBudgetMode::ProofOnly
    };
    let cpu_plan = echo_miner::configure_process_cpu_budget_with_threads(
        cpu_budget_mode,
    ...
    let initial_canonical_tip = {
        ...
        echo_p2p::object_protocol::ChainPoint::new(ctx.tip_height(), ctx.tip_hash())
    };
    ...
    let authorization_verification_executor: echo_mempool::AuthorizationVerificationExecutor =
        Arc::new(|task: echo_mempool::AuthorizationVerificationTask| {
            echo_miner::install_inbound_verifier_cpu(task).map_err(|error| {
    ...
    // --- P2P Network ---
    let topics = echo_p2p::protocol::NetworkTopics::for_network_cfg(&net);
    let p2p_background_capacity = if cli.mode == NodeMode::Node {
        echo_p2p::BackgroundCapacity::Full
    } else {
        echo_p2p::BackgroundCapacity::MiningReserved
    };
    ...
    tokio::spawn(async move {
        loop {
            match mp_events.recv().await {
                Ok(echo_mempool::MempoolEvent::TxAdmitted {
                    hash, intent_bytes, ..
                }) => {
                    let _ = p2p_tx_relay
                        .send(echo_p2p::NetworkCommand::BroadcastTx {
                            intent_bytes,
                            tx_hash: Some(hash.0),
                        })
                        .await;
    ...
    let (rpc_handle, rpc_stop_rx) = start_rpc_server(
        ...
        cli.genesis,
        cfg.mining.enabled,
        echo_core::cpu::selected_backend().to_string(),
    ...
    let miner_handle = if cfg.mining.enabled {
        ...
        let miner_addr = if cfg.mining.miner_address.is_empty() {
            ...
            guard
                ...
                .unwrap_or(echo_poseidon2b::primitives::Address([0u8; 32]))
        } else {
        ...
        // Remote wallets use P2P block subscription independently.
        {
            ...
            miner.set_block_applied_hook(std::sync::Arc::new(move |block| {
                ...
                hook_canonical_tip_changes.send_replace(
                    echo_p2p::object_protocol::ChainPoint::new(
                        block.header.height,
                        echo_chain::block_id(&block.header),
                    ),
                );
            }));
        }
        ...
        tokio::spawn(async move {
            loop {
                match miner_rx.recv().await {
                    Ok(echo_miner::MinerEvent::BlockFound {
                        bundle,
                        height,
                        hash,
                        n_txs,
                        ...
                        let mut command = echo_p2p::NetworkCommand::AnnounceBlock { bundle };
                        loop {
                    ...
                    Ok(echo_miner::MinerEvent::ProveFailed { height, error }) => {
    ...
    // --- Startup Banner ---
    {
        ...
        use echo_chain::fri_state::LOG_SEGMENT_SIZE;
    ...
    // --- Shutdown ---
    // Wait for either Ctrl-C or a `lostecho_stop` RPC call.
...
fn retained_terminal_capability(
    records: &[echo_p2p::header_protocol::HeaderInventoryRecord],
    height: u64,
    block_hash: [u8; 32],
    ...
    records.iter().find_map(|record| {
        ...
        (record.header.height == height
            && echo_chain::block_header::block_id(&record.header) == block_hash
            && terminal.claim.height == height
            && terminal.claim.semantic_header_id
                == echo_chain::block_header::semantic_header_id(&record.header))
...
fn is_single_header_inventory_for_point(
    ...
    records.len() == 1
        ...
        && echo_chain::block_header::block_id(&records[0].header) == block_hash
}
...
struct BoundaryProofTarget {
    header: echo_chain::BlockHeader,
    epoch_anchor_header: echo_chain::BlockHeader,
}
...
impl BoundaryProofTarget {
    ...
    fn block_hash(self) -> [u8; 32] {
        echo_chain::block_header::block_id(&self.header)
    }
}
...
fn missing_snapshot_boundary_proof(
    ...
    let interval = echo_p2p::protocol::SNAPSHOT_BOUNDARY_INTERVAL;
    ...
    let block_hash = echo_chain::block_header::block_id(&header);
    ...
    let semantic_id = echo_chain::block_header::semantic_header_id(&header);
    ...
    let epoch_anchor_height =
        echo_chain::consensus::tx_epoch_anchor_height_for_child(header.height);
...
struct ReadySnapshotHeaderRange {
    ...
    count: u16,
    attempted_peers: std::collections::HashSet<libp2p::PeerId>,
    headers: Vec<echo_chain::BlockHeader>,
}
...
impl SnapshotHeaderPipeline {
    ...
    fn failure_plan(
        ...
        count: u16,
        token: u64,
        kind: echo_p2p::RequestFailureKind,
        peers: &std::collections::HashSet<libp2p::PeerId>,
        ...
        if matches!(kind, echo_p2p::RequestFailureKind::Timeout)
...
fn dispatch_snapshot_header_plans(
    ...
    for request in plans {
        let dispatched = !data_lane_full
            && p2p_cmd
                .try_send(echo_p2p::NetworkCommand::FetchSnapshotHeaders {
                    generation,
                    token: request.token,
                    peer: request.peer,
...
fn highest_snapshot_header_boundary(
    store: &echo_chain::storage::MdbxStore,
    target_height: u64,
...
fn prepare_snapshot_header_sync(
    ...
    Ok(PendingSnapshotHeaderSync {
        snapshot: echo_node::networking::SnapshotId {
            boundary: echo_node::networking::ChainPoint::new(
                manifest.tip_height,
                manifest.tip_hash,
            ),
...
struct VerifiedHistoryStepSnapshot {
    height: u64,
    block_hash: [u8; 32],
    boundary: echo_chain::VerifiedSnapshotBoundary,
...
fn verify_terminal_against_validated_snapshot_headers(
    ...
    runtime: &echo_recursive::acceptance::history_step::HistoryStepRuntime,
    authority: RetainedSnapshotHeaderAuthority,
...
fn validate_snapshot_staged_header_boundary(
    ...
    if bridge_span > echo_chain::consensus::params::RECENT_BLOCK_RETENTION_DEPTH {
    ...
    let expected_epoch_height =
        echo_chain::consensus::tx_epoch_anchor_height_for_child(manifest.tip_height);
...
fn verify_history_step_terminal(
    claim: &echo_chain::storage::HistoryStepTerminalClaim<'_>,
    runtime: Option<&echo_recursive::acceptance::history_step::HistoryStepRuntime>,
) -> Result<(), String> {
    let Some(runtime) = runtime else {
        return Err("embedded HistoryStep verifier unavailable".to_string());
    };
    echo_miner::install_inbound_verifier_cpu(|| {
        echo_recursive::acceptance::history_step::decode_verify_history_step_terminal(
            runtime,
...
fn history_step_context_error_is_terminal_peer_fault(
    error: &echo_chain::storage::MdbxContextError,
) -> bool {
    match error {
        echo_chain::storage::MdbxContextError::Consensus(
            echo_chain::consensus::ConsensusError::BadHistoryStepTerminal(message),
        ) => {
...
fn exact_suffix_context_error_is_body_peer_fault(
    ...
    match error {
        ...
        echo_chain::storage::MdbxContextError::Consensus(_) => true,
        echo_chain::storage::MdbxContextError::Corrupt(message) => matches!(
            *message,
        ...
        echo_chain::storage::MdbxContextError::Store(_)
        | echo_chain::storage::MdbxContextError::ResourceLimit { .. } => false,
    }
}

fn quarantine_exact_suffix_sources(
    header_dag: &mut echo_node::networking::header_dag::HeaderDag,
...
fn validate_history_step_tip_future_drift(
    ...
    echo_chain::consensus::validate_future_drift(boundary.tip_header.timestamp, local_time)
...
async fn apply_exact_suffix_offthread(
    ...
    wallet: &SharedWallet,
    fetched: echo_node::networking::suffix_sync::FetchedSuffix,
    history_step_runtime: Option<Arc<echo_recursive::acceptance::history_step::HistoryStepRuntime>>,
    wallet_operation_gate: &WalletOperationGate,
) -> Result<AppliedExactSuffix, ExactSuffixApplyError> {
    use echo_node::networking::sync_plan::SyncPlanKind;
    ...
    let result = tokio::task::spawn_blocking(move || {
        ...
        for ((bytes, source), expected) in body_bytes
            ...
            .zip(plan.headers())
        {
            let block = echo_chain::Block::from_bytes(bytes).map_err(|error| {
        ...
        let tip_header = blocks
            ...
            .header;
        if echo_chain::block_id(&tip_header) != plan.target().hash {
            return Err(ExactSuffixApplyError::body(
                *body_sources
                    .last()
        ...
        let epoch_height =
            echo_chain::consensus::tx_epoch_anchor_height_for_child(tip_header.height);
        ...
        let terminal_started = Instant::now();
        let verified_terminal = echo_chain::storage::verify_history_step_terminal_candidate(
            tip_header,
            epoch_anchor_header,
            terminal_bytes,
        ...
        match plan.kind() {
            SyncPlanKind::LiveSuffix => {
                ...
                {
                    let txids = match echo_chain::try_compute_logical_txids(&block.transactions) {
                        Ok(txids) => txids,
                        Err(error) => {
                    ...
                    if let Err(error) = ctx.apply_verified_recursive_suffix_block(
                        ...
                        unix_now(),
                        |block, state| {
                            echo_chain::materialize_accepted_block_state(state, block)
            ...
            SyncPlanKind::Reorg => {
                ...
                let confirmed_tx_hashes = blocks
                    .iter()
                    .flat_map(|block| {
                        echo_chain::try_compute_logical_txids(&block.transactions)
...
enum HeaderInventoryPlan {
    Confirmed {
        tip: echo_node::networking::ChainPoint,
    },
    Behind,
    NeedOlder {
        start_height: u64,
    ...
    Candidate {
        headers: Vec<echo_node::networking::header_dag::ValidatedHeader>,
        records: Vec<echo_p2p::header_protocol::HeaderInventoryRecord>,
        old_tip: echo_node::networking::ChainPoint,
        target: echo_node::networking::ChainPoint,
    },
    FinalizedDivergence,
}
...
fn canonical_header_dag(
...
) -> Result<echo_node::networking::header_dag::HeaderDag, String> {
    use echo_node::networking::{header_dag::ValidatedHeader, ChainPoint};
    ...
    let mut dag = echo_node::networking::header_dag::HeaderDag::new(
...
fn reconcile_canonical_header_dag(
    context: &MdbxChainContext,
    dag: &mut echo_node::networking::header_dag::HeaderDag,
) -> Result<(), String> {
    use echo_node::networking::{header_dag::ValidatedHeader, ChainPoint};
    ...
    for height in dag.finalized().height.saturating_add(1)..=context.tip_height() {
        ...
        let hash = echo_chain::block_header::block_id(&header);
...
fn record_validated_headers(
    dag: &mut echo_node::networking::header_dag::HeaderDag,
    headers: &[echo_node::networking::header_dag::ValidatedHeader],
) -> Result<(), echo_node::networking::header_dag::HeaderDagError> {
...
fn advertise_inventory_for_known_headers(
    ...
    peer: libp2p::PeerId,
    records: &[echo_p2p::header_protocol::HeaderInventoryRecord],
) -> Result<usize, echo_node::networking::header_dag::HeaderDagError> {
    let known_inventory = records
        .iter()
        .filter(|record| record.body.is_some() || record.terminal.is_some())
        .filter(|record| {
            let hash = echo_chain::block_id(&record.header);
            dag.get(&hash)
...
fn header_inventory_validation_anchor(
    canonical: Option<echo_node::networking::ChainPoint>,
    validated_dag: Option<echo_node::networking::ChainPoint>,
) -> Option<echo_node::networking::ChainPoint> {
...
fn header_dag_has_unresolved_better_tip(
    dag: &echo_node::networking::header_dag::HeaderDag,
    canonical_tip: echo_node::networking::ChainPoint,
...
async fn plan_header_inventory(
    ...
    use echo_chain::consensus::params::CONSENSUS_FINALITY_DEPTH;
    use echo_node::networking::{header_dag::ValidatedHeader, ChainPoint};
    ...
    for header in competing_headers {
        cumulative_work = echo_chain::add_work(
            &cumulative_work,
            &echo_chain::block_work(&header.difficulty_target),
        );
...
fn source_independent_suffix_offer(
    ...
    preferred_peer: libp2p::PeerId,
    old_tip: echo_node::networking::ChainPoint,
    base: echo_node::networking::ChainPoint,
    headers: Vec<echo_node::networking::header_dag::ValidatedHeader>,
) -> Result<
    (
        libp2p::PeerId,
        echo_node::networking::suffix_sync::SuffixOffer,
        Vec<(
            libp2p::PeerId,
            Vec<echo_p2p::header_protocol::HeaderInventoryRecord>,
        )>,
    ),
    echo_node::networking::suffix_sync::SuffixSyncError,
> {
    use echo_node::networking::suffix_sync::{SuffixOffer, SuffixSyncError};
    ...
    let mut bootstrap = headers
        .iter()
        .map(|header| echo_p2p::header_protocol::HeaderInventoryRecord::header_only(header.header))
        .collect::<Vec<_>>();
    bootstrap
        .last_mut()
...
fn dispatch_exact_suffix_requests(
    sync: &mut echo_node::networking::suffix_sync::SuffixSync,
    p2p_cmd: &echo_p2p::NetworkCommandSender,
) {
    ...
    for request in sync.schedule(now_ms) {
        if p2p_cmd
            .try_send(echo_p2p::NetworkCommand::FetchObjects {
                token: request.token,
                peer: request.peer,
...
fn merge_active_suffix_inventory(
    active: &mut Option<echo_node::networking::suffix_sync::SuffixSync>,
    peer: libp2p::PeerId,
    failure_domain: echo_node::networking::FailureDomain,
    records: &[echo_p2p::header_protocol::HeaderInventoryRecord],
) -> Result<usize, echo_node::networking::suffix_sync::SuffixSyncError> {
...
fn admit_exact_suffix_offer(
    ...
    peer: libp2p::PeerId,
    failure_domain: echo_node::networking::FailureDomain,
    offer: echo_node::networking::suffix_sync::SuffixOffer,
) -> Result<SuffixAdmission, String> {
    use echo_chain::consensus::fork_choice::ChainChoice;
    use echo_node::networking::suffix_sync::SuffixSync;
...
mod tests {
    ...
    fn test_snapshot_id() -> echo_node::networking::SnapshotId {
        echo_node::networking::SnapshotId {
            boundary: echo_node::networking::ChainPoint::new(100, [1; 32]),
            state_root: [2; 32],
            manifest_digest: [3; 32],
            format_version: echo_p2p::protocol::SNAPSHOT_MANIFEST_FORMAT_VERSION,
        }
    }
    ...
    fn exact_progress_retires_only_a_canonical_snapshot_boundary() {
        let snapshot = echo_node::networking::ChainPoint::new(54, [1; 32]);
        let competing = echo_node::networking::ChainPoint::new(54, [2; 32]);
    ...
    fn test_exact_suffix_offer(
        ...
        use echo_chain::block_header::{block_id, semantic_header_id};
        use echo_node::networking::{header_dag::ValidatedHeader, ChainPoint};
        use echo_p2p::{
        ...
        echo_node::networking::suffix_sync::SuffixOffer::live(base, headers, &records).unwrap()
    }

    #[test]
    fn dag_selected_suffix_merges_objects_from_independent_sources() {
        use echo_chain::block_header::{block_id, semantic_header_id};
        use echo_node::networking::{
        ...
        let first_work = echo_chain::add_work(
            &base_work,
            &echo_chain::block_work(&first_header.difficulty_target),
        );
        ...
        let second_work = echo_chain::add_work(
            &first_work,
            &echo_chain::block_work(&second_header.difficulty_target),
        );
        ...
        assert_eq!(
            source_independent_suffix_offer(&dag, tip_peer, base, base, headers.clone())
                .unwrap_err(),
            echo_node::networking::suffix_sync::SuffixSyncError::MissingBodySource
        );
        assert_eq!(
    ...
    fn storage_epoch_initialization_preserves_wallet_and_local_files() {
        ...
        std::fs::write(directory.path().join("lostecho-node.log"), b"old log").unwrap();
        std::fs::write(directory.path().join("lostecho.toml"), b"local config").unwrap();
        ...
        assert_eq!(
            std::fs::read(directory.path().join("lostecho-node.log")).unwrap(),
            b"old log"
        );
        assert_eq!(
            std::fs::read(directory.path().join("lostecho.toml")).unwrap(),
            b"local config"
        );
        assert_eq!(
    ...
    fn missing_marker_is_repaired_for_an_existing_mainnet_database() {
        let directory = tempfile::tempdir().unwrap();
        let context =
            echo_chain::storage::MdbxChainContext::open_or_create(directory.path()).unwrap();
    ...
    fn moving_tip_replaces_a_plan_before_exact_object_progress() {
        ...
        assert_eq!(
            admit_exact_suffix_offer(
                &mut active,
                first_peer,
                echo_node::networking::FailureDomain(1),
                pinned,
            )
            .unwrap(),
        ...
        assert_eq!(
            admit_exact_suffix_offer(
                &mut active,
                extension_peer,
                echo_node::networking::FailureDomain(2),
                extension,
            )
            .unwrap(),
    ...
    fn moving_tip_preserves_a_plan_after_exact_object_progress() {
        use echo_p2p::object_protocol::{ObjectId, ObjectPayload};
    ...
    fn behind_header_range_still_supplies_the_pinned_suffix_objects() {
        use echo_node::networking::FailureDomain;
        use echo_p2p::{header_protocol::HeaderInventoryRecord, object_protocol::ObjectId};
        ...
        let records = vec![
            HeaderInventoryRecord::header_only(echo_chain::consensus::genesis_header()),
            HeaderInventoryRecord {
        ...
        let mut active = Some(
            echo_node::networking::suffix_sync::SuffixSync::from_offer(
                original_peer,
                FailureDomain(1),
                offer,
            )
    ...
    fn only_transient_terminal_transport_failures_retry_the_same_peer() {
        use echo_p2p::RequestFailureKind;
    ...
    fn mining_gate_accepts_one_confirmed_ordinary_peer() {
        ...
        quorum.connect(first, echo_node::networking::FailureDomain(1));
    ...
    fn mining_quorum_reacquisition_prioritizes_unconfirmed_public_peers() {
        ...
        for (index, peer) in peers.iter().enumerate() {
            quorum.connect(*peer, echo_node::networking::FailureDomain(index as u64));
        }
        let height = 31;
        let hash = [0x31; 32];
        quorum.set_canonical_tip(height, hash, false);
    ...
    fn verified_exact_suffix_source_reauthorizes_the_committed_tip_immediately() {
        ...
        quorum.connect(source, echo_node::networking::FailureDomain(1));
        quorum.connect(second_announcer, echo_node::networking::FailureDomain(2));
    ...
    fn header_only_candidate_does_not_pause_until_exact_plan_exists() {
        ...
        quorum.connect(peer, echo_node::networking::FailureDomain(1));
        quorum.set_canonical_tip(height, hash, false);
        quorum.set_sync_state(true, false);
    ...
    fn first_start_creates_and_reuses_default_config() {
        ...
        let path = temp.path().join("nested/lostecho.toml");
    ...
    fn malformed_config_is_reported_instead_of_silently_ignored() {
        let temp = tempfile::tempdir().unwrap();
        let path = temp.path().join("lostecho.toml");
    ...
    fn serving_reserve_does_not_change_finality_or_snapshot_suffix() {
        assert_eq!(echo_chain::consensus::params::CONSENSUS_FINALITY_DEPTH, 18);
        assert_eq!(
            echo_chain::consensus::params::RECENT_BLOCK_RETENTION_DEPTH,
            18
        );
        assert_eq!(
            echo_chain::consensus::params::RETAINED_BLOCK_SERVING_DEPTH,
            42
        );
    }
    ...
    fn connected_tip_probe_reaches_every_first_usable_snapshot_boundary() {
        assert_eq!(
            ...
            echo_chain::consensus::params::CONSENSUS_FINALITY_DEPTH
                + echo_p2p::protocol::SNAPSHOT_BOUNDARY_INTERVAL
                + 1
        );
    ...
    fn unresolved_reorg_probe_starts_at_the_common_ancestor() {
        use echo_chain::block_header::block_id;
        use echo_node::networking::{
            header_dag::{HeaderDag, ValidatedHeader},
            ChainPoint,
        };
        ...
        let local_work = echo_chain::add_work(
            &base_work,
            &echo_chain::block_work(&local_header.difficulty_target),
        );
        ...
        let competing_tip_work = echo_chain::add_work(
            &local_work,
            &echo_chain::block_work(&competing_tip_header.difficulty_target),
        );
        let competing_tip =
    ...
    fn far_rebase_snapshot_is_bound_to_the_selected_header_dag_tip() {
        ...
        let selected_work = echo_chain::add_work(
            &first_work,
            &echo_chain::block_work(&selected_header.difficulty_target),
        );
        ...
        let farther_work = echo_chain::add_work(
            &selected_work,
            &echo_chain::block_work(&selected_header.difficulty_target),
        );
        let farther = echo_p2p::protocol::GetStateManifestResponse {
            tip_height: 3,
            tip_hash: [0xA3; 32],
        ...
        let exact = echo_p2p::protocol::GetStateManifestResponse {
        ...
        let losing = echo_p2p::protocol::GetStateManifestResponse {
            cumulative_chainwork: selected_work,
            ..farther.clone()
        };
        ...
        let third_work = echo_chain::add_work(
            &selected_work,
            &echo_chain::block_work(&third_header.difficulty_target),
        );
        ...
        let fourth_work = echo_chain::add_work(
            &third_work,
            &echo_chain::block_work(&fourth_header.difficulty_target),
        );
        ...
        let selected_ancestor = echo_p2p::protocol::GetStateManifestResponse {
    ...
    fn linear_snapshot_install_is_bound_to_selected_header_dag_ancestry() {
        ...
        for height in 1..=6 {
            ...
            branch_a_work = echo_chain::add_work(
                &branch_a_work,
                &echo_chain::block_work(&branch_a_header.difficulty_target),
            );
            let validated =
        ...
        for height in 1..=7 {
            ...
            branch_b_work = echo_chain::add_work(
                &branch_b_work,
                &echo_chain::block_work(&branch_b_header.difficulty_target),
            );
            let validated =
        ...
        for _ in 8..=12 {
            farther_work = echo_chain::add_work(
                &farther_work,
                &echo_chain::block_work(&branch_b_header.difficulty_target),
            );
        }
    ...
    fn snapshot_segment_errors_separate_remote_bytes_from_local_state() {
        use echo_chain::storage::snapshot_staging::SnapshotStagingError;
        ...
        assert!(matches!(
            classify_snapshot_session_prepare_error(
                echo_chain::storage::snapshot_staging::SnapshotStagingError::EffectiveLogMismatch {
                    expected: 16,
                    actual: 15,
                }
            ),
        ...
        assert!(matches!(
            classify_snapshot_session_prepare_error(
                echo_chain::storage::snapshot_staging::SnapshotStagingError::Io {
                    operation: "test",
                    source: std::io::Error::other("disk failed"),
                }
            ),
        ...
        assert!(matches!(
            classify_snapshot_finalization_error(
                echo_chain::storage::snapshot_staging::SnapshotStagingError::StateRootMismatch {
                    expected: [1; 32],
                    actual: [2; 32],
                }
            ),
        ...
        assert!(matches!(
            classify_snapshot_finalization_error(
                echo_chain::storage::snapshot_staging::SnapshotStagingError::StagedFileLength {
                    segment_id: 7,
                    expected: 10,
                    actual: 9,
                }
    ...
    fn snapshot_header_pipeline_uses_one_ordered_same_peer_window() {
        ...
        let headers = |start: u64, count: u16| {
            (start..start + u64::from(count))
                .map(|height| {
                    let mut header = echo_chain::consensus::genesis::genesis_header();
    ...
    fn snapshot_header_transport_failure_retries_only_its_exact_range() {
        ...
        let retry = pipeline
            .failure_plan(
                ...
                SNAPSHOT_HEADER_BATCH as u16,
                failed.token,
                echo_p2p::RequestFailureKind::Io,
                &peers,
            )
    ...
    fn snapshot_header_timeout_reduces_only_the_slow_range() {
        ...
        let retry = pipeline
            .failure_plan(
                ...
                initial.count,
                initial.token,
                echo_p2p::RequestFailureKind::Timeout,
                &peers,
            )
            .unwrap();
    ...
    fn snapshot_header_failures_try_every_connected_peer_before_reuse() {
        ...
        let retry_one = pipeline
            .failure_plan(
                ...
                initial.count,
                initial.token,
                echo_p2p::RequestFailureKind::InvalidResponse,
                &peers,
            )
            .unwrap();
    ...
    fn snapshot_manifest_selection_replaces_the_first_stale_candidate() {
        let mut first = echo_p2p::protocol::GetStateManifestResponse::default();
    ...
    fn snapshot_manifest_selection_uses_the_canonical_hash_tiebreak() {
        let mut current = echo_p2p::protocol::GetStateManifestResponse::default();
    ...
    fn test_coinbase_child(
        parent: &echo_chain::BlockHeader,
        state: &echo_chain::ChainState,
    ) -> echo_chain::block::Block {
        let timestamp = parent.timestamp + echo_chain::consensus::params::BLOCK_TIME;
        let difficulty_target = echo_chain::consensus::difficulty::next_target(
            0,
            parent.timestamp,
        ...
        let template = echo_chain::consensus::build_block_template(
            parent,
            state,
            &[parent.active_slot_count],
            vec![],
            echo_poseidon2b::primitives::Address([0x22; 32]),
            timestamp,
            difficulty_target,
        )
    ...
    fn snapshot_history_boundary_checks_staged_header_chainwork() {
        let state = echo_chain::ChainState::with_log_slots(
            echo_chain::consensus::params::LOG_SLOTS_GENESIS
                .try_into()
                .expect("genesis log_slots fits usize"),
        );
        let h0 = echo_chain::consensus::genesis_header();
        let h0_hash = echo_chain::hash_block_header(&h0);
        let high_start_work = echo_chain::consensus::block_work(&h0.difficulty_target);

        let block = test_coinbase_child(&h0, &state);
        let h1 = block.header;
        let h1_hash = echo_chain::hash_block_header(&h1);
        let h1_work = echo_chain::consensus::add_work(
            &high_start_work,
            &echo_chain::consensus::block_work(&h1.difficulty_target),
        );
        let mut manifest = echo_p2p::protocol::GetStateManifestResponse {
            tip_height: 1,
            tip_hash: h1_hash,
    ...
    fn snapshot_epoch_anchor_obeys_start_of_block_boundaries() {
        for (tip_height, expected_epoch_height) in [
            ...
            let mut tip_header = echo_chain::consensus::genesis_header();
            tip_header.height = tip_height;
            let tip_hash = echo_chain::hash_block_header(&tip_header);
            ...
            assert!(manifest.seal_manifest_digest());
            let mut epoch_anchor_header = echo_chain::consensus::genesis_header();
        ...
        let tip_height = echo_chain::consensus::params::TX_EPOCH_BLOCKS;
        let mut tip_header = echo_chain::consensus::genesis_header();
        tip_header.height = tip_height;
        let tip_hash = echo_chain::hash_block_header(&tip_header);
        ...
        assert!(manifest.seal_manifest_digest());
        let mut wrong_anchor = echo_chain::consensus::genesis_header();
    ...
    fn snapshot_history_step_tip_obeys_local_future_drift_admission() {
        ...
        let mut tip = echo_chain::consensus::genesis::genesis_header();
        tip.timestamp = local_time + echo_chain::consensus::params::MAX_FUTURE_DRIFT;
        let mut boundary = SnapshotHeaderBoundary {
            tip_header: tip,
            tip_hash: echo_chain::hash_block_header(&tip),
...
async fn handle_p2p_events(
    mut rx: echo_p2p::NetworkEventReceiver,
    chain: Arc<RwLock<MdbxChainContext>>,
    mempool: AsyncMempool,
    wallet: SharedWallet,
    p2p_cmd: echo_p2p::NetworkCommandSender,
    ...
    mut canonical_tip_changes: tokio::sync::watch::Receiver<echo_p2p::object_protocol::ChainPoint>,
) -> anyhow::Result<()> {
    use echo_chain::consensus::params::CONSENSUS_FINALITY_DEPTH;
    ...
    struct PendingManifest {
        ...
        rebase_base: Option<echo_node::networking::ChainPoint>,
        manifest: echo_p2p::protocol::VerifiedStateManifest,
        offer: echo_node::networking::snapshot_sync::SnapshotOffer,
    ...
    struct ExactSuffixApplyCompletion {
        plan_id: echo_node::networking::PlanId,
        target: echo_node::networking::ChainPoint,
        tip_announcement: echo_p2p::header_protocol::HeaderAnnouncement,
    ...
    let mut active_snapshot_sync: Option<echo_node::networking::snapshot_sync::SnapshotSync> = None;
    ...
    let mut active_suffix_sync: Option<echo_node::networking::suffix_sync::SuffixSync> = None;
    ...
    let mut exact_suffix_apply_inflight: Option<echo_node::networking::PlanId> = None;
    let mut peer_failure_domains: std::collections::HashMap<
        libp2p::PeerId,
        echo_node::networking::FailureDomain,
    > = std::collections::HashMap::new();
    ...
    let (mut header_dag, mut header_dag_canonical_tip) = {
        let ctx = chain.read().await;
        let canonical_tip =
            echo_node::networking::ChainPoint::new(ctx.tip_height(), ctx.tip_hash());
    ...
    let mut queued_segment_response: Option<(
        libp2p::PeerId,
        echo_p2p::protocol::GetStateSegmentResponse,
    )> = None;
    ...
    macro_rules! dispatch_snapshot_generation_advance {
        () => {{
            if p2p_snapshot_generation_announced != snapshot_sync_generation
                && p2p_cmd
                    .try_send(echo_p2p::NetworkCommand::AdvanceSnapshotGeneration {
    ...
    macro_rules! try_request_manifest {
        ($peer:expr, $requester_height:expr, $digest:expr) => {{
            ...
            if manifest_requested_peers.contains(&peer) {
                ...
                .try_send(echo_p2p::NetworkCommand::RequestStateManifest {
                    generation: snapshot_sync_generation,
                    peer,
    ...
    macro_rules! dispatch_pending_boundary_terminal_requests {
        () => {{
            if let Some(pending) = snapshot_boundary_terminal_inflight.as_mut() {
                ...
                for request in requests {
                    if p2p_cmd
                        .try_send(echo_p2p::NetworkCommand::RequestHistoryStepTerminal {
                            token: request.token,
                            peer: request.peer,
    ...
    macro_rules! retire_snapshot_plan {
        () => {{
            ...
            if let Some(request) = snapshot_boundary_terminal_inflight.take() {
                let _ = p2p_cmd
                    .send(echo_p2p::NetworkCommand::CancelHistoryStepTerminalRace {
    ...
    macro_rules! begin_snapshot_header_staging {
        ($from:expr, $manifest:expr, $rebase_anchor:expr, $fetches:expr, $recent:expr) => {{
            ...
            let snapshot_offer = match echo_node::networking::snapshot_sync::SnapshotOffer::from_verified_manifest(
                manifest.clone(),
            ) {
                Ok(offer) => offer,
                Err(error) => {
            ...
            let rebase_base = snapshot_rebase_hint.and_then(|hint| {
                (hint.ancestor_height < terminal_height).then_some(
                    echo_node::networking::ChainPoint::new(
    ...
    macro_rules! arm_post_snapshot_tail {
        ($height:expr, $hash:expr) => {{
            let boundary = echo_node::networking::ChainPoint::new($height, $hash);
    ...
    macro_rules! mark_bootstrap_complete_if_caught_up {
        ($local_height:expr) => {{
            ...
            {
                if p2p_cmd
                    .send(echo_p2p::NetworkCommand::BootstrapComplete)
                    .await
                    .is_ok()
                {
    ...
    loop {
        ...
        match rx_item {
            Ok(NetworkEvent::HeaderAnnouncement {
                ...
                let exact_inventory = (source_has_objects && manifest_peers.contains(&from))
                    .then(|| {
                        echo_p2p::header_protocol::HeaderInventoryRecord::from_announcement(
                            announcement,
                        )
                    });
                ...
                let announced_hash = echo_chain::block_id(&announced_header);
                let (our_height, our_hash, finalized_height, canonical_hash_at_height) = {
                    ...
                    (
                        ...
                        ctx.header(height).map(echo_chain::block_id),
                    )
                };
                ...
                if height > our_height.saturating_add(1) {
                    ...
                    let precheck = {
                        ...
                        let validation_context =
                            (|| -> Result<_, echo_chain::storage::MdbxContextError> {
                                Ok((
                                    ctx.prev_timestamps()?,
                                    ctx.anchor_info()?,
                        ...
                        match validation_context {
                            Ok((prev_timestamps, anchor, finalized_active_counts)) => {
                                Some((
                                    echo_chain::consensus::validate_header(
                                        &announced_header,
                                        &parent,
                                        &prev_timestamps,
                    ...
                    if let Err(e) = precheck {
                        if e == echo_chain::consensus::ConsensusError::BadParentHash {
                    ...
                    let cumulative_work = echo_chain::add_work(
                        &base_work,
                        &echo_chain::block_work(&announced_header.difficulty_target),
                    );
                    let validated = echo_node::networking::header_dag::ValidatedHeader::new_after_consensus_checks(
                        announced_header,
                        cumulative_work,
                    );
                    ...
                    {
                        match echo_node::networking::suffix_sync::SuffixOffer::live(
                            base,
                            vec![validated],
                            &[record],
                        ) {
                            Ok(offer) => {
                                let domain = peer_failure_domains.get(&from).copied().unwrap_or(
                                    echo_node::networking::FailureDomain(u64::MAX),
                                );
            ...
            Ok(NetworkEvent::ObjectsResponse {
                ...
                match sync.accept_response(
                    ...
                    Err(
                        echo_node::networking::suffix_sync::SuffixSyncError::UnknownToken,
                    ) => {
                        tracing::debug!(
                            token,
                            peer = %from,
                    ...
                    Err(
                        echo_node::networking::suffix_sync::SuffixSyncError::ContentMismatch,
                    ) => {
            ...
            Ok(NetworkEvent::ObjectsRequestFailed {
                ...
                match result {
                    Ok(()) => {
                        let rejected = matches!(kind, echo_p2p::RequestFailureKind::InvalidResponse);
            ...
            Ok(NetworkEvent::ObjectsRequestBusy {
                ...
                match sync.request_busy(
                    ...
                    Err(echo_node::networking::suffix_sync::SuffixSyncError::UnknownToken) => {}
                    Err(error) => {
            ...
            Ok(NetworkEvent::MempoolSyncResponse {
                ...
                let admission = tokio::spawn(async move {
                    ...
                    for intent_bytes in txs {
                        ...
                        if let Ok(intent) = echo_tx::PagedSpendIntent::from_bytes(&intent_bytes) {
            ...
            Ok(NetworkEvent::NewTx {
                ...
                let mempool_task = mempool.clone();
                tokio::spawn(async move {
                    let acceptance = match echo_tx::PagedSpendIntent::from_bytes(&intent_bytes) {
            ...
            Ok(NetworkEvent::PeerConnected {
                ...
                peer_failure_domains.insert(
                    peer,
                    echo_node::networking::FailureDomain(failure_domain),
                );
                ...
                if request_mempool
                    ...
                    && p2p_cmd
                        .try_send(echo_p2p::NetworkCommand::RequestMempoolSync { peer })
                        .is_ok()
                {
            ...
            Ok(NetworkEvent::SnapshotHeadersBatch {
                ...
                if let Some(boundary) = snapshot_boundary.filter(|boundary| {
                    boundary.height > 0
                        && boundary.height % echo_p2p::protocol::SNAPSHOT_BOUNDARY_INTERVAL == 0
                        && boundary.hash != [0; 32]
                }) {
            ...
            Ok(NetworkEvent::SnapshotHeadersRequestFailed {
                ...
                if kind == echo_p2p::RequestFailureKind::LocalCapacity {
            ...
            Ok(NetworkEvent::HeaderInventoryBatch {
                ...
                if !rejected_suffix_object_peers.contains(&from) {
                    let domain = peer_failure_domains
                        .get(&from)
                        .copied()
                        .unwrap_or(echo_node::networking::FailureDomain(u64::MAX));
                ...
                match plan {
                    ...
                    Ok(HeaderInventoryPlan::Candidate {
                        ...
                        match source_independent_suffix_offer(
                            ...
                            Err(
                                echo_node::networking::suffix_sync::SuffixSyncError::MissingTipTerminal,
                            ) => {
                                let domain = peer_failure_domains.get(&from).copied().unwrap_or(
                                    echo_node::networking::FailureDomain(u64::MAX),
                                );
                ...
                if manifest.tip_height > 0 {
                        ...
                        .is_some_and(|pending| pending.manifest.as_ref() == manifest.as_ref())
                    {
                        ...
                        if let Some(sync) = active_snapshot_sync.as_mut() {
                            ...
                            let offer = echo_node::networking::snapshot_sync::SnapshotOffer::from_verified_manifest(
                                manifest.clone(),
                            );
            ...
            Ok(NetworkEvent::StateSegment { from, response }) => {
                ...
                if snapshot_staging_inflight.is_some() && queued_segment_response.is_some() {
                    ...
                    let request =
                        echo_node::networking::snapshot_sync::SnapshotSegmentRequest {
                            peer: from,
                            segment,
                        };
                ...
                if let Err(error) = sync.accept_response(from, segment, data_len) {
                    ...
                    if matches!(
                        error,
                        echo_node::networking::snapshot_sync::SnapshotSyncError::ResponseLengthMismatch
                    ) {
            ...
            Ok(NetworkEvent::HistoryStepTerminal {
                ...
                if let Some(maintenance_key) = maintenance_key {
                    ...
                    let _ = p2p_cmd.try_send(
                        echo_p2p::NetworkCommand::CancelHistoryStepTerminalRace { token },
                    );
            ...
            Ok(NetworkEvent::HistoryStepTerminalRequestFailed {
                ...
                let source_exhausted = match kind {
                    ...
                    echo_p2p::RequestFailureKind::Unavailable
                    | echo_p2p::RequestFailureKind::UnsupportedProtocol => {
                        manifest_terminal_capabilities.remove(&from);
                        snapshot_terminal_exhausted.insert(source_key);
                        true
                    }
                    echo_p2p::RequestFailureKind::Timeout
                    | echo_p2p::RequestFailureKind::Io
                    | echo_p2p::RequestFailureKind::Dial
                    | echo_p2p::RequestFailureKind::ConnectionClosed => {
            ...
            Err(echo_p2p::NetworkEventRecvError::Lagged(n)) => {
                tracing::warn!(n, "P2P gossip receiver lagged — recoverable gossip events dropped");
            }
            Err(echo_p2p::NetworkEventRecvError::Closed) => {
        ...
        completed = exact_suffix_apply_rx.recv() => {
            ...
            match completed.result {
                Ok(AppliedExactSuffix::Live(mut applied)) => {
                    ...
                    if complete {
                        committed_exact_tip = Some(echo_node::networking::ChainPoint::new(
                            applied.height,
                            applied.block_hash,
                        ));
                        ...
                        tokio::spawn(async move {
                            if relay
                                .send(echo_p2p::NetworkCommand::AnnounceAvailability {
                                    announcement,
                                })
                                .await
                                .is_err()
                    ...
                    let (canonical_tip_height, canonical_boundary) = {
                        ...
                        let canonical_boundary = if ctx.tip_height() >= snapshot.boundary.height {
                            match ctx.get_header_from_store(snapshot.boundary.height) {
                                Ok(Some(header)) => {
                                    Some(echo_node::networking::ChainPoint::new(
                                        snapshot.boundary.height,
                                        echo_chain::block_header::block_id(&header),
                                    ))
                                }
                                Ok(None) => None,
                                Err(error) => {
        ...
        completed = snapshot_install_completion_rx.recv() => {
            ...
            if let Some(token) = completed.key.terminal_request_token {
                let _ = p2p_cmd
                    .send(echo_p2p::NetworkCommand::CancelHistoryStepTerminalRace { token })
                    .await;
            }
            match completed.result {
                Ok(applied) => {
        ...
        completed = history_step_verification_rx.recv() => {
            ...
            sync_phase_telemetry.observe_header_scale(
                ...
                completed
                    .staged_header_count
                    .saturating_mul(echo_chain::BLOCK_HEADER_WIRE_SIZE as u64),
            );
            ...
            let semantic_header_id = echo_chain::block_header::semantic_header_id(&boundary_header);
            let mut snapshot_sync = match echo_node::networking::snapshot_sync::SnapshotSync::new(
                initial_peer,
                initial_domain,
        ...
        _ = heartbeat.tick() => {
            ...
            let (our_height, our_hash, our_prev_hash, our_work, reconciled_header_dag) = {
                let ctx = chain.read().await;
                let canonical_tip = echo_node::networking::ChainPoint::new(
                    ctx.tip_height(),
                    ctx.tip_hash(),
                );
            ...
            if let Some(reconciled) = reconciled_header_dag {
                ...
                match reconciled {
                    Ok(()) => {
                        header_dag_canonical_tip = echo_node::networking::ChainPoint::new(
                            our_height,
                            our_hash,
                        );
            ...
            let unresolved_header_waiting_for_inventory = active_suffix_sync.is_none()
                ...
                && header_dag_has_unresolved_better_tip(
                    &header_dag,
                    echo_node::networking::ChainPoint::new(our_height, our_hash),
                    our_work,
                );
                    ...
                    >= EXACT_INVENTORY_PROBE_INTERVAL
            {
                ...
                let committed_tip = echo_node::networking::ChainPoint::new(our_height, our_hash);
                ...
                .is_some_and(|pending| pending.requests.deadline_due(now))
            {
                ...
                let _ = p2p_cmd.try_send(
                    echo_p2p::NetworkCommand::CancelHistoryStepTerminalRace {
...
fn create_snapshot_staging_session(
    ...
    if echo_chain::block_header::block_id(&header) != manifest.tip_hash
...
fn dispatch_exact_snapshot_segments(
    sync: &mut echo_node::networking::snapshot_sync::SnapshotSync,
    p2p_cmd: &echo_p2p::NetworkCommandSender,
    ...
    for request in sync.schedule(now_ms, 1) {
        let command = echo_p2p::NetworkCommand::RequestStateSegment {
            peer: request.peer,
...
fn update_wallet_for_block(wallet: &SharedWallet, block: &echo_chain::block::Block) {
...
fn print_startup_banner(
    ...
    let bar: String = (0..bar_w)
        ...
        .collect();
    let effective_log = log_slots.min(echo_chain::fri_state::LOG_SEGMENT_SIZE as u32) as u8;
    let max_segment_bytes = echo_chain::storage::max_encoded_segment_len_for_eff_log(effective_log)
    ...
    if mining {
        ...
        row(
            "mining",
            &format!(
                "{reward:.2} ECHO/block   coinbase  {cb}",
                reward = block_reward_echo
            ),
        );
    } else {
...
fn parse_address(s: &str) -> anyhow::Result<echo_poseidon2b::primitives::Address> {
    if s.is_empty() {
        return Ok(echo_poseidon2b::primitives::Address([0u8; 32]));
    }
    echo_poseidon2b::primitives::Address::parse(s)
