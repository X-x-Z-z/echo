...
//! lostecho-cli — Lostecho full node CLI client.
//!
//! Connects to a running `lostecho` daemon via JSON-RPC (no local keys, no crypto).
//! All operations happen inside the daemon; the CLI is a thin terminal UI.
//!
//! Quick start:
//!   lostecho-cli status            — node health at a glance
//!   lostecho-cli balance           — wallet balance
//!   lostecho-cli send <addr> 10.5  — send 10.5 ECHO
//!   lostecho-cli help              — full command list

#![allow(clippy::format_in_format_args, clippy::print_literal)]

use anyhow::{bail, Context};
use clap::{Parser, Subcommand};
use echo_chain::consensus::params::RETAINED_BLOCK_SERVING_DEPTH;
...
const MICRO_PER_ECHO: u64 = 1_000_000;

/// Parse a human amount like "10.5" or "0.000001" as ECHO → μECHO.
fn parse_echo_amount(s: &str) -> anyhow::Result<u64> {
    ...
    whole
        .checked_mul(MICRO_PER_ECHO)
        .and_then(|value| value.checked_add(fractional))
        .ok_or_else(|| anyhow::anyhow!("amount {s:?} is out of range"))
}

fn echo_str(microecho: u64) -> String {
    format!(
        "{}.{:06}",
        microecho / MICRO_PER_ECHO,
        microecho % MICRO_PER_ECHO
    )
}
...
#[command(
    name = "lostecho-cli",
    about = "ParanO(1)d thin client — control a running lostecho daemon",
    version = env!("CARGO_PKG_VERSION"),
    long_about = "\
ParanO(1)d thin client. Connects to a running lostecho daemon via JSON-RPC.

QUICK START:
  lostecho-cli status              Node info (height, hash, slots)
  lostecho-cli balance             Wallet balance
  lostecho-cli send <addr> 10.5   Send 10.5 ECHO to address
  lostecho-cli history             Transaction history
  lostecho-cli mempool             Pending transactions
  lostecho-cli help                All commands

AMOUNT FORMAT:
  Amounts are in ECHO (e.g. 10.5, 0.000001).
  1 ECHO = 1,000,000 μECHO — the CLI converts automatically.

DAEMON:
  The daemon must be running: lostecho --mode miner --data-dir ~/.lostecho/data",
)]
struct Cli {
    /// JSON-RPC endpoint of the running lostecho daemon.
    #[arg(
        long,
        short = 'r',
...
enum Command {
    ...
    /// Confirmed wallet balance (ECHO and μECHO).
    #[command(alias = "bal")]
    Balance,
    ...
    /// Send ECHO to a recipient address.
    ///
    /// Amount is in ECHO (e.g. 10.5 → 10,500,000 μECHO).
    /// Fee is auto-computed if not specified (recommended).
    ///
    /// Examples:
    ///   lostecho-cli send f784...b61e 10.5
    ///   lostecho-cli send f784...b61e 10.5 --fee 0.01
    Send {
        /// Recipient address (canonical bech32m o1...).
        #[arg(value_name = "ADDRESS")]
        to: String,
        /// Amount in ECHO  (1 ECHO = 1 000 000 μECHO).
        /// Examples: "50" = 50 ECHO, "0.5" = 500 000 μECHO, "0.000001" = 1 μECHO (minimum).
        /// Tip: for programmatic use the RPC walletSend accepts raw μECHO directly.
        #[arg(value_name = "AMOUNT")]
        amount: String,
        /// Transaction fee in ECHO. Omit for automatic fee calculation.
        #[arg(long, value_name = "FEE_ECHO")]
        fee: Option<String>,
    ...
    /// Redirect output to a file: lostecho-cli receipt <hash> > receipt.hex
    Receipt {
    ...
    // ---- Node control ---------------------------------------------------------
    /// Gracefully stop the Lostecho daemon.
    Stop,
...
async fn main() {
    ...
    // Without this, `lostecho-cli utxos | head -5` prints a Rust panic traceback.
...
async fn run(cli: Cli) -> anyhow::Result<()> {
    let client = reqwest::Client::builder()
        .user_agent(concat!("lostecho-cli/", env!("CARGO_PKG_VERSION")))
...
fn print_error(msg: &str) {
        ...
        || msg.contains("Node is not responding")
    {
        "Node is not responding.\n\
             Is the lostecho daemon running?  Try: lostecho --mode miner\n\
    ...
    {
        "Invalid address.\nUse a bech32m address (o1…).\nExample: lostecho-cli send o1q9gnyj0zwhqj9tm5sf… 10.5".to_string()
    } else {
        msg.to_string()
    };
...
async fn cmd_status(ctx: &Ctx<'_>) -> anyhow::Result<()> {
    ...
    let diff_bits = hex::decode(diff)
        ...
        .map(|target| echo_chain::consensus::target_leading_zero_bits(&target));

    section("Lostecho node status");
...
async fn cmd_utxos_of(ctx: &Ctx<'_>, address: &str) -> anyhow::Result<()> {
    ...
    separator(74);
    println!("  {:<12}  {:>20}  {:>14}", "slot", "creation id", "ECHO");
    separator(74);
    for s in &slots {
...
async fn cmd_tx(ctx: &Ctx<'_>, txhash: &str) -> anyhow::Result<()> {
    ...
    if result.is_null() {
        ...
        println!("  Use 'lostecho-cli mempool-tx {txhash}' to check if it is pending.");
        return Ok(());
    }
    section("Transaction");
...
async fn cmd_mining(ctx: &Ctx<'_>) -> anyhow::Result<()> {
    ...
    let reward_micro = result["block_reward_microecho"].as_u64().unwrap_or(0);
    ...
    kv2(
        "Block reward",
        &format!("{} ECHO/block", echo_str(reward_micro)),
        &format!("({reward_micro} \u{03bc}ECHO)"),
    );
...
async fn cmd_estimate_fee(ctx: &Ctx<'_>, n_inputs: u32, n_outputs: u32) -> anyhow::Result<()> {
    ...
    let fee_micro = result["fee_microecho"].as_u64().unwrap_or(0);
    ...
    kv2(
        "Min relay fee",
        &format!("{} ECHO", echo_str(fee_micro)),
        &format!("({fee_micro} μECHO)"),
    );
    kv(
        "Base",
        &format!("{} μECHO", b["base"].as_u64().unwrap_or(0)),
    );
    kv(
        "Inputs",
        &format!("{} μECHO", b["input"].as_u64().unwrap_or(0)),
    );
    kv(
        "Outputs",
        &format!("{} μECHO", b["output"].as_u64().unwrap_or(0)),
    );
    kv(
        "State growth burned",
        &format!("{} μECHO", b["state_growth"].as_u64().unwrap_or(0)),
    );
    kv(
        "Miner claimable",
        &format!("{} μECHO", b["miner_claimable"].as_u64().unwrap_or(0)),
    );
    println!();
    println!(
...
async fn cmd_mempool_tx(ctx: &Ctx<'_>, txhash: &str) -> anyhow::Result<()> {
    ...
    if result.is_null() {
        ...
        println!("  It may have been confirmed. Use 'lostecho-cli tx {txhash}' to check.");
        return Ok(());
    }
    section("Mempool transaction");
    kv("tx_hash", result["tx_hash"].as_str().unwrap_or("?"));
    let fee = result["fee_microecho"].as_u64().unwrap_or(0);
    kv2(
        "Fee",
        &format!("{} ECHO", echo_str(fee)),
        &format!("({fee} \u{03bc}ECHO)"),
    );
    kv(
        "Inputs",
...
async fn cmd_mempool(ctx: &Ctx<'_>) -> anyhow::Result<()> {
    ...
    kv2(
        "Fee floor",
        &format!("{} ECHO", echo_str(fee_floor)),
        &format!("({fee_floor} μECHO minimum)"),
    );
    ...
    if is_tty() {
        println!(
            "  {}{:<20}  {:>12}  {:>5}  {:>3}→{:<3}  {:>5}  {}{}",
            BOLD, "tx hash", "fee (μECHO)", "pages", "in", "out", "class", "proof", RST
        );
    } else {
        println!(
            "  {:<20}  {:>12}  {:>5}  {:>3}→{:<3}  {:>5}  {}",
            "tx hash", "fee (μECHO)", "pages", "in", "out", "class", "proof"
        );
    }
    separator(100);
    ...
    for tx in txs.iter().take(show) {
        ...
        let fee = tx["fee_microecho"].as_u64().unwrap_or(0);
...
async fn cmd_address(
    ...
    if let Some(idx) = use_index {
        ...
        let bal = rpc(ctx, "walletGetBalance", &[])
            .await
            .ok()
            .and_then(|balance| balance["balance_echo"].as_f64())
            .unwrap_or(0.0);
    ...
    if list {
        ...
        println!();
        println!(
            "  {} Sends spend from this address; share it to receive ECHO here.",
            c!(DIM, "↑")
        );
        println!(
...
async fn cmd_balance(ctx: &Ctx<'_>) -> anyhow::Result<()> {
    ...
    let micro = result["balance_microecho"].as_u64().unwrap_or(0);
    let utxos = result["utxo_count"].as_u64().unwrap_or(0);
    let pending_out = result["pending_outbound_microecho"].as_u64().unwrap_or(0);
    let spendable = result["spendable_microecho"]
        .as_u64()
    ...
    if is_tty() {
        println!(
            "  {}Balance:{} {}{} ECHO{} {}({} \u{03bc}ECHO){}  {}({} UTXOs){}",
            CYN,
            RST,
            BOLD,
            echo_str(micro),
        ...
        println!(
            "  Balance:           {} ECHO  ({micro} \u{03bc}ECHO)  ({utxos} UTXOs)",
            echo_str(micro)
        );
    }
    ...
    if pending_out > 0 {
        if is_tty() {
            println!(
                "  {}Pending:{}  -{} ECHO outbound  {}({} \u{03bc}ECHO locked){}",
                YLW,
                RST,
                echo_str(pending_out),
                DIM,
                pending_out,
                RST
            );
            ...
            println!(
                "  Pending:           -{} ECHO outbound ({pending_out} \u{03bc}ECHO locked)",
                echo_str(pending_out)
            );
            println!("  Spendable:         {} ECHO", echo_str(spendable));
        }
    }
...
async fn cmd_utxos(ctx: &Ctx<'_>) -> anyhow::Result<()> {
    ...
    if utxos.is_empty() {
        println!(
            "  {}",
            c!(DIM, "(no UTXOs — run 'lostecho-cli scan' to discover)")
        );
        return Ok(());
    }
    ...
    let total: u64 = utxos
        .iter()
        .map(|u| u["value_microecho"].as_u64().unwrap_or(0))
        .sum();
    ...
    if is_tty() {
        println!(
            "  {}{:<8}  {:>20}  {:>14}  {:>7}  {:>9}  {}{}",
            BOLD, "slot", "creation id", "ECHO", "key", "at block", "address", RST
        );
    } else {
        println!(
            "  {:<8}  {:>20}  {:>14}  {:>7}  {:>9}  {}",
            "slot", "creation id", "ECHO", "key", "at block", "address"
        );
    }
    separator(96);
    ...
    for u in &utxos {
        ...
        let micro = u["value_microecho"].as_u64().unwrap_or(0);
    ...
    if is_tty() {
        ...
        println!("  {:<8}  {:>20}  {:>14}", "TOTAL", "", echo_str(total));
    }
...
async fn cmd_send(
    ...
    let amount_micro =
        parse_echo_amount(amount).with_context(|| format!("invalid amount {amount:?}"))?;

    if amount_micro == 0 {
        bail!("Amount cannot be zero.");
    }

    // Warn if the amount looks suspiciously large (> 1 000 000 ECHO = 1e12 μECHO).
    // This catches the common mistake of passing μECHO to a ECHO-denomination CLI.
    const MAX_WARN_MICRO: u64 = 1_000_000 * 1_000_000; // 1M ECHO in μECHO
    if amount_micro > MAX_WARN_MICRO {
        eprintln!(
            "⚠  Large amount: {} ECHO ({} μECHO). \
             Note: this CLI takes ECHO, not μECHO. Press Ctrl-C to cancel.",
            echo_str(amount_micro),
            amount_micro
        );
    }

    let fee_micro = match fee {
        Some(f) => parse_echo_amount(f).with_context(|| format!("invalid fee {f:?}"))?,
        None => 0, // auto
    };
    ...
    if dry_run {
        ...
        kv2(
            "Amount",
            &format!("{} ECHO", echo_str(amount_micro)),
            &format!("({amount_micro} μECHO)"),
        );
        let planned_fee = result["fee_microecho"].as_u64().unwrap_or(0);
        kv2(
            "Fee",
            &format!("{} ECHO", echo_str(planned_fee)),
            &format!(
                "({planned_fee} μECHO){}",
        ...
        let change = result["change_microecho"].as_u64().unwrap_or(0);
        kv2(
            "Change",
            &format!("{} ECHO", echo_str(change)),
            &format!("({change} μECHO)"),
        );
        println!();
        println!(
    ...
    if amount_micro >= 1_000_000_000 /* 1000 ECHO */ && is_tty() {
        print!(
            "  {} Send {}{} ECHO{} to {}{}{}? [y/N] ",
            c!(YLW, "⚠"),
            BOLD,
    ...
    match result {
        ...
        Ok(r) => {
            let actual_fee = r["fee_microecho"].as_u64().unwrap_or(0);
            ...
            kv2(
                "Fee",
                &format!("{} ECHO", echo_str(actual_fee)),
                &format!("({actual_fee} μECHO){auto_tag}"),
            );
            println!();
            println!(
        ...
        Err(e) => {
            ...
            let human = if msg.contains("Insufficient") || msg.contains("insufficient") {
                ...
                format!(
                    "Insufficient funds.\n\
                     \t  Requested: {} ECHO  ({amount_micro} μECHO)\n\
                     \t  Run 'lostecho-cli balance' to check your current balance.",
                    echo_str(amount_micro)
                )
            } else if msg.contains("no UTXO") || msg.contains("no utxo") {
                "No UTXOs available. Run 'lostecho-cli scan' to discover your coins.".into()
...
async fn cmd_history(
    ...
    if is_tty() {
        println!(
            "  {}  {:<8}  {:<8}  {:>14}  {:<16}  {}{}",
            BOLD, "block", "dir", "ECHO", "own[idx]", "counterparty", RST
        );
    } else {
        println!(
            "  {:<8}  {:<8}  {:>14}  {:<16}  {}",
            "block", "dir", "ECHO", "own[idx]", "counterparty"
        );
    }
    separator(88);
    ...
    for e in &filtered {
        ...
        let micro = e["amount_microecho"].as_u64().unwrap_or(0);
        ...
        let amount_str = format!("{:>14}{}", echo_str(micro), sign);
    ...
    if is_tty() {
        println!(
            ...
            "",
            GRN,
            format!("+ {} ECHO received", echo_str(received)),
            RED,
            format!("  - {} ECHO sent", echo_str(sent)),
            RST
        );
    } else {
        println!(
...
async fn cmd_scan(ctx: &Ctx<'_>) -> anyhow::Result<()> {
    ...
    let balance_microecho = result["balance_microecho"].as_u64().unwrap_or(0);
    ...
    println!(
        "  Index {}  \u{2022}  Snapshot height {}  \u{2022}  Found {} UTXO(s)  \u{2022}  Balance: {} ECHO",
        active_index,
        snapshot_height,
        found,
        echo_str(balance_microecho)
    );
...
async fn cmd_receipt(ctx: &Ctx<'_>, txhash: &str) -> anyhow::Result<()> {
    ...
    if is_tty() {
        ...
        eprintln!();
        eprintln!(
            "  {} Redirect to a file: lostecho-cli receipt {} > receipt.hex",
            c!(DIM, "Tip:"),
            txhash
        );
        eprintln!(
            "  {} Verify:              lostecho-cli verify $(cat receipt.hex)",
            c!(DIM, "Tip:")
        );
    }
...
async fn cmd_block_template(ctx: &Ctx<'_>, miner_addr: &str) -> anyhow::Result<()> {
    ...
    let claimable_fees = result["claimable_fees_microecho"].as_u64().unwrap_or(0);
    let coinbase_value = result["coinbase_value_microecho"].as_u64().unwrap_or(0);
    ...
    kv("Claimable fees", &format!("{claimable_fees} μECHO"));
    kv("Coinbase value", &format!("{coinbase_value} μECHO"));
    ...
    println!();
    println!(
        "  {} Search the nonce, then run `lostecho-cli submit-block {template_id} <32-char-lowercase-nonce-hex>`.",
        c!(DIM, "PoW:")
    );
    if !pow_fields.is_empty() {
...
fn input_limit_exceeded_message(error: &anyhow::Error) -> Option<String> {
    ...
    if rpc.code != i64::from(echo_rpc::types::WALLET_INPUT_LIMIT_EXCEEDED_CODE) {
        return None;
    }
    let max_inputs = rpc
        .data
...
async fn rpc(ctx: &Ctx<'_>, method: &str, params: &[Value]) -> anyhow::Result<Value> {
    let method_full = format!("lostecho_{method}");
            ...
            if e.is_connect() || e.is_timeout() {
                anyhow::anyhow!(
                    "Node is not responding.\n\
                     \tIs the lostecho daemon running?  Try: lostecho --mode miner\n\
                     \tRPC endpoint: {}\n\
                     \tOverride with --rpc <URL> or ECHO_RPC env var",
                    ctx.rpc
                )
            } else {
...
#[cfg(test)]
mod amount_tests {
    use super::{input_limit_exceeded_message, echo_str, parse_echo_amount, RpcCallError};

    #[test]
    fn decimal_amounts_are_exact_and_checked() {
        assert_eq!(parse_echo_amount("10.5").unwrap(), 10_500_000);
        assert_eq!(parse_echo_amount(".000001").unwrap(), 1);
        assert_eq!(parse_echo_amount("1.").unwrap(), 1_000_000);
        assert_eq!(echo_str(u64::MAX), "18446744073709.551615");
        assert!(parse_echo_amount("0.0000001").is_err());
        assert!(parse_echo_amount("NaN").is_err());
        assert!(parse_echo_amount("inf").is_err());
        assert!(parse_echo_amount("18446744073710").is_err());
    }

    #[test]
    fn input_limit_error_is_concise_and_uses_the_structured_limit() {
        let error = anyhow::Error::new(RpcCallError {
            code: i64::from(echo_rpc::types::WALLET_INPUT_LIMIT_EXCEEDED_CODE),
            message: echo_rpc::types::WALLET_INPUT_LIMIT_EXCEEDED_MESSAGE.to_string(),
