...
#[cfg(test)]
use echo_chain::consensus::wire_limits::MAX_HISTORY_STEP_TERMINAL_TRANSPORT_BYTES;
use echo_chain::{
...
mod tests {
    ...
    fn protocol() -> StreamProtocol {
        StreamProtocol::new("/echo/test/sync/history-step/1")
    }
    ...
    async fn out_of_bank_class_is_rejected() {
        let mut wire = response_wire(77, [0xA5; 32], [0x5A; 32], 1);
        wire[RESPONSE_HEADER_BYTES + 41] = echo_chain::HISTORY_STEP_CLASS_COUNT;
        assert_eq!(
            HistoryStepTerminalCodec::default()
                .read_response(&protocol(), &mut Cursor::new(wire))
                .await
