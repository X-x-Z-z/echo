...
//! Proof components for the Lostecho transaction system.
//!
//! This crate provides:
//!
...
//!
//! ```compile_fail
//! use echo_gkr::{prove_owner_auth_killshot, OwnerAuthProofKillShot};
//! ```
//!
//! ```compile_fail
//! use echo_gkr::auth_pcs::AuthMleOpeningProof;
//! ```
//!
//! ```compile_fail
//! let _ = echo_gkr::ghost_tx::ghost_authorization();
//! ```
...
mod production_auth_hard_cut_tests {
    ...
    fn selected_wallet_secret_surface_stays_opaque_and_errors_stay_redacted() {
        ...
        assert!(ghost.contains("LOSTECHO-GHOST-TX-SPEND-SECRET.0"));
