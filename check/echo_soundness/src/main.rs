...
fn main() -> Result<(), String> {
    ...
    for argument in std::env::args().skip(1) {
        match argument.as_str() {
            "--exact" => exact = true,
            "-h" | "--help" => {
                println!("Usage: echo_soundness [--exact]");
    ...
    println!("LOSTECHO SOUNDNESS CERTIFICATE");
    ...
    if exact {
        println!(
            ...
            echo_soundness::poseidon2b_cryptanalysis::SKIPPING_CLASS_REVIEWED_VERSION,
            echo_soundness::poseidon2b_cryptanalysis::SKIPPING_CLASS_PDF_SHA256
        );
    }
    println!(
    ...
    if exact {
        println!(
            "reviewed PDF: version={} sha256={}",
            echo_soundness::poseidon2b_cryptanalysis::NONLINEAR_SUBSPACES_REVIEWED_VERSION,
            echo_soundness::poseidon2b_cryptanalysis::NONLINEAR_SUBSPACES_PDF_SHA256
        );
    }
    println!(
